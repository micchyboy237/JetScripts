...
async def async_example(urls: List[str]) -> None:
    ...
    # Create a single stealth-configured context for the entire batch
    context = await setup_async_browser_session(headless=False)
    html_list = []
            ...
            if result["status"] == "completed" and result["html"]:
                ...
                if result["screenshot"]:
                    screenshot_path = os.path.join(
                        screenshots_dir, f"{safe_filename}.png"
                    )
...
def sync_example(
    ...
    # Create a single stealth-configured sync context for the entire batch
    context = setup_sync_browser_session(headless=False)
    html_list = []
    start = time.perf_counter()
    ...
    try:
        ...
        for result in results:
            ...
            if result["status"] == "completed" and html:
                ...
                sub_output_dir = (
                    OUTPUT_DIR / format_sub_source_dir(url) / "sync_results"
                )
                ...
                save_file(
                    ...
                    f"{sub_output_dir}/unstructured_elements.json",
                )
                ...
                original_docs = derive_by_header_hierarchy(
                ...
                if filtered_links and url_patterns:
                    link_urls = [o["url"] for o in filtered_links]
                    filtered_links = filter_links(link_urls, url_patterns)
                ...
                if result["screenshot"]:
                    ...
                    logger.success(
                        f"Scraped {result['url']}, links count: {len(filtered_links)}"
                    )
                else:
                    logger.success(
                        f"Scraped {result['url']}, links count: {len(filtered_links)}, "
