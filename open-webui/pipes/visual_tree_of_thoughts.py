...
version: 0.0.5.user-support

Prominant Notice of Change: Modified by Adam Outler to
fix crashing issue when user data is supplied. Fix based
on patch supplied by nschulzke.
https://github.com/open-webui/open-webui/discussions/6487
"""
...
class Node:
    ...
    def mermaid(self, offset=0, selected=None):
        padding = " " * offset
        msg = (
            f"{padding}{self.id}({self.id}:{self.visits} - {escape_mermaid(self.content[:25])})\n")
...
class Pipe:
    ...
    async def get_streaming_completion(
        ...
        print("ZOMFG I R0X0R TEH BIG 111!!!")
    ...
    async def stream_prompt_completion(self, prompt, **format_args):
        ...
        async for chunk in self.get_message_completion(
            ...
            print("OMFG I R0X0R TEH BIG 111@@")
            complete += chunk
    ...
    def get_response_content(self, response):
        try:
            ...
            logger.error(
                f'ResponseError: unable to extract content from "{
                    response[:100]}"'
            )
            return ""
