...
class _Tools:
    class Valves(pydantic.BaseModel):
        ...
        NETWORKING_ALLOWED: bool = pydantic.Field(
            default=True,
            description=f"Whether to allow network access during code execution; may be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}NETWORKING_ALLOWED.",
        )
        MAX_RUNTIME_SECONDS: int = pydantic.Field(
            ge=1,
            default=30,
            description=f"Maximum number of seconds code is given to run; may be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}MAX_RUNTIME_SECONDS.",
        )
        MAX_RAM_MEGABYTES: int = pydantic.Field(
            ge=0,
            default=128,
            description=f"Maximum number of megabytes that the interpreter has when running. Must run as root with host cgroups writable (`--mount=type=bind,source=/sys/fs/cgroup,target=/sys/fs/cgroup,readonly=false`) for this to work. Set to 0 to disable memory limits. May be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}MAX_RAM_MEGABYTES",
        )
        REQUIRE_RESOURCE_LIMITING: bool = pydantic.Field(
            default=True,
            description=f"Whether to enforce resource limiting, which requires cgroups v2 to be available; may be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}REQUIRE_RESOURCE_LIMITING.",
        )
        AUTO_INSTALL: bool = pydantic.Field(
            default=True,
            description=f"Whether to automatically install gVisor if not installed on the system; may be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}AUTO_INSTALL. Use the 'HTTPS_PROXY' environment variable to control the proxy used for download.",
        )
        CHECK_FOR_UPDATES: bool = pydantic.Field(
            default=True,
            description=f"Whether to automatically check for updates; may be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}CHECK_FOR_UPDATES. Use the 'HTTPS_PROXY' environment variable to control the proxy used for update checks.",
        )
        DEBUG: bool = pydantic.Field(
            default=False,
            description=f"Whether to produce debug logs during execution; may be overridden by environment variable {
                _VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}DEBUG.",
        )
    ...
    def __init__(self, valves):
        ...
        for valve_name, valve_value in valves.dict().items():
            ...
            try:
                ...
                raise ValueError(
                    f"Valve override {self.valves._VALVE_OVERRIDE_ENVIRONMENT_VARIABLE_NAME_PREFIX}{
                        valve_name}={valve_value}: bad value: {e}"
                )
            else:
    ...
    async def _run_code(
        ...
        if valves.CHECK_FOR_UPDATES:
            ...
            try:
                ...
                emitter.set_status_prefix(
                    f"[Code execution update check failed: {e}] ")
            else:
        ...
        async def _fail(error_message, status="SANDBOX_ERROR"):
            if debug:
                await emitter.fail(
                    f"[DEBUG MODE] {error_message}; language={
                        language}; code={code}; valves=[{valves}]"
                )
            else:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sandbox_") as tmp_dir:
                ...
                await emitter.citation(
                    document=[code], metadata=[
                        code], source={"name": "run_code"}
                )
                ...
                try:
                    ...
                    await emitter.fail(
                        f"Code timed out after {
                            valves.MAX_RUNTIME_SECONDS} seconds"
                    )
                    status = "TIMEOUT"
                    output = e.stderr
                    ...
                    await emitter.status(
                        ...
                        description=f"{
                            language_title} code executed successfully.",
                    )
                    status = "OK"
                ...
                if debug:
                    ...
                    await emitter.status(
                        ...
                        description=f"[DEBUG MODE] status={status}; output={
                            output}; valves=[{valves}]; debug={per_file_logs}",
                    )
            return {
                "status": status,
                "output": output,
...
class Sandbox:
    ...
    _LOG_FILENAME_TRUNCATE_RE = re.compile(
        r"^runsc\.log\.\d{8}-\d{6}(?:\.\d+)?\.")
    ...
    class _Libc:
        ...
        def mount(self, source, target, fs, options):
            if (
                ...
                raise OSError(
                    errno,
                    f"mount({source}, {target}, {fs}, {options}): {
                        os.strerror(errno)}",
                )
    ...
    class _Switcheroo:
            ...
            if do_resource_limiting:
                self._operations.extend(
                    (
                        ...
                        ("create_supervisor_cgroup",
                         self._create_supervisor_cgroup),
                        ...
                        ("set_sandbox_cgroup_limits",
                         self._set_sandbox_cgroup_limits),
        ...
        def _status(self):
            ...
            if self._checkpoint == self._operations[-1][0]:
                main_status = "OK"
            my_pid = os.getpid()
            status_line = f"{main_status} (euid={self._my_euid} egid={self._my_egid} pid={my_pid} do_resource_limiting={self._do_resource_limiting} initial_cgroup_name={
                self._initial_cgroup_name} codeeval_cgroup_name={self._codeeval_cgroup_name} controllers={self._cgroup_controllers})"
                ...
                for op, fn in self._operations[op_index + 1:]:
                    ...
                    for attempt in range(1, 4):
                        try:
                            ...
                            errors.append(
                                OSError(f"OSError in {op} (#{attempt}): {e}"))
                        except Exception as e:
                            do_log(f"Exception #{attempt}: {op}: {e}")
                            errors.append(
                                OSError(f"{op} failed (#{attempt}): {e}"))
                        else:
                            success = True
                            break
                        time.sleep(0.1)
                    ...
                    raise errors[0].__class__(
                        f"{first_exception} (other attempts: {
                            other_exceptions})"
                    )
        ...
        def cleanup(self):
            ...
            if self._codeeval_cgroup_name is not None:
                self._best_effort_remove_cgroup_subtree(
                    self._codeeval_cgroup_name)
        ...
        def _find_self_in_cgroup_hierarchy(self):
            ...
            for dirpath, _, subfiles in os.walk(
                ...
                if found_cgroup is not None:
                    raise OSError(
                        f"Found PID {my_pid} in two separate cgroups: {found_cgroup} and {
                            current_cgroup}; racing with another process?"
                    )
                found_cgroup = current_cgroup
            if found_cgroup is None:
                raise OSError(
                    f"PID {my_pid} could not be found in any cgroup (checked {num_checked} cgroups, {
                        num_except} exceptions; sample: {sample_exception})"
                )
        ...
        def _read_cgroup_controllers(self):
            cgroup_controllers = []
            with self._open(
                self._cgroup_path(self._initial_cgroup_name,
                                  "cgroup.controllers"), "rb"
                ...
                for line in cgroup_controllers_f:
                    for controller in line.strip().split(b" "):
                        if controller and controller not in cgroup_controllers:
                            cgroup_controllers.append(
                                controller.decode("ascii"))
        ...
        def _create_codeeval_cgroup(self):
            for counter in range(0, self._CGROUP_MAX_COUNT):
                codeeval_cgroup_name_candidate = f"{
                    self._CGROUP_NAME_PREFIX}{counter}"
            ...
            raise OSError(
                f"Out of cgroups (tried {initial_cgroup_path_prefix}NUM with NUM from 0 to {
                    self._CGROUP_MAX_COUNT-1} and they all already exist)"
            )
        ...
        def _add_cgroup_controllers_to(self, *cgroup_components):
            ...
            cgroup_components = tuple(
                cgroup_components) + ("cgroup.subtree_control",)
            ...
            try:
                with self._open(
                    ...
                    controllers_data = (
                        " ".join(
                            f"+{controller}" for controller in add_controllers)
                        + "\n"
                    )
                    cgroup_subtree_control_f.write(
                        controllers_data.encode("ascii"))
            except OSError as e:
                raise OSError(
                    f"Adding controllers {add_controllers} on {
                        cgroup_subtree_control_path}: {e}"
                )
            got_controllers = set()
            try:
                with self._open(
        ...
        def _set_cgroup_limits(self, *cgroup_components):
            ...
            if self._max_sandbox_ram_bytes is not None:
                try:
                    ...
                    raise OSError(
                        f"Trying to set max RAM limit to {
                            self._max_sandbox_ram_bytes} bytes: {e}"
                    )
        ...
        def _sanity_check_own_cgroup(self):
            ...
            assert cgroup_data.endswith(
                os.sep +
                os.path.join(self._CGROUP_SUPERVISOR_NAME, self._CGROUP_LEAF)
        ...
        def move_process_to_sandbox_leaf_cgroup_lambda(self):
            ...
            def _move(cgroup_path):
                ...
                try:
                    ...
                    raise OSError(f"Cannot open cgroup path {
                                  cgroup_path}: {e}")
                try:
                    f.write(b"0\n")
                except OSError as e:
                ...
                if "unshare" in os.__dict__:  # Python >= 3.12.
                    try:
                        ...
                        raise OSError(
                            f"unshare({clone_newcgroup}) failed: {e}")
                else:
                    import ctypes
                ...
                while new_pids_to_kill:
                    ...
                    for pid_to_kill in pids_to_kill:
                        try:
                            ...
                            pass
                    new_pids_to_kill = prev_pids_to_kill_len < len(
                        pids_to_kill)
            ...
            def _monitor():
                if self._max_sandbox_ram_bytes is not None:
                    try:
                        ...
                        print(
                            f"Warning: Failed to enforce code execution RAM: {
                                e}",
                            file=sys.stderr,
                        )
    ...
    def check_platform(cls):
        ...
        if uname.system != "Linux":
            raise cls.PlatformNotSupportedException(
                f"{uname.system} is not supported")
        if uname.machine not in ("x86_64", "aarch64"):
            raise cls.PlatformNotSupportedException(
                f"{uname.machine} is not supported")
    ...
    def install_runsc(cls):
        ...
        if not cls.runsc_needs_installation():
            return
        uname = platform.uname()
        release_url_dir = f"https://storage.googleapis.com/gvisor/releases/release/latest/{
            uname.machine}"
        os.makedirs(
    ...
    def _setup_sandbox(self):
        ...
        if self._persistent_home_dir is not None:
            if not os.path.isdir(self._persistent_home_dir):
                raise self.SandboxException(
                    f"Persistent home directory {
                        self._persistent_home_dir} does not exist"
                )
        ...
        for d in self.EMPTY_WRITABLE_DIRECTORIES:
            rootfs_subdir = os.path.join(
                rootfs_path, d.removeprefix(os.path.sep))
        ...
        for p in self.EXPOSED_SYSTEM_DIRECTORIES + self.EXPOSED_SYSTEM_FILES:
            if not os.path.islink(p):
                continue
            rootfs_subpath = os.path.join(
                rootfs_path, p.removeprefix(os.path.sep))
            os.makedirs(os.path.dirname(rootfs_subpath),
        ...
        for f in self.EXPOSED_SYSTEM_FILES:
            ...
            rootfs_subpath = os.path.join(
                rootfs_path, f.removeprefix(os.path.sep))
    ...
    def _run(self) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            runsc_argv = [
                ...
                # We already took care of cgroups manually.
                "--ignore-cgroups=true",
            ...
            started_marker_path = os.path.join(
                self._sandbox_shared_path, "started")
            ...
            try:
                ...
                if self._debug:
                    raise self.SandboxRuntimeException(
                        f"Sandbox failed to start: {e}; stderr: {
                            stderr}; logs: {json_logs}"
                    )
                raise self.SandboxRuntimeException(
                    f"Sandbox failed to start: {e} (turn on debug mode to see more information); stderr: {
                        stderr}; logs: {json_logs}"
                )
            finally:
            ...
            exit_code_path = os.path.join(
                self._sandbox_shared_path, "exit_code")
    ...
    def run(self) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            if not result.stdout:
                raise self.SandboxRuntimeException(
                    f"Subprocess interpreter did not produce any output (stderr: {
                        result.stderr})"
                )
            try:
        ...
        if os.path.isdir(self._logs_path):
            ...
            all_logs.append(
                (
                    "[meta]",
                    f"Logs path {
                        self._logs_path} does not exist or is not a directory.",
                )
            )
...
class UpdateCheck:
    ...
    def init_from_frontmatter(cls, file_with_frontmatter):
        ...
        if not contents.startswith('"""'):
            raise cls.VersionCheckError(
                f"Malformed file contents: {
                    contents[:min(8, len(contents))]}[...]"
            )
    ...
    def _get_latest_version(cls):
        ...
        try:
            try:
                releases_xml = urllib.request.urlopen(
                    url=cls.RELEASES_URL).read()
            except urllib.error.HTTPError as e:
                cls.LAST_UPDATE_CACHE = cls.VersionCheckError(
                    f"Failed to retrieve latest version: {
                        e} (URL: {cls.RELEASES_URL})"
                )
                raise cls.LAST_UPDATE_CACHE
            latest_version = None
            ...
            if latest_version is None:
                cls.LAST_UPDATE_CACHE = cls.VersionCheckError(
                    f"Failed to retrieve latest version: no release found (URL: {
                        cls.RELEASES_URL})"
                )
                raise cls.LAST_UPDATE_CACHE
        ...
        try:
            ...
            try:
                ...
                if got_status != want_status:
                    ...
                    success = False
                    print(
                        f"\u274c Self-test {name} failed: status was {
                            got_status}, expected {want_status}",
                        file=sys.stderr,
                    )
                else:
                    if debug:
    ...
    async def _local_run():
        ...
        if args.want_status:
            ...
            if got_status != args.want_status:
                raise RuntimeError(
                    f"Code evaluation status is {
                        got_status} but expected {args.want_status}"
                )
            print(
