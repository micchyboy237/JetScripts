...
def answer_yes_no(
    ...
    try:
        ...
        try:
            ...
            raise InvalidMethodError(f"Error applying chat template: {e}")
        ...
        logits_processors: list = make_logits_processors(logit_bias=logit_bias)
        sampler: make_sampler = make_sampler(temp=temperature, top_p=top_p)
        stop_tokens: List[int] = list(tokenizer.eos_token_ids)
        ...
        if method == "stream_generate":
            ...
            try:
                ...
                prompt_cache: Optional[Dict[int, List[int]]] = None
