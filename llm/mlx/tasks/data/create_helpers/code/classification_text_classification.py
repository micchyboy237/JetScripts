...
def load_model_components(model_path: ModelType) -> ModelComponents:
    ...
    Returns:
        ModelComponents containing the model and tokenizer.
    """
    try:
...
def validate_method(method: str) -> None:
    ...
    Raises:
        InvalidMethodError if the method is invalid.
    """
...
def create_system_prompt(choices: List[str]) -> str:
    ...
    Returns:
        System prompt with the given choices.
    """
...
def log_prompt_details(system_prompt: str, question: str, model_path: ModelType) -> None:
    ...
    Args:
        system_prompt: System prompt with the given choices.
        question: User question.
        model_path: Path to the model (defaults to DEFAULT_MODEL).

    Raises:
        logger.error: Logs the system prompt, tokenized system prompt, and user question.
    """
    logger.gray("System:")
...
def format_chat_messages(system_prompt: str, question: str) -> List[ChatMessage]:
    ...
    Returns:
        List of chat messages with the given choices.
    """
    return [
...
def encode_choices(tokenizer: TokenizerWrapper, choices: List[str]) -> Dict[str, List[int]]:
    ...
    Args:
        tokenizer: Tokenizer wrapper for the model.
        choices: List of possible answer choices.

    Returns:
        Dictionary of encoded choices with the given tokens.
    """
    choice_token_map = {}
...
def setup_generation_parameters(
    ...
    Args:
        tokenizer: Tokenizer wrapper for the model.
        choice_token_map: Dictionary of encoded choices with the given tokens.
        temperature: Sampling temperature.
        top_p: Top-p sampling parameter.

    Returns:
        Tuple containing logits processors, sampler, and stop tokens.
    """
...
def generate_answer_stream(
    ...
    """
    Generates an answer using the stream_generate method.

    Args:
        model_components: Model components containing the model and tokenizer.
        formatted_prompt: System prompt with the given choices.
        max_tokens: Maximum number of tokens to generate.
        logits_processors: Logit bias processor.
        sampler: Sampler for the model.
        stop_tokens: Tokens for the stop tokens.
        choices: List of possible answer choices.

    Returns:
        Tuple containing the answer, token ID, and generated tokens.
    """
    answer = ""
    token_id = -1
    generated_tokens = []
...
def generate_answer_step(
    ...
    """
    Generates an answer using the generate_step method.
...
def validate_answer(answer: str, choices: List[str]) -> None:
    ...
    Raises:
        InvalidOutputError: Logs the generated answer.
    """
...
def answer_multiple_choice(
    ...
    """
    Answers a multiple-choice question using the language model.
