...
from jet.features.search_and_chat import (
    DocumentTokensExceedsError,
    EvalContextError,
    run_scrape_search_chat,
)
...
if __name__ == "__main__":
    ...
    logger.log(
        "Schema:",
        format_json(output_cls.model_json_schema()),
        colors=["GRAY", "ORANGE"],
    )
    ...
    scraped_urls_results = scrape_urls(urls, max_depth=max_search_depth, query=query)
    ...
    for url, html in scraped_urls_results:
        ...
        results_dict = {"query": query, "url": url, "data": results}
        ...
        try:
            for response in response_generator:
                ...
                save_file("\n\n".join(contexts), os.path.join(sub_dir, "context.md"))
                save_file(
                    reranked_nodes_dict, os.path.join(sub_dir, "reranked_nodes.json")
                )
            ...
            save_file(e.eval_result, os.path.join(sub_dir, eval_context_result_file))
            logger.error(
