...
ollama_settings = initialize_ollama_settings()
...
num_tokens = ollama_settings.count_tokens(uber_doc.get_content())
...
async def main():
    ...
    eval_scores_ollama = await run_experiments(
        [uber_doc],
    ...
    llm = Ollama(model="llama3.1", request_timeout=300.0, context_window=4096)
    eval_scores_ollama_ts = await run_experiments(
        [uber_doc],
        position_percentiles,
        context_str,
        query_str,
    ...
    # llm = Anthropic(model="claude-2")
    llm = Ollama(model="llama3.1", request_timeout=300.0, context_window=4096)
    eval_scores_anthropic_ts = await run_experiments(
        [uber_doc],
        position_percentiles,
        context_str,
        query_str,
