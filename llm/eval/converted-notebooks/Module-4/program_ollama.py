...
from program_utils import parse_partial_json
...
class OllamaPydanticProgram(BaseLLMFunctionProgram[LLM]):
    """
    An Ollama-based function that returns a pydantic model.
    ...
    def from_defaults(
        ...
        **kwargs: Any,
    ) -> "OllamaPydanticProgram":
        llm = llm or Settings.llm

        if not isinstance(llm, Ollama):
            raise ValueError(
                "OllamaPydanticProgram only supports Ollama LLMs. " f"Got: {
                    type(llm)}"
            )
        ...
        tool_choice = tool_choice or _default_tool_choice(
            output_cls, allow_multiple)
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        ollama_fn_spec = to_openai_tool(
            self._output_cls, description=description)
    ...
    def stream_list(
        ...
        **kwargs: Any,
        ...
        ollama_fn_spec = to_openai_tool(
            list_output_cls, description=description)
        ...
        for stream_resp in chat_response_gen:
            ...
            new_obj_json_str, obj_start_idx = _get_json_str(
                fn_args, obj_start_idx)
    ...
    def _description_eval(self, **kwargs: Any) -> Optional[str]:
        ...
        if not (self._output_cls.__doc__ or description):
            raise ValueError(
                "Must provide description for your Pydantic Model. Either provide a docstring or add `description=<your_description>` to the method. Required to convert Pydantic Model to Ollama Function."
            )
