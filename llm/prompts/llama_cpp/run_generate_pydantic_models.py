...
from jet.llm.prompt_templates.llama_cpp import generate_pydantic_models
...
def remove_imports(python_code):
    ...
    python_code_without_imports = re.sub(pattern, "", python_code, flags=re.MULTILINE)

    return python_code_without_imports.strip()


pydantic_models_context = (
    f"Field Descriptions:\n{field_descriptions}\n\nQuery:\n{query}"
)
generated_python_code = generate_pydantic_models(context=pydantic_models_context)
