...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(os.path.dirname(__file__), "generated", "text_chunker.log")
        ),
...
def colorize_diff_line_with_substrings(
    line: str, is_addition: bool, is_deletion: bool, prev_line: str = ""
) -> str:
    """
        ...
        if prev_line.startswith("-"):
            ...
            for tag, _, _, j1, j2 in matcher.get_opcodes():
                if tag == "equal":
                    ...
                    result.append(
                        f'<span style="background-color: #99ff99;">{html_escape(line[j1:j2])}</span>'
                    )
...
def generate_unified_diff_summary(
    ...
    if GITPYTHON_AVAILABLE:
        ...
        try:
            logger.debug(
                "Running git diff --no-index with input length: %d", len(input_content)
            )
...
def generate_summary_data(
    input_text: str, chunks_regular: List[dict], chunks_fast: List[dict]
) -> SummaryData:
    ...
    for i, (reg, fast) in enumerate(zip(chunks_regular, chunks_fast)):
        if reg != fast:
            ...
            unified_diff, _ = generate_unified_diff_summary(
                reg_lines, fast_lines, f"chunk_{i}"
            )
            differences.append(
                {
                    "chunk_index": i,
                    "regular_content": reg_str,
                    "fast_content": fast_str,
...
if __name__ == "__main__":
    ...
    result: List[ChunkDict] = chunk_texts_with_data(
        sample,
        chunk_size=64,
        chunk_overlap=32,
        model=EMBED_MODEL,
    ...
    # Process with chunk_texts_with_data
    logger.info("Processing with chunk_texts_with_data...")
    result_fast: List[ChunkDict] = chunk_texts_with_data(
        sample,
        chunk_size=64,
        chunk_overlap=32,
        model=EMBED_MODEL,
    ...
    if chunks_regular == chunks_fast:
        ...
        logger.warning(
            "Chunk contents differ between methods! Generating HTML diff with colored summary..."
        )
        ...
        for i, (reg, fast) in enumerate(zip(chunks_regular, chunks_fast)):
            if reg != fast:
                ...
                html_table = differ.make_file(
                    reg_lines,
                    fast_lines,
                    fromdesc=f"Chunk {i} (Regular)",
                    todesc=f"Chunk {i} (Fast)",
                    context=True,
                    numlines=3,
                )

                # Generate Git-like unified diff summary with substring coloring
                _, unified_summary = generate_unified_diff_summary(
                    reg_lines, fast_lines, f"chunk_{i}"
                )
