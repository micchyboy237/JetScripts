...
from jet.code.markdown_utils import convert_html_to_markdown, parse_markdown
...
def initialize_output_directory(script_path: str, query: str) -> str:
    ...
    output_dir = os.path.join(
        script_dir, "generated", os.path.splitext(os.path.basename(script_path))[0]
    )
...
def initialize_search_components(
    llm_model: LLMModelType, embed_model: EmbedModelType, seed: int
) -> tuple[MLX, callable]:
...
async def fetch_search_results(
    query: str, output_dir: str, use_cache: bool = False
...
async def process_search_results(
    ...
    async for url, status, html in scrape_urls(
        selected_urls, num_parallel=top_n, limit=top_n, show_progress=True
    ):
    ...
    all_links = [
        ...
        if (
            link not in selected_urls
            if isinstance(link, str)
            else link["url"] not in selected_urls
        )
    ]
    ...
    for result in sorted_search_results:
        num_tokens = getattr(result, "num_tokens", result.metadata.get("num_tokens", 0))
...
def generate_response(query: str, context: str, mlx: MLX, output_dir: str) -> str:
...
def evaluate_results(
    query: str, context: str, response: str, llm_model: LLMModelType, output_dir: str
) -> None:
    ...
    save_file(
        eval_context_result,
        os.path.join(output_dir, "eval", "evaluate_context_relevance_result.json"),
    )
    eval_response_result = evaluate_response_relevancy(query, response, llm_model)
    save_file(
        eval_response_result,
        os.path.join(output_dir, "eval", "evaluate_response_relevance_result.json"),
    )


def group_search_results_by_source_url_for_context(
    ...
    for url, nodes in grouped.items():
        ...
        nodes = sorted(
            nodes,
            key=lambda n: (getattr(n, "doc_index", 0), getattr(n, "chunk_index", 0)),
        )
        for node in nodes:
            block += node.get_text() + "\n\n"
...
async def main():
    ...
    mlx, _ = initialize_search_components(args.llm_model, args.embed_model, args.seed)
    save_file(args.query, os.path.join(output_dir, "query.md"))
    browser_results = await fetch_search_results(
        args.query, output_dir, use_cache=args.use_cache
    )
    search_results = await process_search_results(
        browser_results, args.query, output_dir, max_length=1500
    )
    ...
    evaluate_results(args.query, context_md, response, args.llm_model, output_dir)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use cached search results if available",
    )
    p.add_argument(
        "--seed", type=int, default=45, help="Random seed for reproducibility"
    )
    return p.parse_args()
