...
def chunk_text(text: str, chunk_size: int, overlap: int = 30) -> List[ChunkResult]:
    ...
    while start < len(tokens):
        ...
        if chunk_text.strip():
            chunks.append(
                {"text": chunk_text, "index": index, "token_count": len(chunk_tokens)}
            )
...
def rank_chunks_by_similarity(
    ...
    query_embedding = generate_embeddings(query, EMBEDDING_MODEL, return_format="numpy")
...
def select_relevant_chunks(
    ...
    def merge_adjacent_chunks(
        start_chunk: ChunkResult, start_score: float
        ...
        for offset in [-1, 1]:
            ...
            while adj_index not in used_indices:
                for chunk, score in ranked_chunk_metadata:
                    ...
                    if (
                        chunk["doc_index"] == adj_index
                        and total_tokens + merged_tokens + token_count <= max_tokens
                    ):
                        if score >= start_score * 0.8:
                            merged_text = (
                                merged_text + " " + text
                                if offset == 1
                                else text + " " + merged_text
                            )
    ...
    for chunk, score in ranked_chunk_metadata:
        ...
        merged_text, chunk_tokens, merged_indices = merge_adjacent_chunks(chunk, score)
...
def main(query):
    ...
    save_file(
        {"query": query, "markdown_tokens": markdown_tokens},
    ...
    save_file(
        {"query": query, "header_docs": header_docs}, f"{output_dir}/header_docs.json"
    )

    header_nodes = [node for node in header_docs.as_nodes() if node.type == "header"]
    ...
    save_file(header_recursive_nodes, f"{output_dir}/header_recursive_nodes.json")

    chunk_headers = chunk_headers_by_hierarchy(resume_md, chunk_size)
    save_file(
        {"query": query, "chunk_headers": chunk_headers},
        f"{output_dir}/chunk_headers.json",
    )

    derived_chunks = derive_by_header_hierarchy(resume_md)
    save_file(
        {"query": query, "derived_chunks": derived_chunks},
    ...
    save_file(
        {"query": query, "selected_chunks": top_chunks, "total_tokens": total_tokens},
