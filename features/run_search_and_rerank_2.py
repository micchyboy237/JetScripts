...
from jet.code.markdown_utils._preprocessors import (
...
from jet.models.tokenizer.base import (
    count_tokens,
    get_string_detokenizer_fn,
    get_string_tokenizer_fn,
    get_tokenizer_fn,
)
...
def is_valid_header(header: Optional[str]) -> bool:
    ...
    if any(keyword in header.lower() for keyword in generic_keywords) or re.match(
        date_pattern, header
    ):
        return False
...
def separate_by_headers(paragraphs: List) -> List[Dict]:
    ...
    for paragraph in paragraphs:
        if paragraph.is_heading:
            ...
            header_tag = header_tag_match.group(1) if header_tag_match else None
            if header_tag and header_tag in header_tags:
                if (
                    current_section["content"] or current_section["header"]
                ) and is_valid_header(current_section["header"]):
                ...
                current_section = {
                    "header": f"{'#' * level} {paragraph.text}"
                    if paragraph.text
                    else None,
                    "content": [],
                    "xpath": paragraph.xpath,
                    "parent_header": f"{'#' * parent['level']} {parent['content']}"
                    if parent
                    else None,
                    "parent_level": parent["level"] if parent else None,
                    "level": level,
                }
                header_stack.append({"level": level, "content": paragraph.text})
            else:
                current_section["content"].append(paragraph.text)
        else:
            current_section["content"].append(paragraph.text)
    if (current_section["content"] or current_section["header"]) and is_valid_header(
...
def merge_similar_docs(
    embeddings: List[Dict], similarity_threshold: float = 0.8
    ...
    for cluster_texts in clusters:
        cluster_docs = [
            embeddings[i] for i, text in enumerate(texts) if text in cluster_texts
        ]
...
async def prepare_for_rag(
    ...
    for source in sources[:sources_limit] if sources_limit else sources:
        sub_source_dir = format_sub_source_dir(source)
        ...
        if is_url:
            async for url, status, html in scrape_urls([source], show_progress=True):
                if status == "completed" and html:
                    content = html
                    links = set(scrape_links(html, source))
                    links = [
                        link
                        for link in links
                        if (
            ...
            if not source.endswith((".md", ".txt")):
                logger.warning(
                    f"Unsupported file extension for {source}. Expected .md or .txt"
                )
                continue
            try:
                ...
                logger.warning(f"Failed to read {source}: Invalid encoding")
                continue
            except Exception as e:
                logger.warning(f"Failed to read file {source}: {str(e)}")
                continue

        if content:
            if is_url:
                content = preprocess_html(content)
                save_file(content, f"{sub_output_dir}/page.html")
                doc_markdown = convert_html_to_markdown(content)
            else:
                doc_markdown = content
            ...
            _tokenizer = (
                get_string_tokenizer_fn(tokenizer_model) if tokenizer_model else None
            )
            _detokenizer_fn = (
                get_string_detokenizer_fn(tokenizer_model) if tokenizer_model else None
            )
            ...
            token_counts: List[int] = count_tokens(
                tokenizer_model,
                [
                    clean_markdown_links(f"{doc['header']}\n{doc['content']}")
            ...
            for doc, num_tokens in zip(documents, token_counts):
                ...
                if doc["parent_header"]:
                    doc["parent_header"] = clean_markdown_links(doc["parent_header"])
                ...
                if (
                    doc["header"].strip()
                    and doc["word_count"] >= 8
                    and readability["mtld"] > 0.0
                    and not link_to_text_ratio_result["is_link_heavy"]
                ):
            ...
            for i, doc in enumerate(documents):
                content_tokens = (
                    _tokenizer(doc["content"])
                    if _tokenizer
                    else word_tokenize(doc["content"])
                )
                if i > 0 and chunk_overlap > 0 and doc["chunk_index"] > 0:
                    prev_overlap = (
                        prev_content_tokens[-chunk_overlap:]
                        if len(prev_content_tokens) >= chunk_overlap
                        else prev_content_tokens
                    )
            ...
            generated_embeddings = generate_embeddings(texts, model, show_progress=True)
            embeddings = []
            ...
            embedding_matrix = np.array(
                [doc["embedding"] for doc in embeddings]
            ).astype("float32")
        ...
        if (
            total_high_score_tokens >= TARGET_HIGH_SCORE_TOKENS
            or total_tokens >= TARGET_TOKENS
        ):
            logger.info(
                f"Stopping processing: {total_tokens} tokens collected from source: {source}."
            )
            break

    if not all_documents:
        logger.warning("No documents collected after processing all sources.")
        return None, [], model, [], []
    ...
    for result in all_results:
        ...
        source_summary[source]["scores"].append(float(score))
        source_summary[source]["num_tokens"] += num_tokens
        for lower, upper in score_ranges:
            if lower <= score < upper:
                source_summary[source]["score_categories"][f"{upper}"] += 1
                break
    header_insights = {}
    for result in all_results:
        source = result.get("source", "Unknown Source")
        ...
        score = result.get("score", 0.0)
        if source not in header_insights:
            header_insights[source] = {}
        if header not in header_insights[source]:
            header_insights[source][header] = {
                "count": 0,
                "scores": [],
                "avg_score": 0.0,
                "max_score": 0.0,
        ...
        header_insights[source][header]["count"] += 1
        header_insights[source][header]["scores"].append(float(score))

    for source in header_insights:
        for header in header_insights[source]:
            scores = header_insights[source][header]["scores"]
            if scores:
                header_insights[source][header]["avg_score"] = sum(scores) / len(scores)
                header_insights[source][header]["max_score"] = max(scores)
                header_insights[source][header]["min_score"] = min(scores)
            else:
                header_insights[source][header]["avg_score"] = 0.0
                header_insights[source][header]["max_score"] = 0.0
                header_insights[source][header]["min_score"] = 0.0
    ...
    summary_data = [
        {
            ...
            "header_insights": [
                {
                    "header": header,
                    "count": header_insights[source][header]["count"],
                    "avg_score": header_insights[source][header]["avg_score"],
                    "max_score": header_insights[source][header]["max_score"],
                    "min_score": header_insights[source][header]["min_score"],
                }
                for header in header_insights.get(source, {})
            ],
        }
    ...
    logger.info(
        f"Saved summary.json with insights for {len(source_summary)} sources to {OUTPUT_DIR}/summary.json"
    )
    logger.info(f"Clustering {len(all_documents)} documents...")
    return all_orig_documents, all_documents, model, all_results, all_links
...
def compute_similarity(
    ...
    parent_header_clean = strip_hashtags(parent_header) if parent_header else ""
    ...
    texts_to_embed = [text for text in [combined_header_clean, content_clean] if text]
    similarities = {"query_vs_combined_header": 0.0, "query_vs_content": 0.0}
    ...
    if content_clean:
        ...
        similarities["query_vs_content"] = cosine_similarity(
            [query_embedding], [text_embeddings[content_idx]]
        )[0][0]
        computed_similarities.append(similarities["query_vs_content"])

    if mode == "max":
        score = float(np.max(computed_similarities)) if computed_similarities else 0.0
    else:
        score = float(np.mean(computed_similarities)) if computed_similarities else 0.0
...
def query_rag(
    ...
    for idx, score, ind_scores in zip(I[0], scores, individual_scores):
        ...
        selected_doc_id = (
            merge_entry["original_doc_ids"][0]
            if merge_entry and len(merge_entry["original_doc_ids"]) > 1
            else doc_id
        )
        rank += 1
        results.append(
...
def group_results_by_source_for_llm_context(
    ...
    high_score_docs = [doc for doc in documents if doc["score"] >= 0.6]
    med_score_docs = [doc for doc in documents if 0.35 <= doc["score"] < 0.6]

    high_score_docs_sorted = sorted(
        high_score_docs, key=lambda x: x.get("score", 0), reverse=True
    )
    med_score_docs_sorted = sorted(
        med_score_docs, key=lambda x: x.get("score", 0), reverse=True
    )
...
def rerank_by_headers_documents_relevance(
    query: str, docs: List[Dict], embed_model: EmbedModelType = "all-MiniLM-L12-v2"
):
    texts = []
    for d in docs:
        _texts = []
    ...
    save_file(reranked_results, f"{OUTPUT_DIR}/reranked_results_by_keywords.json")
...
def select_best_from_group(
    ...
    candidates = [id_to_result[doc_id].get("content", "") for doc_id in group]
    ids = group
...
async def main():
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Reset and ignore cached search results",
    )
    p.add_argument(
        "-s",
        "--sources",
    ...
    save_file(
        reranked_results_by_keywords, f"{OUTPUT_DIR}/contexts_reranked_results.json"
    )
    ...
    save_file(contexts_grouped_results, f"{OUTPUT_DIR}/contexts_grouped_results.json")
    ...
    save_file(final_results, f"{OUTPUT_DIR}/contexts_before_max_filter.json")
    ...
    save_file(
        {"num_tokens": count_tokens(llm_model, context)},
