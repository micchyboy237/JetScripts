...
async def main(query):
    ...
    async for url, status, html in scrape_urls(urls, show_progress=True):
        if status == "started":
            ...
            save_file(preprocessed_html,
                      f"{sub_output_dir}/page_preprocessed.html")

            doc_markdown = convert_html_to_markdown(
                preprocessed_html, ignore_links=False)
            # all_links, doc_markdown = extract_markdown_links(
            #     doc_markdown, base_url=url, ignore_links=True)
            doc_markdown = clean_markdown_links(doc_markdown)
            ...
            save_file({
                ...
                "mtld": analyze_readability(preprocessed_html)["mtld"],
                "mtld_category": analyze_readability(preprocessed_html)["mtld_category"],
