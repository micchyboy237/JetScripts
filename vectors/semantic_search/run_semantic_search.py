...
if __name__ == "__main__":
    ...
    texts_token_counts: List[int] = count_tokens(embed_model, texts, prevent_total=True)
    docs = [
        {
            "id": doc["id"],
    ...
    texts_to_search = [
        "\n".join([chunk["header"], chunk["content"]]).strip() for chunk in chunks
    ]
    ...
    for doc_id, chunks in doc_scores.items():
        ...
        for chunk in chunks:
            ...
            for query_term, score in query_scores.items():
                query_max_scores[query_term] = max(query_max_scores[query_term], score)
    ...
    save_file(
        {"query": query, "results": mapped_docs_with_scores[:10]},
        f"{OUTPUT_DIR}/final_results_top_10.json",
    )
    save_file(
        {"query": query, "results": mapped_docs_with_scores[:20]},
        f"{OUTPUT_DIR}/final_results_top_20.json",
    )
    high_score_results = [doc for doc in mapped_docs_with_scores if doc["score"] >= 0.7]
    save_file(
        {
            "query": query,
