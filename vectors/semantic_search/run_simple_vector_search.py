...
if __name__ == "__main__":
    ...
    save_file({
        ...
        "results": [{"doc_index": idx, "tokens": tokens, "text": text} for idx, (tokens, text) in enumerate(zip(token_counts, documents))]
    ...
    query = queries[0]
    search_results = search_engine.search(query, top_k=len(documents))
    print(f"\nQuery: {query}")
    print("Top matches:")
    for num, (doc, score) in enumerate(search_results[:10], 1):
