...
def embed_texts(texts: list[str], model: str = "embeddinggemma", url: str = OLLAMA_BASE_URL, batch_size: int = 32) -> np.ndarray:
    ...
    embed_stream = generate_embeddings_stream(texts, model, url=url, batch_size=batch_size, return_format="numpy", show_progress=True)
    embed_results = []
    for embeddings in embed_stream:
        embed_results.extend(embeddings)
    return np.array(embed_results)
...
if __name__ == '__main__':
    ...
    embed_results = embed_texts(texts, model, url=embed_url, batch_size=batch_size)
