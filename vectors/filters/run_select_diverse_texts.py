...
def load_and_process_common_data(embed_model: EmbedModelType) -> tuple[list[dict], list[str]]:
    ...
    diverse_texts = select_diverse_texts(
    ...
    save_file({
        "count": len(diverse_texts),
        "total_tokens": sum(token_counts),
        "texts": [{
            "tokens": tokens,
            "text": text,
        } for tokens, text in zip(token_counts, diverse_texts)],
    }, f"{OUTPUT_DIR}/diverse_texts.json")
    ...
    diverse_headers = select_diverse_texts(
    ...
    save_file({
        "count": len(diverse_headers),
        "total_tokens": sum(token_counts),
        "texts": [{
            "tokens": tokens,
            "text": text,
        } for tokens, text in zip(token_counts, diverse_headers)],
...
def main(mode: ClusteringMode, docs: list[dict], documents: list[str]):
    ...
    doc_id_to_embeddings = {doc["metadata"]["doc_id"]: emb for doc, emb in zip(docs, embeddings)}
    ...
    for group in grouped_similar_texts:
        ...
        cluster_diverse_texts.extend(diverse_texts)
    ...
    save_file({
        ...
        "texts": [{
            "tokens": tokens,
            "text": text,
        } for tokens, text in zip(token_counts, cluster_diverse_texts)],
    }, f"{mode_output_dir}/diverse_cluster_texts.json")
