# practical_02_graph_rag_retrieval.py
import json
...
def _build_tech_knowledge_graph(d_model: int = 768):
    """Build a consistent 768-dim knowledge graph (instead of using sample 256-dim one)."""
    schema = Schema(
    ...
    kg = GraphRAG(d_model=d_model).knowledge_graph  # Reuse GraphRAG's KG with correct d_model
    kg.schema = schema

    # Hardcoded entities (will be embedded with embeddinggemma → 768-dim)
    entities = [
...
def practical_02_graph_rag_retrieval():
    ...
    # Build consistent 768-dim KG
    kg, entities, relations = _build_tech_knowledge_graph(d_model=768)
    graph_rag = GraphRAG(d_model=768)
    graph_rag.knowledge_graph = kg  # Replace with our correctly sized KG

    # Generate real 768-dim embeddings
    logger.info("Generating 768-dim embeddings for entities...")
    ...
    # Generate queries
    prompt = "Generate 4 natural questions about TechCorp, its people, products, and location. Return as JSON list."
    save_file(prompt, str(example_dir / "llm" / "query_prompt.md"))

    logger.info("Generating queries with LLM...")
    query_output = ""
    for chunk in llm.generate(prompt, temperature=0.7, max_tokens=300, stream=True):
        query_output += chunk
    save_file(query_output, str(example_dir / "llm" / "query_response.md"))

    try:
        queries = json.loads(query_output)
        ...
        logger.warning("Failed to parse queries → using fallback")

    save_file(json.dumps(queries, indent=2), str(example_dir / "queries" / "queries.json"))
    ...
    logger.info("PRACTICAL 2 COMPLETE — No more dimension errors!")
