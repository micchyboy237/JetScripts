...
class EnhancedHierarchicalMemory(HierarchicalMemory):
    ...
    def _token_count(self, text: str, model: str = "gpt-4o") -> int:
        ...
        enc = tiktoken.encoding_for_model(
            model
        )  # automatically picks cl100k_base or o200k_base
        return len(enc.encode(text))

    def add_context(
        self, embedding: np.ndarray, text: str, source: str, chunk_index: int
    ) -> Dict[str, int]:
        token_count = self._token_count(text)
        chunk = MemoryChunk(
            embedding, text, source, token_count, chunk_index
        )  # include chunk_index
        ...
        while self._total_tokens(self.short_term) > self.short_term_size:
            ...
            compressed_emb = self._compress_context(
                oldest.embedding, self.compress_medium
            )
            ...
            self.medium_term.append(
                MemoryChunk(
                    ...
                    compressed_text,
                    oldest.source,
                    oldest.token_count // self.compression_ratio,
                    oldest.chunk_index,
                )
            )
        ...
        while self._total_tokens(self.medium_term) > self.medium_term_size:
            ...
            compressed_emb = self._compress_context(
                oldest.embedding, self.compress_long
            )
    ...
    def retrieve_relevant(
        self, query_emb: np.ndarray, max_tokens: int = 256
        ...
        combined_emb = (
            np.concatenate(selected_embs, axis=0)
            if selected_embs
            else np.zeros((0, self.d_model))
        )
        combined_text = "\n\n".join(selected_texts)
...
class RAGContextProcessor(ContextProcessor):
    def __init__(
        self, d_model: int = 768, mechanism: str = "streaming"
    ):  # ← d_model changed from 384 to 768
        super().__init__(d_model, mechanism)
        self.memory = EnhancedHierarchicalMemory(d_model)

    def process_chunk(
        self, embedding: np.ndarray, text: str, source: str = "", chunk_index: int = 0
    ) -> np.ndarray:
        if self.memory.short_term:
            relevant_emb, _, _ = self.memory.retrieve_relevant(
...
def scrape_urls_playwright(urls: List[str]) -> Iterator[Tuple[str, str]]:
    ...
    """
    print(
        f"Scraping {len(urls)} URLs using Playwright ({min(10, len(urls))} parallel)..."
    )
    for result in scrape_urls_sync(
        ...
        if status == "completed" and html:
            ...
            if text.strip():
                ...
                yield url, text
                print(
                    f"  ✓ Successfully scraped: {url[:70]}{'...' if len(url) > 70 else ''}"
                )
            else:
...
def build_rag_prompt(
    query: str, context: str, metadata: List[Dict]
) -> List[Dict[str, str]]:
...
def main():
    ...
    search_query = input(
        "\nEnter search query for URLs (press Enter for default): "
    ).strip()
    if not search_query:
    ...
    try:
        search_results = search_searxng(query=search_query, count=10, use_cache=True)
    ...
    urls = [
        line.strip()
        for line in INPUT_URLS_FILE.read_text().splitlines()
        if line.strip()
    ]
    print(f"Found {len(urls)} URLs to index")

    embedder = LlamacppEmbedding(model=EMBED_MODEL, use_cache=True, verbose=True)
    ...
    try:
        ...
        print(
            " Make sure `llama-server --model embeddinggemma ... --port 8081` is running"
        )
        exit(1)
    ...
    chunk_embeddings = embedder.encode(
        chunk_texts, return_format="numpy", batch_size=128, show_progress=True
    )
    if chunk_embeddings.ndim == 1:
        chunk_embeddings = chunk_embeddings.reshape(1, -1)

    chunk_embeddings_mappings = {
        chunk["id"]: chunk_emb
        for chunk_emb, chunk in zip(chunk_embeddings, chunks_with_data)
    }
        ...
        save_file(
            {
                ...
                "tokens": {
                    ...
                    "ave": sum(doc_token_counts) // len(doc_token_counts)
                    if doc_token_counts
                    else 0,
    ...
    print(
        f" Short: {stats['short']}, Medium: {stats['medium']}, Long: {stats['long']} tokens"
    )
    ...
    _, context, metadata = processor.memory.retrieve_relevant(
        query_emb, MAX_RETRIEVAL_TOKENS
    )
    print(
