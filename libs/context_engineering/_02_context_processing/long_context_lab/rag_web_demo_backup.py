...
def chunk_text(
    text: str, embedder: LlamacppEmbedding
    ...
    raw_sentences = [
        s.strip() for s in text.split(".") if len(s.strip()) > SENTENCE_MIN_CHARS // 10
    ]
    ...
    sentence_embeddings = embedder.encode(
        sentences, return_format="numpy", batch_size=64, show_progress=True
...
def main():
    ...
    # 2. Initialize
    embedder = LlamacppEmbedding(model=EMBEDDER_MODEL, use_cache=True, verbose=True)
    ...
    processor = RAGContextProcessor(
        d_model=768, mechanism="streaming"
    ...
    _, context_text, metadata = processor.memory.retrieve_relevant(
    ...
    for item in metadata[:3]:
        print(
            f"  • [{item['level'].upper()}] {item['source'][:60]}... (score: {item['score']:.3f})"
        )
