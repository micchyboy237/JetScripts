...
def demo_subtitle_metadata() -> None:
    ...
    overlay = LiveSubtitlesOverlay.create(
        title="Live Subtitles – Full Metadata + RMS Demo (2026)",
    ...
    def feed_subtitles() -> None:
        for seg in demo_segments:
            ...
            overlay.add_message(
                ...
                normalized_rms=seg.get("normalized_rms"),
                normalized_rms_label=seg.get("normalized_rms_label"),
            )
            ...
            table.add_row(
                ...
                if len(seg["translated_text"]) > 60
                else seg["translated_text"],
            )
            table.add_row(
                "source_text",
                seg.get("source_text", "[none]")[:60] + "..."
                if seg.get("source_text") and len(seg["source_text"]) > 60
                else seg.get("source_text", "[none]"),
            )
            ...
            table.add_row("normalized_rms", f"{seg.get('normalized_rms', 0.0):.3f}")
            table.add_row("normalized_rms_label", seg.get("normalized_rms_label", "—"))
            rprint(table)
            print()
