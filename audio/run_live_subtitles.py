...
if __name__ == "__main__":
    ...
    for speech_seg, seg_audio_np, full_audio_np in data_stream:
        # Create a shallow copy and convert sample counts → seconds
        ...
        # Use the idx from SpeechSegment as the authoritative segment number
        seg_number: int = speech_seg["idx"] + 1
        seg_dir = Path(OUTPUT_DIR) / "segments" / f"segment_{seg_number:03d}"
        seg_dir.mkdir(parents=True, exist_ok=True)

        # Metadata that travels with the segment through the async pipeline
        meta: dict = {
