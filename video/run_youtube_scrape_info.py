...
from jet.video.utils import download_audio, find_audio
from jet.video.youtube.youtube_info_extractor import YoutubeInfoExtractor
from jet.video.youtube.youtube_scrape_info import transcribe_youtube_video_info_and_chapters
...
async def main():
    ...
    chunk_duration = 60
    model_size = "small"
    overlap_duration = 5.0  # Add overlap to prevent information loss
    ...
    transcription_info = []
    for segments, info in transcribe_youtube_video_info_and_chapters(
        video_url,
        audio_path,
        info,
        audio_dir,
        ...
        print(f"Saved {len(segments)} segments and info for chunk")
