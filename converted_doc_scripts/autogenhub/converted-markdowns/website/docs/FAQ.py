...
"""
logger.info("## Agents keep thanking each other when using `gpt-3.5-turbo`")
...
"""
logger.info("## ChromaDB fails in codespaces because of old version of sqlite3")

def print_messages(recipient, messages, sender, config):
    if "callback" in config and  config["callback"] is not None:
        callback = config["callback"]
        callback(sender, recipient, messages[-1])
    logger.debug(f"Messages sent to: {recipient.name} | num messages: {len(messages)}")
...
For example,
"""
logger.info("## How to get last message ?")
...
"""
logger.info("You can also use Redis cache instead of disk cache. For example,")
...
"""
logger.info("## Agents are throwing due to docker not running, how can I resolve this?")
...
E.g.:
"""
logger.info("### If you want to run code execution in docker")
...
"""
logger.info("## None of the devcontainers are building due to "Hash sum mismatch", what should I do?")
...
"""
logger.info("This is a combination of StackOverflow suggestions [here](https://stackoverflow.com/a/48777773/2114580) and [here](https://stackoverflow.com/a/76092743/2114580).")
