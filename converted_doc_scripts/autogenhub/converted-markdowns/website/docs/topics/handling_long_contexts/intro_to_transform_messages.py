...
Install `autogen`:
"""
logger.info("# Introduction to Transform Messages")
...
"""
logger.info("### Exploring and Understanding Transformations")
...
"""
logger.info("#### Example 1: Limiting the Total Number of Messages")
...
pprint.plogger.debug(processed_messages)
...
"""
logger.info("#### Example 2: Limiting the Number of Tokens")
...
"""
logger.info("We can see that we were able to limit the number of tokens to 3, which is equivalent to 3 words for this instance.")
...
pprint.plogger.debug(processed_short_messages)
...
"""
logger.info("### Apply Transformations Using Agents")
...
def test(assistant: autogen.ConversableAgent, user_proxy: autogen.UserProxyAgent):
    ...
    try:
        ...
        logger.debug(f"Encountered an error with the base assistant: \n{e}")

"""
The first run will be the default implementation, where the agent does not have the `TransformMessages` capability.
"""
logger.info("The first run will be the default implementation, where the agent does not have the `TransformMessages` capability.")
...
"""
logger.info("Now let's add the `TransformMessages` capability to the assistant and run the same test.")
