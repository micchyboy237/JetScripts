...
"""
logger.info("We define the two functions.")
...
"""
logger.info("And then associate them with the `user_proxy` for execution and `functionbot` for the LLM to consider using them.")
...
"""
logger.info("Finally, we start the conversation with a request for help from our customer on their upcoming trip to New York and the Euro they would like exchanged to USD.")
...
"""
logger.info("After the conversation has finished, we'll print out the summary.")

logger.debug(f"Here's the LLM summary of the conversation:\n\n{res.summary['content']}")
