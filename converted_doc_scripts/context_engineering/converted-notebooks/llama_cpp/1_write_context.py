...
def generate_joke(state: State) -> dict[str, str]:
    ...
    """
    logger_simple_joker = CustomLogger(
        "simple_joker", filename=f"{AGENTS_DIR}/simple_joker.log"
    )
...
info = get_model_ctx_embd_size(EMBED_MODEL)


embeddings = EmbedLlamaCpp(model=EMBED_MODEL)
store = InMemoryStore(
    index={
        "embed": embeddings,
        "dims": info["embd_dims"],
...
def generate_joke_with_memory(state: State, store: BaseStore) -> dict[str, str]:
    ...
    logger_local = CustomLogger(
        "example_3_thread_isolated_memory", filename=str(log_file)
    )
    ...
    logger_memory_joker = CustomLogger(
        "memory_joker", filename=f"{AGENTS_DIR}/memory_joker.log"
    )
...
def example_1_basic_joke_generation():
    ...
    log_file = example_dir / "main.log"
    logger_local = CustomLogger(
        "example_1_basic_joke_generation", filename=str(log_file)
    )
...
def example_2_memory_store_write_read():
    ...
    log_file = example_dir / "main.log"
    logger_local = CustomLogger(
        "example_2_memory_store_write_read", filename=str(log_file)
    )
    logger_local.orange(f"Example 2 logs: {log_file}")

    # Use result from example 1
    prior_result_path = (
        Path(BASE_OUTPUT_DIR) / "example_1_basic_joke_generation" / "result.json"
    )
    ...
    (example_dir / "stored_items.json").write_text(
        json.dumps(make_serializable(stored_items), indent=2)
    )
...
def example_3_thread_isolated_memory():
    ...
    (example_dir / "thread1_result.json").write_text(
        json.dumps(make_serializable(result1), indent=2)
    )

    latest_state = chain.get_state(config1)
    (example_dir / "thread1_latest_state.json").write_text(
        json.dumps(make_serializable(latest_state), indent=2, default=str)
    )

    config2 = {"configurable": {"thread_id": "2"}}
    result2 = chain.invoke({"topic": "cats"}, config2)
    (example_dir / "thread2_result.json").write_text(
        json.dumps(make_serializable(result2), indent=2)
    )
