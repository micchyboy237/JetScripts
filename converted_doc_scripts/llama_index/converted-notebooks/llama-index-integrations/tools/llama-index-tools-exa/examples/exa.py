...
To get started, you will need an [MLX api key](https://platform.openai.com/account/api-keys) and an [Exa API key](https://dashboard.exa.ai/api-keys)
...
"""
logger.info("You can constrain results to only return from specified domains (or exclude domains)")
...
"""
logger.info("You can turn off autoprompt, enabling more direct and fine grained control of Exa querying.")
...
async def run_async_code_6efac94f():
    ...
    return 
 = asyncio.run(run_async_code_6efac94f())
...
async def run_async_code_98fa2491():
    async def run_async_code_f50cf5da():
        response = await agent.run("Tell me more about the recent news on semiconductors")
        return response
    response = asyncio.run(run_async_code_f50cf5da())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_98fa2491())
logger.success(format_json(response))
