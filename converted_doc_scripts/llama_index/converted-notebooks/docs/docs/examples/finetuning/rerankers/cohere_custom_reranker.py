...
MLX - For creating synthetic dataset.
...
for rerank_name, reranker in RERANKERS.items():
    ...
    class CustomRetriever(BaseRetriever):
        ...
        async def aretrieve(
            ...
            async def run_async_code_81836ff9():
                return await self._aretrieve(str_or_query_bundle)
                return 
             = asyncio.run(run_async_code_81836ff9())
    ...
    async def async_func_102():
        eval_results = retriever_evaluator.evaluate_dataset(
            qa_dataset_uber_val
        )
        return eval_results
    eval_results = asyncio.run(async_func_102())
