...
"""
logger.info("# Agentic GraphRAG Implementation with LlamaIndex and Vertex AI")
...
def download_file(url, file_path):
    ...
    logger.debug(f"Downloaded file from {url} to {file_path}")
    ...
    async def _aextract(self, node: BaseNode) -> BaseNode:
        ...
        try:
            async def async_func_91():
                ...
                return llm_response
            llm_response = asyncio.run(async_func_91())
...
"""
logger.info("### Create nodes/ chunks from the text.")
...
"""
logger.info("### Build PropertyGraphIndex using `GraphRAGExtractor` and `GraphRAGStore`")
...
"""
logger.info("### Building an Agent Reasoning Loop")
...
async def async_func_73():
    response = await agent.run(
        "what are agent roles in MetaGPT, "
        "and then how they communicate with each other."
    )
    return response
response = asyncio.run(async_func_73())
