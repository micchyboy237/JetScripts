...
async def run_async_code_7e95924c():
    async def run_async_code_1fdc8d00():
        response_chat = gemini_pro.chat(messages=[msg])
        return response_achat
    response_achat = asyncio.run(run_async_code_1fdc8d00())
    logger.success(format_json(response_achat))
    return response_achat
response_achat = asyncio.run(run_async_code_7e95924c())
logger.success(format_json(response_achat))
...
async def run_async_code_9726b803():
    async def run_async_code_b7d32907():
        streaming_handler = gemini_pro.stream_chat(messages=[msg])
        return streaming_handler
    streaming_handler = asyncio.run(run_async_code_b7d32907())
    logger.success(format_json(streaming_handler))
    return streaming_handler
streaming_handler = asyncio.run(run_async_code_9726b803())
logger.success(format_json(streaming_handler))
async for chunk in streaming_handler:
    logger.debug(chunk.delta, end="")
    async def run_async_code_110796a4():
        await asyncio.sleep(0.5)
        return 
     = asyncio.run(run_async_code_110796a4())
