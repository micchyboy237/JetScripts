...
"""
logger.info("Next, we build a RAG over the same source documents used to created the `rag_dataset`.")
...
prediction_dataset = asyncio.run(async_func_0())
...
"""
logger.info("Now, we can use our evaluator to make evaluations by looping through all of the <example, prediction> pairs.")
...
async def run_async_code_41993941():
    async def run_async_code_3d0dae83():
        eval_results1 = await tqdm_asyncio.gather(*eval_tasks[:250])
        return eval_results1
    eval_results1 = asyncio.run(run_async_code_3d0dae83())
    logger.success(format_json(eval_results1))
    return eval_results1
eval_results1 = asyncio.run(run_async_code_41993941())
logger.success(format_json(eval_results1))

async def run_async_code_f06e4c75():
    async def run_async_code_bc15c08a():
        eval_results2 = await tqdm_asyncio.gather(*eval_tasks[250:])
        return eval_results2
    eval_results2 = asyncio.run(run_async_code_bc15c08a())
    logger.success(format_json(eval_results2))
    return eval_results2
eval_results2 = asyncio.run(run_async_code_f06e4c75())
logger.success(format_json(eval_results2))
...
"""
logger.info("The above utility also provides the mean score across all of the evaluations in `mean_df`.")
