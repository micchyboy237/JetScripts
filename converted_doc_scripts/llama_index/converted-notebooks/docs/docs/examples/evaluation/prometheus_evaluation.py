...
"""
logger.info("### Faithfulness Evaluation Prompt")
...
"""
logger.info("### Relevancy Evaluation Prompt")
...
"""
Set MLX Key for indexing
"""
logger.info("Set MLX Key for indexing")
...
"""
logger.info("## Define parser function")
...
"""
logger.info("## Define Correctness, FaithFulness, Relevancy Evaluators")
...
token_counter = TokenCountingHandler(
    tokenizer=tiktoken.encoding_for_model(model="qwen3-1.7b-4bit", log_dir=f"{OUTPUT_DIR}/chats").encode
)
...
"""
logger.info("## Let's create a function to create `query_engine` and `rag_dataset` for different datasets.")
...
"""
logger.info("## Function to run batch evaluations on defined evaluators")
...
async def batch_eval_runner(
    ...
    async def async_func_10():
        ...
        return eval_results
    eval_results = asyncio.run(async_func_10())
...
"""
logger.info("## Function to check the distribution of scores")
...
"""
logger.info("## Function to check correctness, faithfulness and relevancy evaluation score")
...
def get_eval_results(key, eval_results):
    ...
    logger.debug(f"{key} Score: {round(score, 2)}")
    return score

"""
## Function to compute `Hamming Distance`.
"""
logger.info("## Function to compute `Hamming Distance`.")
...
"""
logger.info("## Evaluation on PaulGraham Essay text")
...
"""
logger.info("### Compute Correctness, Faithfulness and Relevancy Evaluation")

async def async_func_0():
    prometheus_eval_results = await batch_eval_runner(
        prometheus_evaluators, query_engine, questions, reference
    )
    return prometheus_eval_results
prometheus_eval_results = asyncio.run(async_func_0())
logger.success(format_json(prometheus_eval_results))

async def async_func_4():
    gpt4_eval_results = await batch_eval_runner(
        gpt4_evaluators, query_engine, questions, reference
    )
    return gpt4_eval_results
gpt4_eval_results = asyncio.run(async_func_4())
logger.success(format_json(gpt4_eval_results))

"""
### Correctness Evaluation score distribution with Prometheus Evaluator.
"""
logger.info("### Correctness Evaluation score distribution with Prometheus Evaluator.")
...
"""
logger.info("### Correctness Evaluation score distribution with GPT-4 Evaluator.")
...
"""
logger.info("### Feedback comparison between prometheus and gpt-4.")
...
logger.debug(f"GPT-4 Feedback: {gpt4_feedback} \n\n {gpt4_score}")
...
"""
logger.info("### GPT-4 Faithfulness and Relevancy Evaluation scores.")
...
(Lower the better)
"""
logger.info("### Hamming Distance comparison between Prometheus and GPT-4")
...
logger.debug(f"Faithfulness Hamming Distance: {faithfulness_hamming_distance}")
logger.debug(f"Relevancy Hamming Distance: {relevancy_hamming_distance}")
...
"""
logger.info("### Feedback comparison between prometheus and gpt-4 for correctness.")
...
logger.debug(f"Context-1: {contexts[0]}")

logger.debug(f"Context-2: {contexts[1]}")

logger.debug(
    f"Prometheus Faithfulness Feedback: {prometheus_faithfulness_feedback}\n\n"
)
logger.debug(f"Prometheus Faithfulness Score: {prometheus_faithfulness_score}\n\n")
logger.debug(f"Prometheus Relevancy Feedback: {prometheus_relevancy_feedback}\n\n")
logger.debug(f"Prometheus Relevancy Score: {prometheus_relevancy_score}")
...
"""
logger.info("#### If you compare the feedback and contexts, there is mention of range of parameters in the context and response but the feedback says the model could not find such information.")

logger.debug(f"GPT-4 Faithfulness Feedback: {gpt4_faithfulness_feedback}\n\n")
logger.debug(f"GPT-4 Faithfulness Score: {gpt4_faithfulness_score}\n\n")
logger.debug(f"GPT-4 Relevancy Feedback: {gpt4_relevancy_feedback}\n\n")
logger.debug(f"GPT-4 Relevancy Score: {gpt4_relevancy_score}")
...
### GPT-4 Cost analysis
"""
logger.info("#### GPT-4 Evaluates it correctly, unlike prometheus model.")
...
"""
logger.info("## Total Cost Analysis")
