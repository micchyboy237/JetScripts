async def main():
    ...
    **WARNING**: This can be *expensive*, especially with GPT-4. Use caution and tune the sample size to fit your budget.
    ...
    evaluator_c = CorrectnessEvaluator(llm=OllamaFunctionCallingAdapter(model="llama3.2", request_timeout=300.0, context_window=4096))
    evaluator_s = SemanticSimilarityEvaluator()
    evaluator_r = RelevancyEvaluator(llm=OllamaFunctionCallingAdapter(model="llama3.2", request_timeout=300.0, context_window=4096))
    evaluator_f = FaithfulnessEvaluator(llm=OllamaFunctionCallingAdapter(model="llama3.2", request_timeout=300.0, context_window=4096))
