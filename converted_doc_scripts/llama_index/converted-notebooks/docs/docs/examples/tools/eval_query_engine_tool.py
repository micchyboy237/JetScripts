...
async def run_async_code_974c06e1():
    async def run_async_code_8ad2b313():
        response = await agent.run("What was Lyft's revenue growth in 2021?")
        return response
    response = asyncio.run(run_async_code_8ad2b313())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_974c06e1())
logger.success(format_json(response))
