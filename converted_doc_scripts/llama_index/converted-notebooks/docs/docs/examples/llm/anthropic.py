...
async def run_async_code_6239e614():
    async def run_async_code_bb55ad33():
        output = await agent.run("How do LlamaIndex and LLMs work together?")
        return output
    output = asyncio.run(run_async_code_bb55ad33())
    logger.success(format_json(output))
    return output
output = asyncio.run(run_async_code_6239e614())
logger.success(format_json(output))
