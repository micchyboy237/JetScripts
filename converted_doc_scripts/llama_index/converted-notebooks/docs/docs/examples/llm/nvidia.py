...
async def run_async_code_2264fd19():
    completion_llm.complete("Hello!")
    return 
 = asyncio.run(run_async_code_2264fd19())
...
async def run_async_code_93de58fd():
    chat_llm.chat(messages)
    return 
 = asyncio.run(run_async_code_93de58fd())
logger.success(format_json())
...
async def run_async_code_5aefc278():
    async def run_async_code_99650296():
        streamed_response = stream_llm.stream_chat(messages)
        return streamed_response
    streamed_response = asyncio.run(run_async_code_99650296())
    logger.success(format_json(streamed_response))
    return streamed_response
streamed_response = asyncio.run(run_async_code_5aefc278())
logger.success(format_json(streamed_response))
...
documents = SimpleDirectoryReader(f"{GENERATED_DIR}/hhgttg").load_data()
