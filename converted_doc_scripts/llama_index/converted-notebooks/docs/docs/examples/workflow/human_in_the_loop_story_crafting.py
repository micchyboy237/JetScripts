...
class ChooseYourOwnAdventureWorkflow(Workflow):
    ...
    async def create_segment(
        ...
        async def run_async_code_85aea93c():
            async def run_async_code_a9052efd():
                blocks = await ctx.store.get("blocks", [])
                return blocks
            blocks = asyncio.run(run_async_code_a9052efd())
            logger.success(format_json(blocks))
            return blocks
        blocks = asyncio.run(run_async_code_85aea93c())
        logger.success(format_json(blocks))
        ...
        if len(blocks) < self.max_steps:
            ...
            async def run_async_code_0f72f219():
                await ctx.store.set("blocks", blocks)
                return 
             = asyncio.run(run_async_code_0f72f219())
    ...
    async def prompt_human(
        ...
        async def run_async_code_5ae7d4d5():
            async def run_async_code_5535a31a():
                blocks = await ctx.store.get("blocks")
                return blocks
            blocks = asyncio.run(run_async_code_5535a31a())
            logger.success(format_json(blocks))
            return blocks
        blocks = asyncio.run(run_async_code_5ae7d4d5())
        logger.success(format_json(blocks))
        block.choice = human_input
        blocks[-1] = block
        async def run_async_code_f95d010f():
            await ctx.store.set("block", blocks)
            return 
         = asyncio.run(run_async_code_f95d010f())
        logger.success(format_json())
...
async def run_async_code_f776dcf8():
    async def run_async_code_701126ae():
        result = await w.run()
        return result
    result = asyncio.run(run_async_code_701126ae())
    logger.success(format_json(result))
    return result
result = asyncio.run(run_async_code_f776dcf8())
logger.success(format_json(result))
