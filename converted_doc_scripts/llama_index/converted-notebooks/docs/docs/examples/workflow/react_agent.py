...
class ReActAgent(Workflow):
    ...
    async def new_user_msg(self, ctx: Context, ev: StartEvent) -> PrepEvent:
        ...
        async def run_async_code_96e2d8b7():
            await ctx.store.set("current_reasoning", [])
            return 
         = asyncio.run(run_async_code_96e2d8b7())
        logger.success(format_json())
    ...
    async def prepare_chat_history(
        ...
        current_reasoning = asyncio.run(async_func_61())
    ...
    async def handle_llm_input(
        ...
        async def async_func_75():
            ...
            return current_reasoning
        current_reasoning = asyncio.run(async_func_75())
        logger.success(format_json(current_reasoning))
        async def run_async_code_7cb3b9e4():
            async def run_async_code_94b374ec():
                memory = await ctx.store.get("memory")
        ...
        async def run_async_code_aeac64cf():
            async def run_async_code_9fac412d():
                response_gen = self.llm.stream_chat(chat_history)
                return response_gen
            response_gen = asyncio.run(run_async_code_9fac412d())
            logger.success(format_json(response_gen))
            return response_gen
        response_gen = asyncio.run(run_async_code_aeac64cf())
        logger.success(format_json(response_gen))
        ...
        try:
            ...
            if reasoning_step.is_done:
                ...
                async def run_async_code_53efafd1():
                    await ctx.store.set("memory", memory)
                    return 
                 = asyncio.run(run_async_code_53efafd1())
                logger.success(format_json())
                async def run_async_code_80b1e9d6():
                    await ctx.store.set("current_reasoning", current_reasoning)
                    return 
                 = asyncio.run(run_async_code_80b1e9d6())
                logger.success(format_json())

                async def run_async_code_54ba06e9():
                    async def run_async_code_a64a15da():
                        sources = await ctx.store.get("sources", default=[])
                        return sources
                    sources = asyncio.run(run_async_code_a64a15da())
            ...
            async def run_async_code_25bf7049():
                await ctx.store.set("current_reasoning", current_reasoning)
                return 
             = asyncio.run(run_async_code_25bf7049())
            logger.success(format_json())
    ...
    async def handle_tool_calls(
        ...
        async def async_func_134():
            current_reasoning = await ctx.store.get(
                "current_reasoning", default=[]
            )
            return current_reasoning
        current_reasoning = asyncio.run(async_func_134())
        logger.success(format_json(current_reasoning))
        ...
        logger.success(format_json())
        async def run_async_code_0e27c9a4():
            await ctx.store.set("current_reasoning", current_reasoning)
            return 
         = asyncio.run(run_async_code_0e27c9a4())
        logger.success(format_json())
...
async def run_async_code_29aa001d():
    async def run_async_code_a8dac490():
        ret = await agent.run(input="Hello! My name is Logan", ctx=ctx)
        return ret
    ret = asyncio.run(run_async_code_a8dac490())
    logger.success(format_json(ret))
    return ret
ret = asyncio.run(run_async_code_29aa001d())
logger.success(format_json(ret))
logger.debug(ret["response"])
