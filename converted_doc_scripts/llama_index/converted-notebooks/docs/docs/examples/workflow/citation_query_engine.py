...
class CitationQueryEngineWorkflow(Workflow):
    ...
    async def synthesize(
        ...
        async def run_async_code_cc050d3b():
            async def run_async_code_dbf25ebf():
                response = await synthesizer.asynthesize(query, nodes=ev.nodes)
                return response
            response = asyncio.run(run_async_code_dbf25ebf())
            logger.success(format_json(response))
            return response
        response = asyncio.run(run_async_code_cc050d3b())
        logger.success(format_json(response))
...
async def run_async_code_5631a6de():
    async def run_async_code_c7e00399():
        result = await w.run(query="What information do you have", index=index)
        return result
    result = asyncio.run(run_async_code_c7e00399())
    logger.success(format_json(result))
    return result
result = asyncio.run(run_async_code_5631a6de())
logger.success(format_json(result))
