...
class QueryPlanningWorkflow(Workflow):
    ...
    async def planning_step(
        ...
        if isinstance(ev, StartEvent):
            ...
            async def run_async_code_e31a34c4():
                await ctx.store.set("tools", {t.metadata.name: t for t in tools})
                return 
             = asyncio.run(run_async_code_e31a34c4())
            logger.success(format_json())
            async def run_async_code_481e043c():
                await ctx.store.set("original_query", query)
                return 
             = asyncio.run(run_async_code_481e043c())
            logger.success(format_json())
            ...
            async def run_async_code_7a3c0fca():
                await ctx.store.set("context", context_str)
                return 
             = asyncio.run(run_async_code_7a3c0fca())
            logger.success(format_json())
            ...
            async def run_async_code_774c695a():
                await ctx.store.set("num_items", num_items)
                return 
             = asyncio.run(run_async_code_774c695a())
            logger.success(format_json())
            for item in query_plan.items:
                ctx.send_event(item)
        else:
            async def run_async_code_9d3592b2():
                async def run_async_code_fc5bfb29():
                    query = await ctx.store.get("original_query")
                    return query
                query = asyncio.run(run_async_code_fc5bfb29())
                logger.success(format_json(query))
                return query
            query = asyncio.run(run_async_code_9d3592b2())
            logger.success(format_json(query))
            ...
            if "PLAN" in decision:
                async def run_async_code_09f5e126():
                    async def run_async_code_29d77704():
                        context_str = await ctx.store.get("context")
                        return context_str
                    context_str = asyncio.run(run_async_code_29d77704())
                    logger.success(format_json(context_str))
                    return context_str
                context_str = asyncio.run(run_async_code_09f5e126())
                logger.success(format_json(context_str))
                ...
                num_items = len(query_plan.items)
                async def run_async_code_1a1f2a5f():
                    await ctx.store.set("num_items", num_items)
                    return 
                 = asyncio.run(run_async_code_1a1f2a5f())
                logger.success(format_json())
                for item in query_plan.items:
                    ctx.send_event(item)
            else:
    ...
    async def execute_item(
        ...
        async def run_async_code_e9601077():
            async def run_async_code_f3aff059():
                tools = await ctx.store.get("tools")
                return tools
            tools = asyncio.run(run_async_code_f3aff059())
            logger.success(format_json(tools))
            return tools
        tools = asyncio.run(run_async_code_e9601077())
        logger.success(format_json(tools))
        tool = tools[ev.name]
    ...
    async def aggregate_results(
        ...
        async def run_async_code_181268d3():
            async def run_async_code_5104678a():
                num_items = await ctx.store.get("num_items")
                return num_items
            num_items = asyncio.run(run_async_code_5104678a())
            logger.success(format_json(num_items))
            return num_items
        num_items = asyncio.run(run_async_code_181268d3())
        logger.success(format_json(num_items))
