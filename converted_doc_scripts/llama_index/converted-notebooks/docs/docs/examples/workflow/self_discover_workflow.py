...
class SelfDiscoverWorkflow(Workflow):
    ...
    async def refine_modules(
        ...
        task = ev.task
        modules = ev.modules
        async def run_async_code_6a501457():
            async def run_async_code_562de47c():
                llm: LLM = await ctx.store.get("llm")
                return llm: LLM
            llm: LLM = asyncio.run(run_async_code_562de47c())
            logger.success(format_json(llm: LLM))
            return llm: LLM
        llm: LLM = asyncio.run(run_async_code_6a501457())
        logger.success(format_json(llm: LLM))
...
async def run_async_code_c1bb982c():
    async def run_async_code_4ac04cbf():
        result = await workflow.run(task=task, llm=llm)
        return result
    result = asyncio.run(run_async_code_4ac04cbf())
    logger.success(format_json(result))
    return result
result = asyncio.run(run_async_code_c1bb982c())
logger.success(format_json(result))
