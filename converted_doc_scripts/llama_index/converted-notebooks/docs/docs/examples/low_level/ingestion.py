...
You will need an [MLX](https://openai.com/) api key for this tutorial. Login to your [platform.openai.com](https://platform.openai.com/) account, click on your profile picture in the upper right corner, and choose 'API Keys' from the menu. Create an API key for this tutorial and save it. You will need it below.
...
"""
Set your MLX api key, and Pinecone api key and environment in the file we created.
"""
logger.info("Set your MLX api key, and Pinecone api key and environment in the file we created.")
...
async def run_async_code_4ab64942():
    async def run_async_code_b22c3299():
        nodes = await pipeline.arun(nodes=nodes, in_place=False)
        return nodes
    nodes = asyncio.run(run_async_code_b22c3299())
    logger.success(format_json(nodes))
    return nodes
nodes = asyncio.run(run_async_code_4ab64942())
logger.success(format_json(nodes))
...
Generate document embeddings for each Node using our MLX embedding model (`text-embedding-ada-002`).
