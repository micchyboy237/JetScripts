...
async def run_async_code_bb684644():
    async def run_async_code_8436b15d():
        response = await agent.run("Hi, My name is Mayank.", memory=memory_from_client)
        return response
    response = asyncio.run(run_async_code_8436b15d())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_bb684644())
logger.success(format_json(response))
