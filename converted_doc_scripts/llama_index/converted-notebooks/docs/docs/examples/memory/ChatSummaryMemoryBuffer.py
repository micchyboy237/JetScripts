from jet.llm.mlx.base import MLX as OpenAiLlm
...
"""
logger.info("First, we simulate some chat history that will not fit in the memory buffer in its entirety.")
...
"""
logger.info("By supplying an *llm* and *token_limit* for summarization, we create a *ChatSummaryMemoryBuffer* instance.")
...
"""
logger.info("When printing the history, we can observe that older messages have been summarized.")
...
"""
logger.info("Let's add some new chat history.")
...
"""
logger.info("The history will now be updated with a new summary, containing the latest information.")
...
"""
logger.info("Using a longer *token_limit* allows the user to control the balance between retaining the full chat history and summarization.")
