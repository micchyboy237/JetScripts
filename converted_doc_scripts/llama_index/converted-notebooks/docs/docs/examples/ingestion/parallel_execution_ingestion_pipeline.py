...
async def run_async_code_4634abf7():
    async def run_async_code_f2f3abe9():
        nodes = await pipeline.arun(documents=documents, num_workers=4)
        return nodes
    nodes = asyncio.run(run_async_code_f2f3abe9())
    logger.success(format_json(nodes))
    return nodes
nodes = asyncio.run(run_async_code_4634abf7())
logger.success(format_json(nodes))
...
async def run_async_code_0f93fe1f():
    async def run_async_code_4640808f():
        nodes = await pipeline.arun(documents=documents)
        return nodes
    nodes = asyncio.run(run_async_code_4640808f())
    logger.success(format_json(nodes))
    return nodes
nodes = asyncio.run(run_async_code_0f93fe1f())
logger.success(format_json(nodes))
