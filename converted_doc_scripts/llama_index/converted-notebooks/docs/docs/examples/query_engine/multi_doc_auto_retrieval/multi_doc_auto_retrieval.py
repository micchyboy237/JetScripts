...
async def aprocess_doc(doc, include_summary: bool = True):
    ...
    async def run_async_code_7ae48189():
        async def run_async_code_9eb79949():
            ...
            return summary_txt
        summary_txt = asyncio.run(run_async_code_9eb79949())
        logger.success(format_json(summary_txt))
        return summary_txt
    summary_txt = asyncio.run(run_async_code_7ae48189())
    logger.success(format_json(summary_txt))
...
async def aprocess_docs(docs):
    ...
    async def run_async_code_ce2622ba():
        async def run_async_code_358a6ee6():
            index_nodes = await run_jobs(tasks, show_progress=True, workers=3)
            return index_nodes
        index_nodes = asyncio.run(run_async_code_358a6ee6())
        logger.success(format_json(index_nodes))
        return index_nodes
    index_nodes = asyncio.run(run_async_code_ce2622ba())
    logger.success(format_json(index_nodes))

    return index_nodes

async def run_async_code_f0cded99():
    async def run_async_code_3cee7e29():
        index_nodes = await aprocess_docs(docs)
        return index_nodes
    index_nodes = asyncio.run(run_async_code_3cee7e29())
    logger.success(format_json(index_nodes))
    return index_nodes
index_nodes = asyncio.run(run_async_code_f0cded99())
logger.success(format_json(index_nodes))
