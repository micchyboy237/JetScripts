...
async def get_agent_with_context_awareness(
    ...
    async def run_async_code_a8908edf():
        async def run_async_code_3bd1cf5e():
            context_nodes = await context_retriever.aretrieve(query)
            return context_nodes
        context_nodes = asyncio.run(run_async_code_3bd1cf5e())
        logger.success(format_json(context_nodes))
        return context_nodes
    context_nodes = asyncio.run(run_async_code_a8908edf())
    logger.success(format_json(context_nodes))
...
async def run_async_code_c3285c55():
    async def run_async_code_512dcb7d():
        ...
        return response
    response = asyncio.run(run_async_code_512dcb7d())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_c3285c55())
logger.success(format_json(response))
...
async def run_async_code_206be75f():
    async def run_async_code_2ead98bd():
        ...
        return response
    response = asyncio.run(run_async_code_2ead98bd())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_206be75f())
logger.success(format_json(response))
