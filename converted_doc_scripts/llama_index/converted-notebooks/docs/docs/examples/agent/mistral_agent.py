...
1. the MLX API (using our own `llama_index` LLM class)
...
async def run_async_code_d2bf24ef():
    async def run_async_code_6a2214e9():
        response = await agent.run("What is (121 + 2) * 5?")
        return response
    response = asyncio.run(run_async_code_6a2214e9())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_d2bf24ef())
logger.success(format_json(response))
