...
Inspired by GPTs interface, presented at MLX Dev Day 2023. Construct an agent with natural language.
...
Given the task, please generate a system prompt for an MLX-powered bot to solve this task:
{task} \
"""
...
async def create_system_prompt(task: str):
    ...
    async def run_async_code_c369b841():
        async def run_async_code_34e15407():
            ...
            return response
        response = asyncio.run(run_async_code_34e15407())
        logger.success(format_json(response))
        return response
    response = asyncio.run(run_async_code_c369b841())
    logger.success(format_json(response))
...
async def get_tools(task: str):
    ...
    async def run_async_code_decb767e():
        async def run_async_code_effed5b9():
            subset_tools = await tool_retriever.aretrieve(task)
            return subset_tools
        subset_tools = asyncio.run(run_async_code_effed5b9())
        logger.success(format_json(subset_tools))
        return subset_tools
    subset_tools = asyncio.run(run_async_code_decb767e())
    logger.success(format_json(subset_tools))
...
async def run_async_code_7169b826():
    response = await city_agent.run("Tell me about the parks in Toronto")
    return response
response = asyncio.run(run_async_code_7169b826())
logger.success(format_json(response))
