...
During query-time, we would first use the `ObjectRetriever` to retrieve a set of relevant Tools. These tools would then be passed into the agent; more specifically, their function signatures would be passed into the MLX Function calling API.
"""
...
async def async_func_11():
    ...
    return resp
resp = asyncio.run(async_func_11())
...
async def async_func_17():
    ...
    return resp
resp = asyncio.run(async_func_17())
logger.success(format_json(resp))
logger.debug(str(resp))
logger.debug(resp.tool_calls)
