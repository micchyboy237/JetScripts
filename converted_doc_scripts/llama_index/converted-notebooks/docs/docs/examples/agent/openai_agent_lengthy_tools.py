...
# MLX Agent Workarounds for Lengthy Tool Descriptions
In this demo, we illustrate a workaround for defining an MLX tool
whose description exceeds MLX's current limit of 1024 characters.
For simplicity, we will build upon the `QueryPlanTool` notebook
example.

If you're opening this Notebook on Colab, you will probably need to install LlamaIndex 🦙.
"""
logger.info("# MLX Agent Workarounds for Lengthy Tool Descriptions")
...
a single MLX tool is ultimately created from the `QueryPlanTool`
when the MLX API call is made. The description of this tool begins with
general instructions about the query plan approach, followed by the
descriptions of each constituent `QueryEngineTool`.

Currently, each MLX tool description has a maximum length of 1024 characters.
...
attempts to construct the MLX tool.
...
async def run_async_code_c1f5977d():
    async def run_async_code_70067f1a():
        response = await agent.run(query_planned_query)
        return response
    response = asyncio.run(run_async_code_70067f1a())
    logger.success(format_json(response))
    return response
response = asyncio.run(run_async_code_c1f5977d())
logger.success(format_json(response))
response
