...
- An MLX API key (or switch to another provider and adjust the code below)
...
async def async_func_44():
    ...
    return response
response = asyncio.run(async_func_44())
logger.success(format_json(response))
logger.debug(str(response))
...
class DocsAssistantWorkflow(Workflow):
    ...
    async def evaluate_query(
        ...
        async def run_async_code_ccb3341a():
            await ctx.store.set("results", [])
            return 
         = asyncio.run(run_async_code_ccb3341a())
        logger.success(format_json())
        sllm = self.llm.as_structured_llm(Actions)
        ...
        logger.success(format_json(response))
        actions = response.raw.actions
        async def run_async_code_ed35f31a():
            await ctx.store.set("num_events", len(actions))
            return 
         = asyncio.run(run_async_code_ed35f31a())
        logger.success(format_json())
        async def run_async_code_ccb3341a():
            await ctx.store.set("results", [])
            return 
    ...
    async def write_li_docs(
        ...
        async def run_async_code_45518997():
            async def run_async_code_46887e5f():
                results = await ctx.store.get("results")
                return results
            results = asyncio.run(run_async_code_46887e5f())
            logger.success(format_json(results))
            return results
        results = asyncio.run(run_async_code_45518997())
        logger.success(format_json(results))
    ...
    async def collect(
        ...
        async def run_async_code_26c45e88():
            async def run_async_code_5d2a7a91():
                num_events = await ctx.store.get("num_events")
                return num_events
            num_events = asyncio.run(run_async_code_5d2a7a91())
            logger.success(format_json(num_events))
            return num_events
        num_events = asyncio.run(run_async_code_26c45e88())
        logger.success(format_json(num_events))
...
async def run_docs_agent(query: str):
    ...
    async def run_async_code_92ff2ffa():
        async def run_async_code_52a6b2e1():
            result = await handler
            return result
        result = asyncio.run(run_async_code_52a6b2e1())
        logger.success(format_json(result))
        return result
    result = asyncio.run(run_async_code_92ff2ffa())
    logger.success(format_json(result))
    async def run_async_code_b3a7a9f1():
        for response in await handler.ctx.store.get("results"):
        return 
     = asyncio.run(run_async_code_b3a7a9f1())
    logger.success(format_json())
...
async def run_async_code_68c70f33():
    await run_docs_agent("How do I use loops in llamaindex workflows?")
    return 
 = asyncio.run(run_async_code_68c70f33())
logger.success(format_json())
