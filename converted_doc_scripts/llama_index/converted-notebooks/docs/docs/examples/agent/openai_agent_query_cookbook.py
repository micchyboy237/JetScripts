...
# MLX Agent + Query Engine Experimental Cookbook
...
Since the MLX Function API can infer function parameters, we explore its capabilities in performing auto-retrieval here.

If you're opening this Notebook on colab, you will probably need to install LlamaIndex 🦙.
"""
logger.info("# MLX Agent + Query Engine Experimental Cookbook")
...
Here we define the function interface, which is passed to MLX to perform auto-retrieval.

We were not able to get MLX to work with nested pydantic objects or tuples as arguments,
...
async def auto_retrieve_fn(
    ...
    async def run_async_code_be3ce108():
        async def run_async_code_645c2e77():
            response = query_engine.query(query)
            return response
        response = asyncio.run(run_async_code_645c2e77())
        logger.success(format_json(response))
        return response
    response = asyncio.run(run_async_code_be3ce108())
    logger.success(format_json(response))
    return str(response)
...
async def async_func_49():
    ...
    return response
response = asyncio.run(async_func_49())
logger.success(format_json(response))
...
async def async_func_46():
    ...
    return response
response = asyncio.run(async_func_46())
logger.success(format_json(response))
