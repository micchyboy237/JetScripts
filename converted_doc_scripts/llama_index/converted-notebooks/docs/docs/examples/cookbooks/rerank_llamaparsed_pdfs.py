...
async def run_async_code_3dc00f35():
    await zclient.collections.add(collection_name=collection_name)
    return 
 = asyncio.run(run_async_code_3dc00f35())
...
async def async_func_11():
    ...
    return text_data
text_data = asyncio.run(async_func_11())
...
async def upload_documents_async(
    ...
    async def run_async_code_f6172cc1():
        async def run_async_code_591bcdb4():
            results = await tqdm.gather(*tasks, desc="Uploading Documents")
            return results
        results = asyncio.run(run_async_code_591bcdb4())
        logger.success(format_json(results))
        return results
    results = asyncio.run(run_async_code_f6172cc1())
    logger.success(format_json(results))
...
async def run_async_code_e3af92cb():
    await upload_documents_async(docs, file_names, collection_name)
    return 
 = asyncio.run(run_async_code_e3af92cb())
logger.success(format_json())
...
async def rerank_top_pages_with_metadata(
    ...
    for result in top_pages_response.results:
        async def async_func_19():
            ...
            return page_info
        page_info = asyncio.run(async_func_19())
    ...
    rerank_response = asyncio.run(async_func_49())
...
async def async_func_0():
    ...
    return reranked_results
reranked_results = asyncio.run(async_func_0())
