...
async def run_async_code_d634c033():
    async def run_async_code_f583ad7e():
        eval_dataset = await dataset_generator.agenerate_dataset_from_nodes(num=100)
        return eval_dataset
    eval_dataset = asyncio.run(run_async_code_f583ad7e())
    logger.success(format_json(eval_dataset))
    return eval_dataset
eval_dataset = asyncio.run(run_async_code_d634c033())
logger.success(format_json(eval_dataset))

len(eval_dataset.qr_pairs)

eval_dataset.save_json(f"{GENERATED_DIR}/tesla10k_eval_dataset.json")

eval_dataset = QueryResponseDataset.from_json(
    f"{GENERATED_DIR}/tesla10k_eval_dataset.json"
)
...
async def run_evals(
    ...
    async def async_func_31():
        ...
        return eval_results
    eval_results = asyncio.run(async_func_31())
...
for k, pipeline in pipeline_dict.items():
    async def async_func_17():
        ...
        return eval_results
    eval_results = asyncio.run(async_func_17())
    logger.success(format_json(eval_results))
...
We keep the parsers fixed (HTML parser, sentence splitter w/ overlap 200) and the embedding model fixed (MLXEmbedding).
"""
...
for k, pipeline in pipeline_dict.items():
    async def async_func_38():
        ...
        return eval_results
    eval_results = asyncio.run(async_func_38())
    logger.success(format_json(eval_results))
