...
for txt in sample_texts:
    ...
    coros.append(coro)
async def run_async_code_9958323c():
    async def run_async_code_4917efcd():
        classifications = await asyncio.gather(*coros)
        return classifications
    classifications = asyncio.run(run_async_code_4917efcd())
    logger.success(format_json(classifications))
    return classifications
classifications = asyncio.run(run_async_code_9958323c())
logger.success(format_json(classifications))
