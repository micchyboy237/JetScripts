...
async def aprocess_docs(docs):
    ...
    async def run_async_code_95712ad3():
        async def run_async_code_0dafbf4f():
            new_docs = await run_jobs(tasks, show_progress=True, workers=5)
            return new_docs
        new_docs = asyncio.run(run_async_code_0dafbf4f())
        logger.success(format_json(new_docs))
        return new_docs
    new_docs = asyncio.run(run_async_code_95712ad3())
    logger.success(format_json(new_docs))


    return new_docs

async def run_async_code_90ae9b2f():
    async def run_async_code_c5422fc6():
        new_docs = await aprocess_docs(docs)
        return new_docs
    new_docs = asyncio.run(run_async_code_c5422fc6())
    logger.success(format_json(new_docs))
    return new_docs
new_docs = asyncio.run(run_async_code_90ae9b2f())
logger.success(format_json(new_docs))
