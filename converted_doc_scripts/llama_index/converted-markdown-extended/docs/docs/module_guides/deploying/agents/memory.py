...
"""
logger.info("### Managing the Memory Manually")
...
"""
logger.info("### Retrieving the Latest Memory from an Agent")
...
"""
logger.info("## Customizing Memory")
...
"""
logger.info("Here, we've setup three memory blocks:")
...
"""
logger.info("As the memory is used, the short-term memory will fill up. Once the short-term memory exceeds the `chat_history_token_ratio`, the oldest messages that fit into the `token_flush_size` will be flushed and sent to each memory block for processing.")

chat_history = memory.get(messages=[...])

logger.debug(chat_history[0].content)
...
"""
logger.info("## Customizing Memory Blocks")
...
"""
logger.info("## Memory vs. Workflow Context")
...
"""
logger.info("In other cases, like when using `FunctionAgent`, `AgentWorkflow`, or `ReActAgent`, if you customize the memory, then you will want to provide that as a separate runtime argument (especially since beyond the default, the `Memory` object is not serializable).")
...
"""
logger.info("Lastly, there are cases ([like human-in-the-loop](/python/framework/understanding/agent/human_in_the_loop)) where you will need to provide both the `Context` (to resume the workflow) and the `Memory` (to store the chat history).")
...
"""
logger.info("## (Deprecated) Memory Types")
