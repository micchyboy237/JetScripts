...
class VectorStoreRetriever:
    ...
    def from_docs(cls, docs, oai_client):
        embeddings = oai_client.embeddings.create(
            model="mxbai-embed-large", input=[doc["page_content"] for doc in docs]
        )
    ...
    def query(self, query: str, k: int = 5) -> list[dict]:
        embed = self._client.embeddings.create(
            model="mxbai-embed-large", input=[query]
        )
...
"""
logger.info("After that, define the **excursion assistant**.")
...
"""
logger.info("Finally, create the **primary assistant**.")
