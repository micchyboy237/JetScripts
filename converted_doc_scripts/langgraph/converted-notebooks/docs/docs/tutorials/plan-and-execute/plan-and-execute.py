...
{plan_str}\n\nYou are tasked with executing step {1}, {task}."""
    ...
    agent_response = asyncio.run(async_func_10())
...
async def plan_step(state: PlanExecute):
    async def run_async_code_cf3667cf():
        async def run_async_code_065a197a():
            plan = await planner.ainvoke({"messages": [("user", state["input"])]})
            return plan
        plan = asyncio.run(run_async_code_065a197a())
        logger.success(format_json(plan))
        return plan
    plan = asyncio.run(run_async_code_cf3667cf())
    logger.success(format_json(plan))
    return {"plan": plan.steps}


async def replan_step(state: PlanExecute):
    async def run_async_code_484d9120():
        async def run_async_code_2929e650():
            output = await replanner.ainvoke(state)
            return output
        output = asyncio.run(run_async_code_2929e650())
        logger.success(format_json(output))
        return output
    output = asyncio.run(run_async_code_484d9120())
    logger.success(format_json(output))
