...
async def generation_node(state: State) -> State:
    async def run_async_code_abf45683():
        return {"messages": [await generate.ainvoke(state["messages"])]}
        return 
     = asyncio.run(run_async_code_abf45683())
...
async def reflection_node(state: State) -> State:
    ...
    async def run_async_code_14e2157a():
        async def run_async_code_75762750():
            res = await reflect.ainvoke(translated)
            return res
        res = asyncio.run(run_async_code_75762750())
        logger.success(format_json(res))
        return res
    res = asyncio.run(run_async_code_14e2157a())
    logger.success(format_json(res))
