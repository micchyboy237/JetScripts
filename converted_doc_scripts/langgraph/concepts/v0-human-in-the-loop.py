...
"""
logger.info("### Dynamic Breakpoints")
...
"""
logger.info("Let's assume we run the graph with an input that triggers the dynamic breakpoint and then attempt to resume the graph execution simply by passing in `None` for the input.")
...
"""
logger.info("Alternatively, what if we want to keep our current input and skip the node (`my_node`) that performs the check? To do this, we can simply perform the graph update with `as_node="my_node"` and pass in `None` for the values. This will make no update the graph state, but run the update as `my_node`, effectively skipping the node and bypassing the dynamic breakpoint.")
...
"""
logger.info("Each checkpoint has a unique ID, which we can use to replay from a specific checkpoint.")
...
"""
logger.info("This creates a new forked checkpoint, `xxx-fork`, which we can then run the graph from.")
...
"""
logger.info("See [this additional conceptual guide](https://langchain-ai.github.io/langgraph/concepts/persistence/#update-state) for related context on forking.")
