...
class SimpleVectorStore:
    ...
    def add_item(self, text, embedding, metadata=None):
        ...
        self.vectors.append(np.array(embedding)
                            )  # Convert and store the embedding
        self.texts.append(text)                   # Store the original text
        # Store metadata (empty dict if None)
    ...
    def similarity_search(self, query_embedding, k=5, filter_func=None):
        ...
        # Convert query_embedding to 1D numpy array
        query_vector = np.squeeze(np.array(query_embedding))  # Shape: (1024,)
        ...
        for i, vector in enumerate(self.vectors):
            ...
            # Ensure vector is 1D
            vector = np.squeeze(np.array(vector))  # Shape: (1024,)

            # Compute cosine similarity
            dot_product = np.dot(query_vector, vector)
            norm_product = np.linalg.norm(
                query_vector) * np.linalg.norm(vector)
            similarity = dot_product / norm_product if norm_product != 0 else 0.0

            # Store index and similarity score
...
def create_embeddings(text, model="BAAI/bge-en-icl"):
    ...
    response = embed_func(input_text)
    ...
    for i, (chunk, embedding) in enumerate(zip(chunks, chunk_embeddings)):
        store.add_item(
            ...
            metadata={
                ...
                # Initial relevance score (will be updated with feedback)
                "relevance_score": 1.0,
...
def adjust_relevance_scores(query, results, feedback_data):
    ...
    for i, result in enumerate(results):
        ...
        for feedback in feedback_data:
            is_relevant = assess_feedback_relevance(
                query, document_text, feedback)
            if is_relevant:
...
def evaluate_feedback_loop(pdf_path, test_queries, reference_answers=None):
    ...
    for i, query in enumerate(test_queries):
        ...
        if reference_answers and i < len(reference_answers):
            similarity_to_ref = calculate_similarity(
                result["response"], reference_answers[i])
    ...
    comparison = compare_results(
        test_queries, round1_results, round2_results, reference_answers)
...
def calculate_similarity(text1, text2):
    ...
    # Convert embeddings to 1D arrays
    vec1 = np.squeeze(np.array(embedding1))  # Remove extra dimensions
    vec2 = np.squeeze(np.array(embedding2))  # Remove extra dimensions

    # Check if vectors are valid and non-zero
