...
First, we need to import the necessary libraries and set up the environment. We will be using HuggingFace Models hosted under **Nebius** platform. Obviously, you can use your own models as long as they are compatible with OllamaFunctionCallingAdapter's API.
"""
...
def generate_embeddings_batch(chunks_batch: List[str], model: str = "BAAI/bge-en-icl") -> List[List[float]]:
    """
    Generate embeddings for a batch of text chunks using the OllamaFunctionCallingAdapter client.
...
When we have a query and a set of relevant document chunks, we can use a large language model (LLM) to generate a response based on the query and the retrieved information. In this section, we will use the OllamaFunctionCallingAdapter API to generate a response to a query by providing the query text and the relevant document chunks as context to the LLM.
...
"""
To generate an LLM response, we need to implement a function that takes the constructed input prompt and sends it to the OllamaFunctionCallingAdapter API for response generation.
"""
logger.info("To generate an LLM response, we need to implement a function that takes the constructed input prompt and sends it to the OllamaFunctionCallingAdapter API for response generation.")
...
def generate_response(
    ...
    """
    Generate a response from the OllamaFunctionCallingAdapter chat model based on the constructed prompt.
