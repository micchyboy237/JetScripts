...
"""
logger.info("A minimal method for keyword extraction with Large Language Models (LLM). There are a number of implementations that allow you to mix and match `KeyBERT` with `KeyLLM`. You could also choose to use `KeyLLM` without `KeyBERT`.")
...
"""
Then we can use any Ollama model, such as ChatGPT, as follows:
"""
logger.info("Then we can use any Ollama model, such as ChatGPT, as follows:")
...
"""
logger.info("This creates the following keywords:")
...
"""
logger.info("## 2. **Extract** Keywords with `KeyLLM`")
...
"""
logger.info("## 3. **Fine-tune** Candidate Keywords")
...
"""
client = ollama.Ollama(api_key=MY_API_KEY)
llm = Ollama(client,model="llama3.2", prompt=prompt, chat=True)
...
"""
logger.info("## 4. **Efficient** `KeyLLM`")
...
!!! Tip
    ...
    `pip install --upgrade git+https://github.com/UKPLab/sentence-transformers`
"""
logger.info("## 5. **Efficient** `KeyLLM` + `KeyBERT`")
