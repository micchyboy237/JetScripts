...
<Note>
Navigate to `ec_app` folder and create `.env` file in this folder and set your MLX API key as shown in `.env.example` file. If you want to use other open-source models, feel free to use the app config in `app.py`. More details for using custom configuration for Embedchain app is [available here](https://docs.embedchain.ai/api-reference/advanced/configuration).
</Note>
...
"""
logger.info("To run the app in development, run the following command:")
...
Now, we need to launch this application with fly.io. You can see your app on [fly.io dashboard](https://fly.io/dashboard). Run the following command to launch your app on fly.io:
"""
logger.info("Before deploying the app, open `fly.toml` file and update the `name` variable to something unique. This is important as `fly.io` requires users to provide a globally unique deployment app names.")
...
"""
logger.info("Run `ec deploy` to deploy your app on Fly.io. Once you deploy your app, your slack bot will be live!")
