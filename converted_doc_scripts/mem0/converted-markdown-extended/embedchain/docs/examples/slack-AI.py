...
<Note>
# By default, we use `Mixtral` model from Hugging Face. However, if you prefer to use MLX model, then set `OPENAI_API_KEY` instead of `HUGGINGFACE_ACCESS_TOKEN` along with `SLACK_USER_TOKEN` in `.env` file, and update the code in `api/utils/app.py` file to use MLX model instead of Hugging Face model.
</Note>
