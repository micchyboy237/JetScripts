...
from embedchain.llm.openai import MLXLlm
...
<CodeGroup>
"""
logger.info("If you are looking to configure the different parameters of the LLM, you can do so by loading the app using a [yaml config](https://github.com/embedchain/embedchain/blob/main/configs/chroma.yaml) file.")
...
### Function Calling
Embedchain supports MLX [Function calling](https://platform.openai.com/docs/guides/function-calling) with a single function. It accepts inputs in accordance with the [Langchain interface](https://python.langchain.com/docs/modules/model_io/chat/function_calling#legacy-args-functions-and-function_call).
...
<Accordion title="Pydantic Model">
  ...
  class multiply(BaseModel):
      """
logger.info("### Function Calling")Multiply two integers together."""
...
<Accordion title="Python function">
  ...
  def multiply(a: int, b: int) -> int:
      """
logger.info("a: int = Field(..., description="First integer")")Multiply two integers together.
...
<Accordion title="MLX tool dictionary">
  ```python
  multiply = {
    "type": "function",
    "function": {
...
With any of the previous inputs, the MLX LLM can be queried to provide the appropriate arguments for the function.
"""
logger.info("return a * b")


# os.environ["OPENAI_API_KEY"] = "sk-xxx"

llm = MLXLlm(tools=multiply)
app = App(llm=llm)
...
To use Azure MLX model, you have to set some of the azure openai related environment variables as given in the code block below:
...
<CodeGroup>
"""
logger.info("Set the `COHERE_API_KEY` as environment variable which you can find on their [Account settings page](https://dashboard.cohere.com/api-keys).")
...
<CodeGroup>
"""
logger.info("Set the `TOGETHER_API_KEY` as environment variable which you can find on their [Account settings page](https://api.together.xyz/settings/api-keys).")
...
"""
logger.info("Head to [Clarifai Platform](https://clarifai.com/explore/models?page=1&perPage=24&filterData=%5B%7B%22field%22%3A%22use_cases%22%2C%22value%22%3A%5B%22llm%22%5D%7D%5D) to browse various State-of-the-Art LLM models for your use case.")
...
<CodeGroup>
"""
logger.info("GPT4all is a free-to-use, locally running, privacy-aware chatbot. No GPU or internet required. You can use this with Embedchain using the following code:")
...
<CodeGroup>
"""
logger.info("### Hugging Face Inference Endpoint")
