...
# We use MLX's `gpt-4` model as default LLM model for automatic evaluation. Hence, we require you to set `OPENAI_API_KEY` as an environment variable.
...
"""
logger.info("### Step-2: Run evaluation")
...
Context relevancy is a metric to determine "how relevant the context is to the question". We use MLX's `gpt-4` model to determine the relevancy of the context. We achieve this by prompting the model with the question and the context and asking it to return relevant sentences from the context. We then use the following formula to determine the score:
...
"""
logger.info("In the above example, we used sensible defaults for the evaluation. However, you can also configure the evaluation metric as per your needs using the `ContextRelevanceConfig` class.")


eval_config = ContextRelevanceConfig(model="llama-3.2-3b-instruct", log_dir=f"{OUTPUT_DIR}/chats", api_key="sk-xxx", language="en")
...
"""
logger.info("#### `ContextRelevanceConfig`")
...
"""
logger.info("In the above example, we used sensible defaults for the evaluation. However, you can also configure the evaluation metric as per your needs using the `AnswerRelevanceConfig` class. Here is a more advanced example where you can provide your own evaluation config:")
...
Groundedness is a metric to determine how grounded the answer is to the context. We use MLX's `gpt-4` model to determine the groundedness of the answer. We achieve this by prompting the model with the answer and asking it to generate claims from the answer. We then again prompt the model with the context and the generated claims to determine the verdict on the claims. We then use the following formula to determine the score:
...
"""
logger.info("#### `AnswerRelevanceConfig`")
...
"""
logger.info("In the above example, we used sensible defaults for the evaluation. However, you can also configure the evaluation metric as per your needs using the `GroundednessConfig` class. Here is a more advanced example where you can configure the evaluation config:")


eval_config = GroundednessConfig(model="llama-3.2-3b-instruct", log_dir=f"{OUTPUT_DIR}/chats", api_key="sk-xxx")
...
"""
logger.info("#### `GroundednessConfig`")
