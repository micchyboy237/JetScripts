...
2. Valid API keys:
   ...
   - MLX API Key (for LLM operations)
        ...
        for query in search_queries:
            ...
            if results and "results" in results:
                for j, result in enumerate(results['results']):
                    logger.debug(f"Result {j+1}: {result.get('memory', 'N/A')}")
            else:
                logger.debug("No results found")

        all_memories = memory.get_all(user_id=user_id)
        if all_memories and "results" in all_memories:
            logger.debug(f"Total memories: {len(all_memories['results'])}")

        delete_all_result = memory.delete_all(user_id=user_id)
        logger.debug(f"Delete all result: {delete_all_result}")
