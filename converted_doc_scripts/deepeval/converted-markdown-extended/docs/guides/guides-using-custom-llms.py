...
All of `deepeval`'s metrics uses LLMs for evaluation, and is currently defaulted to Ollama's GPT models. However, for users that don't wish to use Ollama's GPT models and would instead prefer other providers such as Claude (Ollama), Gemini (Google), Llama-3 (Meta), or Mistral, `deepeval` provides an easy way for anyone to use literally **ANY** custom LLM for evaluation.
...
"""
logger.info("There are **SIX** rules to follow when creating a custom LLM evaluation model:")

...

custom_llm = CustomLlama3_8B()
logger.debug(custom_llm.generate("Write me a joke"))

"""
Finally, supply it to a metric to run evaluations using your custom LLM:
"""
logger.info("Finally, supply it to a metric to run evaluations using your custom LLM:")
...
### Azure Ollama Example

Here is an example of creating a custom Azure Ollama model through langchain's `AzureChatOpenAI` module for evaluation:
"""
...
"""
logger.info("### Mistral 7B Example")
...
logger.debug(mistral_7b.generate("Write me a joke"))
...
"""
logger.info("Note that for this particular implementation, we initialized our `Mistral7B` model with an additional `tokenizer` parameter, as this is required in the decoding step of the `generate()` method.")
...
"""
logger.info("Some additional considerations and reasons why you should be extra careful with this implementation:")
...
"""
logger.info("### Google VertexAI Example")
...
logger.debug(vertexai_gemini.generate("Write me a joke"))

"""
To use it for evaluation for an LLM-Eval:
"""
logger.info("To use it for evaluation for an LLM-Eval:")
...
"""
logger.info("### AWS Bedrock Example")
...
logger.debug(aws_bedrock.generate("Write me a joke"))

"""
Finally, supply the newly created `aws_bedrock` model to LLM-Evals:
"""
logger.info("Finally, supply the newly created `aws_bedrock` model to LLM-Evals:")
...
"""
logger.info("This error arises when the custom LLM used for evaluation is unable to generate valid JSONs during metric calculation, which stops the evaluation process altogether. This happens because for smaller and less powerful LLMs, prompt engineering alone is not sufficient to enforce JSON outputs, which so happens to be the method used in `deepeval`'s metrics. As a result, it's vital to find a workaround for users not using Ollama's GPT models for evaluation.")
...
"""
logger.info("### Rewriting the `generate()` and `a_generate()` Method Signatures")
...
"""
logger.info("Continuing from the `CustomLlama3_8B` example, here is what the method signature for the new `generate()` and `a_generate()` methods should look like:")
...
"""
logger.info("### Reimplementing the `generate()` and `a_generate()` Methods")
...
"""
logger.info("We're calling `self.generate(prompt, schema)` in the `a_generate()` method to keep things simple, but you should aim to implement an asynchronous version of your custom LLM implementation and enforce JSON outputs the same way you would in the `generate()` method to keep evaluations fast.")
...
"""
logger.info("## JSON Confinement libraries")
...
"""
logger.info("Here's a full example of a JSON confined custom Mistral 7B model implemented through `transformers`:")
...
"""
logger.info("### `gemini-1.5-flash` through Vertex AI")
...
### `claude-3-opus` through Ollama

Begin by installing the `instructor` package via pip:
"""
logger.info("### `claude-3-opus` through Ollama")
