...
1. **Vectorizing the initial input into an embedding**, using an embedding model of your choice (eg. Ollama's `text-embedding-3-large` model).
...
- **Does the embedding model you're using capture domain-specific nuances?** (If you're working on a medical use case, a generic embedding model offered by Ollama might not provide expected the vector search results.)
...
"""
logger.info("# id: guides-rag-evaluation")
...
"""
logger.info("All metrics in `deepeval` allows you to set passing `threshold`s, turn on `strict_mode` and `include_reason`, and use literally **ANY** LLM for evaluation. You can learn about each metric in detail, including the algorithm used to calculate them, on their individual documentation pages:")
...
"""
logger.info("The `input` is the user input, `actual_output` is the final generation of your RAG pipeline, `expected_output` is what you expect the ideal `actual_output` to be, and the `retrieval_context` is the retrieved text chunks during the retrieval step. The `expected_output` is needed because it acts as the ground truth for what information the `retrieval_context` should contain.")

...

contextual_precision.measure(test_case)
logger.debug("Score: ", contextual_precision.score)
logger.debug("Reason: ", contextual_precision.reason)

contextual_recall.measure(test_case)
logger.debug("Score: ", contextual_recall.score)
logger.debug("Reason: ", contextual_recall.reason)

contextual_relevancy.measure(test_case)
logger.debug("Score: ", contextual_relevancy.score)
logger.debug("Reason: ", contextual_relevancy.reason)

"""
Or in bulk, which is useful if you have a lot of test cases:
"""
logger.info("Or in bulk, which is useful if you have a lot of test cases:")
...
"""
logger.info("Then, create a test case (we're reusing the same test case in the previous section):")
...
"""
logger.info("Lastly, run individual evaluations:")

...

answer_relevancy.measure(test_case)
logger.debug("Score: ", answer_relevancy.score)
logger.debug("Reason: ", answer_relevancy.reason)

faithfulness.measure(test_case)
logger.debug("Score: ", faithfulness.score)
logger.debug("Reason: ", faithfulness.reason)

"""
Or as part of a larger dataset:
"""
logger.info("Or as part of a larger dataset:")
...
"""
logger.info("### Beyond Generic Evaluation")
...
logger.debug("Score: ", dark_humor.score)
logger.debug("Reason: ", dark_humor.reason)
...
"""
logger.info("## E2E RAG Evaluation")
...
"""
logger.info("## Unit Testing RAG Systems in CI/CD")
...
"""
logger.info("Then, simply execute `deepeval test run` in the CLI:")
...
"""
logger.info("You can learn about everything `deepeval test run` has to offer [here (including parallelization, caching, error handling, etc.).](/docs/evaluation-flags-and-configs#flags-for-deepeval-test-run)")
...
"""
logger.info("## Optimizing On Hyperparameters")
...
"""
logger.info("Follow the instructions to create an account, copy and paste your API key in the CLI, and add these few lines of code in your test file to start logging hyperparameters with each test run:")
...
"""
logger.info("You can simply return an empty dictionary `{}` if you don't have any custom parameters to log.")
