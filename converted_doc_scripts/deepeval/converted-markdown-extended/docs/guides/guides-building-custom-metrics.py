...
"""
logger.info("## Rules To Follow When Creating A Custom Metric")
...
"""
logger.info("### 2. Implement the `__init__()` method")
...
"""
logger.info("### 3. Implement the `measure()` and `a_measure()` methods")
...
"""
logger.info("When you run `assert_test()` with `run_async=True` (which is the default behavior), `deepeval` calls the `a_measure()` method which allows all metrics to run concurrently in a non-blocking way.")
...
class CustomMetric(BaseMetric):
    ...
    async def a_measure(self, test_case: LLMTestCase) -> float:
        try:
            ...
            logger.success(format_json(self.score))
            if self.include_reason:
                self.reason = await async_generate_hypothetical_reason(test_case)
                logger.success(format_json(self.reason))
            self.success = self.score >= self.threshold
            return self.score
        except Exception as e:
            self.error = str(e)
...
Often times, the blocking part of an LLM evaluation metric stems from the API calls made to your LLM provider (such as Ollama's API endpoints), and so ultimately you'll have to ensure that LLM inference can indeed be made asynchronous.
...
"""
logger.info("Often times, the blocking part of an LLM evaluation metric stems from the API calls made to your LLM provider (such as Ollama's API endpoints), and so ultimately you'll have to ensure that LLM inference can indeed be made asynchronous.")
...
"""
logger.info("### 4. Implement the `is_successful()` method")
...
"""
logger.info("### 5. Name Your Custom Metric")
...
"""
logger.info("## Building a Custom Non-LLM Eval")
...
"""
logger.info("Although you're free to implement your own rouge scorer, you'll notice that while not documented, `deepeval` additionally offers a `scorer` module for more traditional NLP scoring method and can be found [here.](https://github.com/confident-ai/deepeval/blob/main/deepeval/scorer/scorer.py)")
...
logger.debug(metric.is_successful())
...
"""
logger.info("## Building a Custom Composite Metric")
...
"""
logger.info("Now go ahead and try to use it:")
