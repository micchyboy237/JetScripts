...
We'll use `OllamaEmbeddings` and the `FAISS` vector store from `langchain` to build our knowledge base, though other models and stores can be used.
"""
logger.info("## Create Agent and Load Data")
...
class RAGAgent:
    def __init__(
        ...
        self.embedding_model = embedding_model or OllamaEmbeddings(model="nomic-embed-text")
...
"""
logger.info("You can modify the above code to use an embedding model or vector store of your choice.")

document_paths = ["theranos_legacy.txt"]
agent = RAGAgent(document_paths)
logger.debug(agent.vector_store)
...
"""
logger.info("This allows us to retrieve `k` documents that are most relevant to the `query` we supplied by using similarity search. We can test our retriever with the following code:")
...
"""
logger.info("I have created a file called `theranos_legacy.txt` that has all the information about **Theranos** company. Feel free to use your own documents or the sample content provided below:")
...
We'll now add a `generate()` method to our `RAGAgent` class. This function will take the retrieved context and use an Ollama language model (via `langchain`) to generate the final answer.
"""
...
class RAGAgent:
    ...
    def generate(
        ...
        context = "\n".join(retrieved_docs)
        model = llm_model or Ollama(temperature=0)
...
"""
logger.info("This allows us to generate an answer to the query based on the retrieved docs. Here's how we can use our generator:")
...
"""
logger.info("Running the above code will get you an output similar to the following:")
...
"""
logger.info("You can now send a query and test your entire RAG QA Agent.")
...
"""
logger.info("Most LLMs output a response in markdown format by default, which makes it harder to extract structured data such as citations. This is not ideal because we cannot parse the")
...
"""
logger.info("## Updating The RAG Agent")
...
<TabItem id="raw" value="Raw">
"""
logger.info("Now our `RAGAgent` outputs a valid `json`, we can use this output to render UI and create webpages or handle our responses in")
...
"""
logger.info("We now have a RAG agent that generates the output in our desired format, but how reliable are the generated answers? It is very important to make sure")
