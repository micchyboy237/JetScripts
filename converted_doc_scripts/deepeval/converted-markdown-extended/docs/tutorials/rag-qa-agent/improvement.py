...
from langchain.llms import Ollama, Ollama, HuggingFaceHub
...
"""
logger.info("The dataset pulled contains goldens, which can be used to create test cases during run time and run evals. Here's an example of how to create test cases using the dataset pulled:")
...
logger.debug(len(retriever_test_cases))
logger.debug(len(generator_test_cases))
...
"""
logger.info("## Iterating on Hyperparameters")
...
models = [
    ...
    ("ollama", Ollama(model_name="gpt-4")),
...
"""
logger.info("## RAG Agent Improvement")
...
"""
logger.info("The new `RAGAgent` now answers reliably in the desired `json` format. This is the new UI and raw output generated by the improved agent:")
...
"""
logger.info("Now that we have a reliable RAG QA Agent, in the next section we'll see how to set up tracing to [prepare our RAG QA Agent for deployment](/tutorials/rag-qa-agent/tutorial-rag-qa-deployment).")
