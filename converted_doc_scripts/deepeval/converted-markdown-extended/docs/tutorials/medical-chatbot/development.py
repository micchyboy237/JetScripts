...
First create a `MedicalChatbot` class and use `langchain`'s chat models to call `Ollama`:
"""
logger.info("## Setup Your Model")
...
"""
:::note
You can also use other interfaces to call Ollama, or any other model.
:::

Try prompting it with a messages array:
"""
logger.info("You can also use other interfaces to call Ollama, or any other model.")

chatbot = MedicalChatbot(model="llama3.2")
...
Since Ollama models weren't specifically trained on medical knowledge, we'll need to leverage RAG to provide additional context at runtime to diagnose patients that are grounded in context.
...
"""
logger.info("## Create RAG Pipeline For Diagnosis")
...
"""
logger.info("Then, simply run your `index_knowledge` method usign the encyclopedia you've downloaded as `.txt`:")
...
"""
logger.info("### Query your knowledge base")
...
Now try calling it:
"""
logger.info("The `@tool` decorator tells `langchain` that the `retrieve_knowledge` method can be called as a function call and will come in handy in later sections.")
...
"""
logger.info("## Create Tool To Book Appointments")
...
"""
logger.info("Now let's implement the `create_appointment` tool:")
...
"""
logger.info("## Implementing Chat Histories")
...
"""
logger.info("Then we'll combine the agent setup and memory functionality into one clean implementation, including the `retrieve_knowledge` and `create_appointment` tools in our agent:")
...
"""
logger.info("## Eyeball Your First Output")

def start_session(session_id: Optional[str] = None):
    """Start an interactive session with the chatbot"""
    logger.debug("Hello! I am Baymax, your personal healthcare companion.")
    logger.debug("How are you feeling today? (type 'exit' to quit."))
    ...
    while True:
        ...
        logger.debug("Baymax:", response["output"])
...
"""
logger.info("The model and system prompt are the variables you'll want to be improving in the next section.")
...
"""
logger.info("Here's an example conversation:")
