...
"""
logger.info("The dataset pulled contains goldens, which can be used to create test cases during run time and run evals. This is how we can use our `ConversationalGolden`s and `ConversationSimulator` to generate `ConversationalTestCase`s:")
...
models = ["gpt-4", "llama3.2", "gpt-3.5-turbo"]
...
- system prompt
"""
logger.info("These were the updates made to our medical chatbot. You can now change the following configurations for your chatbot in the initialization itself")
...
"""
logger.info("This new updated model now performs as we intended and can be used to create a UI interface, this is what a UI integrated chatbot looks like:")
