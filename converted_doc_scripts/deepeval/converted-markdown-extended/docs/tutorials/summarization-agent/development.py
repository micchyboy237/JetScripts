...
In this section, we're going to create our **meeting summarization agent** using the Ollama API. Our summarization agent should be able to take an entire meeting transcript as `input` and returns
...
We are now going to create a `MeetingSummarizer` class that uses Ollama's chat completions API to generate summaries and action items using the system prompt mentioned above for any given transcript.
"""
logger.info("## Creating Meeting Summarizer")
...
class MeetingSummarizer:
    def __init__(
        ...
        self.model = model
#         self.client = Ollama(api_key=os.getenv("OPENAI_API_KEY"))
...
"""
logger.info("I have saved a file named `meeting_transcript.txt` that contains a mock transcript which is provided to the summarizer as shown above. You can provide your own transcript here or use the mock transcript that I've used:")
...
"""
logger.info("After running the summarizer, the summary generated was a _string of markdown_ (that's how most LLMs respond by default). And this is not desirable for us as we need to parse the response from the LLM and create a UI/UX interface that is appealing for users.")
...
"""
logger.info("## Updating Meeting Summarizer")
...
class MeetingSummarizer:
    ...
    def get_action_items(self, transcript: str) -> dict:
        try:
            ...
            logger.debug(f"Error generating action items: {e}")
...
"""
logger.info("We can now call these helper functions in our `summarize()` function and return their respective responses. Here's how we can do that:")
...
"""
logger.info("You can run the new `MeetingSummarizer` as follows:")
...
logger.debug(summary)
logger.debug("JSON:")
logger.debug(json.dumps(action_items, indent=2))
...
"""
logger.info("Here is an example of a nice looking UI that shows how we can manipulate our new responses.")
...
"""
logger.info("We now have a summarization agent that generates responses in our desired format. Now it's time to evaluate how good this agent works. Many developers stop at a quick glance of the output and assume it's good enough. But **LLMs are probabilistic and prone to inconsistency** — eyeballing results won't catch subtle regressions, logical errors, or hallucinated action items. That's why rigorous evaluation is essential.")
