...
:::
"""
logger.info("Next, create a Qdrant collection with the appropriate vector configurations. This collection will store your document embeddings as `vectors` and the corresponding text chunks as metadata. In the code snippet below, we set the `distance` function to cosine similarity and define a vector dimension of 384.")
...
"""
logger.info("To add documents to your Qdrant collection, first embed the chunks before upserting them using the `PointStruct` structure. In this example, we'll use `all-MiniLM-L6-v2` from `sentence_transformers` as our embedding model.")
...
"""
logger.info("## Evaluating Qdrant Retrieval")
...
"""
logger.info("and the corresponding expected output:")
...
"""
logger.info("### Preparing your Test Case")
...
"""
logger.info("We'll then insert these contexts into our prompt template to provide additional context and help ground the response.")
...
"""
logger.info("We'll then pass the input and expected output that was initially defined into an `LLMTestCase`, along with the actual output and retrieval context that we generated and searched for.")
...
"""
logger.info("Before proceeding with evaluations, let's examine the `actual_output` that was generated:")
...
:::
"""
logger.info("## Improving Qdrant Retrieval")
