...
"""
logger.info("To enable efficient similarity search, define a **Weaviate schema** that stores documents with a `text` property for raw content and an associated vector for embeddings. Since Weaviate supports both internal and external vectorization, this schema is configured to use an external embedding model.")
...
:::
"""
logger.info("Before adding documents to Weaviate, convert text into vector representations using an embedding model. We'll be using `all-MiniLM-L6-v2` from `sentence_transformers`.")
...
"""
logger.info("## Evaluating Weaviate Retrieval")
...
"""
logger.info("Next, incorporate the retrieved context into your LLM's prompt template to generate a response.")
...
"""
logger.info("With both the `actual_output` and `retrieval_context` generated, we now have all the necessary parameters to construct our test case:")
...
"""
logger.info("## Improving Weaviate Retrieval")
