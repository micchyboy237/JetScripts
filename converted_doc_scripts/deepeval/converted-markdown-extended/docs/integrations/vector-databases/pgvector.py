...
"""
logger.info("Next, you'll need to create a table to store `text` chunks along with their corresponding embedding `vectors`. To enable vector operations, you'll need to activate the `pgvector` extension.")
...
"""
logger.info("Finally, you'll need to convert your document chunks into vectors using an embedding model and store them in PostgreSQL. We'll use `all-MiniLM-L6-v2` from `sentence-transformers` to generate embeddings and insert them into the `documents` table.")
...
"""
logger.info("## Evaluating PGVector Retrieval")
...
"""
logger.info("Next, we'll insert the `retrieval_context` retrieved from the vector database into our prompt template to generate an LLM response, referred to as `actual_output`. This step finalizes the required parameters needed to construct an `LLMTestCase`.")
...
:::
"""
logger.info("## Improving PGVector Retrieval")
