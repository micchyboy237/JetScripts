...
"""
logger.info("DeepEval autoloads environment files (at import time)")
...
"""
logger.info("## Create Your First Test Run")
...
"""
logger.info("Then, run `deepeval test run` from the root directory of your project to evaluate your LLM app **end-to-end**:")
...
Run `touch test_example.py` in your terminal and paste in the following code:
"""
logger.info("Congratulations! Your test case should have passed ✅ Let's breakdown what happened.")
...
"""
logger.info("If you run more than one test run, you will be able to **catch regressions** by comparing test cases side-by-side. This is also made easier if you're using `deepeval` alongside Confident AI ([see below](/docs/getting-started#save-results-on-cloud) for video demo).")
...
- **Rate limits (429):** retried unless the provider marks them non-retryable
  (for Ollama, `insufficient_quota` is treated as non-retryable).
...
"""
logger.info("The `deepeval view` command requires that the test run that you ran above has been successfully cached locally. If something errors, simply run a new test run after logging in with `deepeval login`:")
...
"""
logger.info("After you've pasted in your API key, Confident AI will **generate testing reports and automate regression testing** whenever you run a test run to evaluate your LLM application inside any environment, at any scale, anywhere.")
...
"""
logger.info("## Test Runs With LLM Tracing")
...
"""
logger.info("Then run `python main.py` to run a **component-level** eval:")
...
"""
logger.info("Once the for loop has ended, `deepeval` will aggregate all metrics, test cases in each component, and run evals across them all, before generating the final testing report.")
...
"""
logger.info("## Continue With Your Use Case")
