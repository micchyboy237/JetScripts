...
"""
logger.info("Since `deepeval` is an LLM evaluation framework, the ** `input` and `actual_output` are always mandatory.** However, this does not mean they are necessarily used for evaluation, and you can also add additional parameters such as the `tools_called` for each `LLMTestCase`.")
...
"""
logger.info("## What Is An LLM "Interaction"?")
...
<div style={{textAlign: 'center', margin: "2rem 0"}}>
"""
logger.info("There are different ways you scope an interaction:")
...
<div style={{textAlign: 'center', margin: "2rem 0"}}>
"""
logger.info("If you would want to evaluate using the `ToolCorrectnessMetric` however, you'll need to create an `LLMTestCase` at the **Agent-Level**, and supply the `tools_called` parameter instead:")
...
"""
logger.info("Not all `input`s should include your prompt template, as this is determined by the metric you're using. Furthermore, the `input` should **NEVER** be a json version of the list of messages you are passing into your LLM.")
...
"""
logger.info("A `ToolCall` object accepts 1 mandatory and 4 optional parameters:")
...
"""
logger.info("The `input` parameter accepts a list of strings and `MLLMImage`s, which is a class specific `deepeval`. The `MLLMImage` class accepts an image path and automatically sets the `local` attribute to `true` or `false` depending on whether the image is locally stored or hosted online. By default, `local` is set to `false`.")
