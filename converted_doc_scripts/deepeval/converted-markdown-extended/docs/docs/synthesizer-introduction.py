...
pip install langchain langchain-community langchain-text-splitters chromadb tiktoken

"""
Then try this code:
"""
logger.info("Then try this code:")
...
"""
logger.info("## Create Your First Synthesizer")
...
- [Optional] `model`: a string specifying which of Ollama's GPT models to use for generation, **OR** [any custom LLM model](/docs/metrics-introduction#using-a-custom-llm) of type `DeepEvalBaseLLM`. Defaulted to `gpt-4.1`.
...
"""
logger.info("## Generate Your First Golden")
...
"""
logger.info("In this example, we've used the `generate_goldens_from_docs` method, which one one of the four generation methods offered by `deepeval`'s `Synthesizer`. The four methods include:")
...
"""
logger.info("## Save Your Synthetic Dataset")
...
"""
logger.info("This keeps your dataset on the cloud and you'll be able to edit and version control it in one place. When you are ready to evaluate your LLM application using the generated goldens, simply pull the dataset from the cloud like how you would pull a GitHub repo:")
...
"""
logger.info("The `save_as()` method supports the following parameters:")
...
"""
logger.info("## Customize Your Generations")
...
"""
logger.info("### Evolution Complexity")
...
"""
logger.info("There are **TWO** optional parameters when creating an `EvolutionConfig`:")
