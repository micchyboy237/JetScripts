...
"""
logger.info("Pass in `deepeval`'s `CallbackHandler` for LangGraph to your agent's invoke method.")
...
"""
logger.info("Pass in `deepeval`'s `CallbackHandler` for LangChain to your agent's invoke method.")


def multiply(a: int, b: int) -> int:
    return a * b

llm = init_chat_model("gpt-4.1", model_provider="ollama")
...
"""
logger.info("Instrument `deepeval` as an span handler for CrewAI and import your `Agent` from `deepeval` instead.")
...
"""
logger.info("This will prevent traces from being lost in case of an early program termination.")
...
"""
logger.info("Run your agent as you would normally do:")
...
"""
logger.info("You can also use these models for evaluation: [Ollama](https://deepeval.com/integrations/models/ollama), [Azure Ollama](https://deepeval.com/integrations/models/azure-ollama), [Ollama](https://deepeval.com/integrations/models/anthropic), [Gemini](https://deepeval.com/integrations/models/gemini), etc. To use **ANY** custom LLM of your choice, [check out this part of the docs](/guides/guides-using-custom-llms).")
...
"""
logger.info("Other metrics on `deepeval` can also be used to evaluate agents but _ONLY_ if you run [component-level evaluations](/docs/getting-started-agents#component-level-evaluations), since they require you to set up an LLM test case. These metrics include:")
...
"""
logger.info("Supply the **task completion metric** to the metrics argument inside the `CallbackHandler`.")
...
"""
logger.info("Supply the **task completion metric** to the metrics argument inside the `Agent` from `deepeval`.")
...
"""
logger.info("In the end, you will have the same number of test cases in your test run as goldens in the dataset you ran evals with.")
...
"""
logger.info("## Evaluate Agentic Components")
...
"""
logger.info("Supply the metrics to the `@observe` decorator of each function, then define a test case in `update_span` if needed. The test case should include every parameter required by the metrics you select.")
...
def llm_component(query):
    response = client.responses.create(model="llama3.2", input=[{"role": "user", "content": query}], tools=tools)
...
It does so by invoking `llm`, which calls the LLM using [Ollama’s Responses API](https://platform.ollama.com/docs/api-reference/responses). The LLM can decide to either produce a direct response to the user query or call `web_search_tool` to perform a web search.

:::info
Although `tools=[...]` is condensed in the example below, it must be defined in the following format before being passed to Ollama’s `client.responses.create` method.
"""
logger.info("It does so by invoking `llm`, which calls the LLM using [Ollama’s Responses API](https://platform.ollama.com/docs/api-reference/responses). The LLM can decide to either produce a direct response to the user query or call `web_search_tool` to perform a web search.")
...
This will benchmark your agent for this point-in-time and **create a test run.**
"""
logger.info("In the example below, [Task Completion](/docs/metrics-task-completion) is used to evaluate the performance of the `your_ai_agent` function, while [Argument Correctness](/docs/metrics-argument-correctness) is used to evaluate `llm`.")
