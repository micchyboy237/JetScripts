...
"""
logger.info("Then, to generate synthetic `Golden`s from documents, simply provide a list of document paths:")
...
"""
logger.info("## Customize Context Construction")
...
- [Optional] `embedder`: a string specifying which of Ollama's embedding models to during document parsing and context grouping, **OR** [any custom embedding model](/guides/guides-using-custom-embedding-models) of type `DeepEvalBaseEmbeddingModel`. Defaulted to 'nomic-embed-text'.
...
"""
logger.info("## How Does Context Construction Work?")
