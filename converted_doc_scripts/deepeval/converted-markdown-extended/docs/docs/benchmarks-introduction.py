...
To benchmark your LLM, you will need to wrap your LLM implementation (which could be anything such as a simple API call to Ollama, or a Hugging Face transformers model) within `deepeval`'s `DeepEvalBaseLLM` class. Visit the [custom models section](/docs/metrics-introduction#using-a-custom-llm) for a detailed guide on how to create a custom model object.
...
logger.debug(mistral_7b("Write me a joke"))
...
"""
logger.info("Notice you can also **optionally** define a `batch_generate()` method if your LLM offers an API to generate outputs in batches.")
...
"""
logger.info("Lastly, call the `evaluate()` method to benchmark your custom LLM:")

...

results = benchmark.evaluate(model=mistral_7b, batch_size=5)
logger.debug("Overall Score: ", results)
...
logger.debug("Overall Score:", benchmark.overall_score)
...
logger.debug("Task-specific Scores: ", benchmark.task_scores)
...
logger.debug("Detailed Predictions: ", benchmark.predictions)
...
"""
logger.info("## Configurating LLM Benchmarks")
...
:::
"""
logger.info("Not all benchmarks offers CoTs as a prompting technique, but the [original paper for BIG-Bench Hard](https://arxiv.org/abs/2210.09261) found major improvements when using CoTs prompting during benchmarking.")
