...
from jet.adapters.langchain.chat_ollama import AzureChatOpenAI
...
"""
logger.info("## Why DeepEval Metrics?")
...
logger.debug(correctness.score, correctness.reason)
...
logger.debug(professionalism_metric.score, professionalism_metric.reason)
...
logger.debug(text_image_coherence.score, text_image_coherence.reason)
...
logger.debug(relevancy.score, relevancy.reason)
...
"""
logger.info("The most used agentic metrics include:")
...
"""
logger.info("Chatbots require "conversational" (or multi-turn) metrics and they include:")
...
logger.debug(role_adherence.score, role_adherence.reason)
...
logger.debug(image_coherence.score, image_coherence.reason)
...
logger.debug(bias.score, bias.reason)
...
"""
logger.info("## Choosing Your Metrics")
...
You can use **ANY** LLM judge in `deepeval`, including Ollama, Azure Ollama, Ollama, Ollama, Gemini, LiteLLM, etc. You can also wrap your own LLM API in `deepeval`'s `DeepEvalBaseLLM` class to use ANY model of your choice. [Click here](/guides/guides-using-custom-llms) for full guide.

<Tabs>

<TabItem value="open-ai" label="Open AI">

# To use Ollama for `deepeval`'s LLM metrics, supply your `OPENAI_API_KEY` in the CLI:
"""
logger.info("## Configure LLM Judges")
...
"""
# logger.info("Alternatively, if you're working in a notebook environment (Jupyter or Colab), set your `OPENAI_API_KEY` in a cell:")
...
`deepeval` also allows you to use Azure Ollama for metrics that are evaluated using an LLM. Run the following command in the CLI to configure your `deepeval` environment to use Azure Ollama for **all** LLM-based metrics.
"""
logger.info("Please **do not include** quotation marks when setting your `API_KEYS` as environment variables if you're working in a notebook environment.")
...
"""
:::info
Your Ollama API version must be at least `2024-08-01-preview`, when structured output was released.
:::

Note that the `model-version` is **optional**. If you ever wish to stop using Azure Ollama and move back to regular Ollama, simply run:
"""
logger.info("Your Ollama API version must be at least `2024-08-01-preview`, when structured output was released.")
...
"""
logger.info("Before getting started, make sure your [Ollama model](https://ollama.com/search) is installed and running. You can also see the full list of available models by clicking on the previous link.")
...
"""
logger.info("To use **Ollama** models for your metrics, run `deepeval set-ollama <model>` in your CLI. For example:")
...
"""
logger.info("Optionally, you can specify the **base URL** of your local Ollama model instance if you've defined a custom port. The default base URL is set to `http://localhost:11434`.")

deepeval set-ollama deepseek-r1:1.5b \
    --base-url="http://localhost:11434"

"""
To stop using your local Ollama model and move back to Ollama, run:
"""
logger.info("To stop using your local Ollama model and move back to Ollama, run:")
...
"""
logger.info("The `deepeval set-ollama` command is used exclusively to configure LLM models. If you intend to use a custom embedding model from Ollama with the synthesizer, please [refer to this section of the guide](/guides/guides-using-custom-embedding-models).")
...
All the examples can be [found here](/guides/guides-using-custom-llms#more-examples), but down below is a quick example of a custom Azure Ollama model through langchain's `AzureChatOpenAI` module for evaluation:
"""
logger.info("This includes any of your favorite models such as:")
...
class AzureOpenAI(DeepEvalBaseLLM):
    ...
    def get_model_name(self):
        return "Custom Azure Ollama Model"
...
logger.debug(azure_openai.generate("Write me a joke"))
...
"""
logger.info("When creating a custom LLM evaluation model you should **ALWAYS**:")
...
"""
:::note
While the Azure Ollama command configures `deepeval` to use Azure Ollama globally for all LLM-Evals, a custom LLM has to be set each time you instantiate a metric. Remember to provide your custom LLM instance through the `model` parameter for metrics you wish to use it for.
:::
...
"""
logger.info("While the Azure Ollama command configures `deepeval` to use Azure Ollama globally for all LLM-Evals, a custom LLM has to be set each time you instantiate a metric. Remember to provide your custom LLM instance through the `model` parameter for metrics you wish to use it for.")
...
"""
logger.info("### For Component-Level Evals")
...
"""
logger.info("## Using Metrics Async")

...

metric = FaithfulnessMetric(async_mode=True)
metric.measure(test_case)
logger.debug("Metric finished!")
...
"""
logger.info("When `async_mode=True`, steps 1 and 2 execute concurrently (i.e., at the same time) since they are independent of each other, while `async_mode=False` causes steps 1 and 2 to execute sequentially instead (i.e., one after the other).")
...
"""
logger.info("## Debug A Metric Judgement")
...
You'll find this particularly valuable when [using a custom LLM](/guides/guides-using-custom-llms), as `deepeval`'s default metrics are optimized for Ollama's models, which are generally more powerful than most custom LLMs.
...
"""
logger.info("## Customize Metric Prompts")
...
"""
logger.info("## What About Non-LLM-as-a-judge Metrics?")
