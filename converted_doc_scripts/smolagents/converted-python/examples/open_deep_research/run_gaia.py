...
logger.debug("Make sure you deactivated any VPN like Tailscale, else some URLs will be blocked!")
...
def append_answer(entry: dict, jsonl_file: str) -> None:
    ...
    logger.debug("Answer exported to file:", jsonl_path.resolve())
...
""" + example["question"]
    ...
    try:
        ...
        logger.debug("Error on ", augmented_question, e)
        output = None
        intermediate_steps = []
        parsing_error = False
...
def get_examples_to_answer(answers_file: str, eval_ds: datasets.Dataset) -> list[dict]:
    logger.debug(f"Loading answers from {answers_file}...")
    try:
        done_questions = pd.read_json(answers_file, lines=True)["question"].tolist()
        logger.debug(f"Found {len(done_questions)} previous results!")
    except Exception as e:
        logger.debug("Error when loading records: ", e)
        logger.debug("No usable records! ▶️ Starting new.")
        done_questions = []
...
def main():
    args = parse_args()
    logger.debug(f"Starting run with arguments: {args}")

    eval_ds = load_gaia_dataset(args.use_raw_dataset, args.set_to_run)
    logger.debug("Loaded evaluation dataset:")
    logger.debug(pd.DataFrame(eval_ds)["task"].value_counts())
