...
def load_eval_dataset(eval_dataset):
    ...
    logger.debug(pd.DataFrame(eval_ds["simpleqa"]).head())
    return eval_ds
...
def answer_questions(
    ...
    for task in eval_ds:
        ...
        logger.debug(f"Starting processing and writing output to '{file_name}'")
        answered_questions = []
        ...
        logger.debug(f"Launching {parallel_workers} parallel workers.")
        ...
        if push_answers_to_hub and answers_dataset:
            logger.debug("Pushing answers to hub...")
