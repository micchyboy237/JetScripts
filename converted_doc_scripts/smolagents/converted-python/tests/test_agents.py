...
from smolagents import OllamaServerModel
...
a = 2
b = a * 2
    logger.debug("Failing due to unexpected indent")
logger.debug("Ok, calculation done!")
</code>
""",
            )
        else:  # We're at step 2
...
class TestToolCallingAgent:
    ...
    @patch("ollama.Ollama")
    def test_toolcalling_agent_stream_logs_multiple_tool_calls_observations(self, mock_ollama_client, test_tool):
        ...
        model = OllamaServerModel(model_id="fakemodel")
    ...
    def test_toolcalling_agent_final_answer_cannot_be_called_with_parallel_tool_calls(
        self, mock_ollama_client, test_tool
    ):
        ...
        class ExtendedChatCompletionOutputMessage(ChatCompletionOutputMessage):
            ...
            def model_dump(self, include=None):
                logger.debug("TOOL CALLS", self.tool_calls)
                return {
                    "role": self.role,
...
class TestCodeAgent:
    ...
    def test_syntax_error_show_offending_lines(self):
        ...
        assert isinstance(output, AgentText)
        assert output == "got an error"
        assert '    logger.debug("Failing due to unexpected indent")' in str(agent.memory.steps)
...
logger.debug("Ok, calculation done!")""")
    ...
    def test_use_structured_outputs_internally(self):
        expected_code = "logger.debug('Hello, world!')"
        model = MagicMock()
