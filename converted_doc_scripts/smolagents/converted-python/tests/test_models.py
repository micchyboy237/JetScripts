...
class TestModel:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("ollama/o3", ["stop1", "stop2"], False),  # o3 model should not include stop
            ("ollama/o4-mini", ["stop1", "stop2"], False),  # o4-mini model should not include stop
...
class TestInferenceClientModel:
    ...
    def test_init_model_with_tokens(self):
        ...
        with pytest.raises(ValueError, match="Received both `token` and `api_key` arguments."):
            InferenceClientModel(model_id="test-model", token="abc")
...
class TestOllamaServerModel:
    def test_client_kwargs_passed_correctly(self):
        ...
        with patch("ollama.Ollama") as MockOllama:
        ...
        assert model.client == MockOllama.return_value

    @require_run_all
    def test_streaming_tool_calls(self):
        model = OllamaServerModel(model_id="llama3.2")
        messages = [
            ChatMessage(
                role=MessageRole.USER,
                content=[
...
class TestAzureOllamaServerModel:
    def test_client_kwargs_passed_correctly(self):
        model_id = "gpt-3.5-turbo"
        ...
        with patch("ollama.Ollama") as MockOllama, patch("ollama.AzureOllama") as MockAzureOllama:
            model = AzureOllamaServerModel(
                model_id=model_id,
                api_key=api_key,
        ...
        assert MockOllama.call_count == 0
        MockAzureOllama.assert_called_once_with(
            base_url=None,
            api_key=api_key,
            api_version=api_version,
            azure_endpoint=azure_endpoint,
        ...
        assert model.client == MockAzureOllama.return_value
...
@pytest.mark.parametrize(
    ...
    [
        (AzureOllamaServerModel, {}, ("ollama.AzureOllama", {}), False),
        ...
        (OllamaServerModel, {}, ("ollama.Ollama", {}), False),
        (OllamaServerModel, {"flatten_messages_as_text": True}, ("ollama.Ollama", {}), True),
        (
            TransformersModel,
            {},
            [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ollama/o3", False),
        ("ollama/o4-mini", False),
        ("ollama/o3-2025-04-16", False),
        ("ollama/o4-mini-2025-04-16", False),
        # Supported models
