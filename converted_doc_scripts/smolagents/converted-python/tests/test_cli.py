...
from smolagents.models import InferenceClientModel, LiteLLMModel, OllamaServerModel, TransformersModel
...
def test_load_model_ollama_server_model(set_env_vars):
    with patch("ollama.Ollama") as MockOllama:
        model = load_model("OllamaServerModel", "test_model_id")
    assert isinstance(model, OllamaServerModel)
    assert model.model_id == "test_model_id"
    assert MockOllama.call_count == 1
    assert MockOllama.call_args.kwargs["base_url"] == "https://api.fireworks.ai/inference/v1"
    assert MockOllama.call_args.kwargs["api_key"] == "test_fireworks_api_key"


def test_load_model_litellm_model():
    model = load_model("LiteLLMModel", "test_model_id", api_base="https://api.test.com")
