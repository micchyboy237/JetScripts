...
class TestAgentMemory:
    ...
    def test_return_all_code_actions(self):
        ...
        memory.steps = [
            ActionStep(step_number=1, timing=Timing(start_time=0.0, end_time=1.0), code_action="logger.debug('Hello')"),
            ActionStep(step_number=2, timing=Timing(start_time=0.0, end_time=1.0), code_action=None),
            ActionStep(step_number=3, timing=Timing(start_time=0.0, end_time=1.0), code_action="logger.debug('World')"),
        ]  # type: ignore
        assert memory.return_full_code() == "logger.debug('Hello')\n\nlogger.debug('World')"
