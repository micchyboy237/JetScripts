...
def supports_stop_parameter(model_id: str) -> bool:
    ...
    Not supported with reasoning models ollama/o3, ollama/o4-mini, and the ollama/gpt-5 series (and their versioned variants).

    Args:
        model_id (`str`): Model identifier (e.g. "ollama/o3", "o4-mini-2025-04-16")
...
class VLLMModel(Model):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Override the Ollama schema for VLLM compatibility
...
class InferenceClientModel(ApiModel):
    ...
    Parameters:
        ...
        api_key (`str`, *optional*):
            ...
            follow the same pattern as `ollama.Ollama` client. Cannot be used if `token` is set. Defaults to None.
        ...
        base_url (`str`, `optional`):
            ...
            follow the same pattern as `ollama.Ollama` client. Cannot be used if `model` is set. Defaults to None.
        **kwargs:
        ...
        **kwargs,
        ...
        if token is not None and api_key is not None:
            raise ValueError(
                ...
                " `api_key` is an alias for `token` to make the API compatible with Ollama's client."
...
class OllamaServerModel(ApiModel):
    """This model connects to an Ollama-compatible API server.

    Parameters:
        model_id (`str`):
            The model identifier to use on the server (e.g. "gpt-3.5-turbo").
        api_base (`str`, *optional*):
            The base URL of the Ollama-compatible API server.
        api_key (`str`, *optional*):
            The API key to use for authentication.
        ...
        client_kwargs (`dict[str, Any]`, *optional*):
            Additional keyword arguments to pass to the Ollama client (like organization, project, max_retries etc.).
        custom_role_conversions (`dict[str, str]`, *optional*):
            Custom role conversion mapping to convert message roles in others.
            Useful for specific models that do not support specific message roles like "system".
        flatten_messages_as_text (`bool`, default `False`):
        ...
        **kwargs:
            Additional keyword arguments to forward to the underlying Ollama API completion call, for instance `temperature`.
    """
    ...
    def create_client(self):
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'ollama' extra to use OllamaServerModel: `pip install 'smolagents[ollama]'`"
            ) from e

        return ollama.Ollama(**self.client_kwargs)
...
OllamaModel = OllamaServerModel


class AzureOllamaServerModel(OllamaServerModel):
    """This model connects to an Azure Ollama deployment.

    Parameters:
        model_id (`str`):
            The model deployment name to use when connecting (e.g. "llama3.2").
        azure_endpoint (`str`, *optional*):
            The Azure endpoint, including the resource, e.g. `https://example-resource.azure.ollama.com/`. If not provided, it will be inferred from the `AZURE_OPENAI_ENDPOINT` environment variable.
        api_key (`str`, *optional*):
#             The API key to use for authentication. If not provided, it will be inferred from the `AZURE_OPENAI_API_KEY` environment variable.
        ...
        client_kwargs (`dict[str, Any]`, *optional*):
            Additional keyword arguments to pass to the AzureOllama client (like organization, project, max_retries etc.).
        custom_role_conversions (`dict[str, str]`, *optional*):
            Custom role conversion mapping to convert message roles in others.
            Useful for specific models that do not support specific message roles like "system".
        **kwargs:
            Additional keyword arguments to forward to the underlying Azure Ollama API completion call.
    """
    ...
    def create_client(self):
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'ollama' extra to use AzureOllamaServerModel: `pip install 'smolagents[ollama]'`"
            ) from e

        return ollama.AzureOllama(**self.client_kwargs)


AzureOllamaModel = AzureOllamaServerModel
