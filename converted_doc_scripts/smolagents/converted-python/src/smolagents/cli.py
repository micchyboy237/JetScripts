...
from smolagents import CodeAgent, InferenceClientModel, LiteLLMModel, Model, OllamaServerModel, Tool, TransformersModel
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help="The model type to use (e.g., InferenceClientModel, OllamaServerModel, LiteLLMModel, TransformersModel)",
    )
    parser.add_argument(
        "--model-id",
        type=str,
...
def load_model(
    ...
    if model_type == "OllamaServerModel":
        return OllamaServerModel(
...
def run_smolagent(
    ...
    logger.debug(f"Running agent with these tools: {tools}")
