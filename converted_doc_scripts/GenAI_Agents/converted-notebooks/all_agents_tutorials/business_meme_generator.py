...
async def get_meme_templates() -> Dict[str, TemplateInfo]:
    ...
    async def async_func_186():
        async with aiohttp.ClientSession() as session:
            async with session.get("https://api.memegen.link/templates/") as response:
                async def run_async_code_93c83012():
                    all_templates = await response.json()
                    return all_templates
                all_templates = asyncio.run(run_async_code_93c83012())
                logger.success(format_json(all_templates))
    ...
    result = asyncio.run(async_func_186())
...
async def display_meme(url: str):
    ...
    try:
        async def async_func_404():
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        async def run_async_code_777025c4():
                            image_data = await response.read()
                            return image_data
                        image_data = asyncio.run(run_async_code_777025c4())
                        logger.success(format_json(image_data))
        ...
        result = asyncio.run(async_func_404())
        logger.success(format_json(result))
    except Exception as e:
...
async def run_workflow(website_url: str):
    ...
    async def run_async_code_ac8f668c():
        async def run_async_code_6687adef():
            available_templates = await get_meme_templates()
            return available_templates
        available_templates = asyncio.run(run_async_code_6687adef())
        logger.success(format_json(available_templates))
        return available_templates
    available_templates = asyncio.run(run_async_code_ac8f668c())
    logger.success(format_json(available_templates))
    ...
    try:
        ...
        async def run_async_code_3f6df20b():
            async def run_async_code_4b40ac1d():
                result = await app.ainvoke(initial_state)
                return result
            result = asyncio.run(run_async_code_4b40ac1d())
            logger.success(format_json(result))
            return result
        ...
        if isinstance(result, dict) and "company_context" in result:
            ...
            if "generated_memes" in result:
                ...
                for meme_id, meme_info in result["generated_memes"].items():
                    ...
                    logger.debug("")
                    async def run_async_code_c07f3801():
                        async def run_async_code_19208469():
                            meme_image = await display_meme(meme_info["final_url"])
                            return meme_image
                        meme_image = asyncio.run(run_async_code_19208469())
                        logger.success(format_json(meme_image))
                        return meme_image
                    meme_image = asyncio.run(run_async_code_c07f3801())
                    logger.success(format_json(meme_image))
                    if meme_image:
                      display(meme_image)
                    else:
...
async def run_async_code_e45f39d8():
    async def run_async_code_dd569cb2():
        result = await run_workflow(website_url)
        return result
    result = asyncio.run(run_async_code_dd569cb2())
    logger.success(format_json(result))
    return result
result = asyncio.run(run_async_code_e45f39d8())
logger.success(format_json(result))
