...
async def run_async_code_cc1bb2b1():
    async def run_async_code_ec446b38():
        tools = await discover_tools()
        return tools
    tools = asyncio.run(run_async_code_ec446b38())
    logger.success(format_json(tools))
    return tools
...
async def execute_tool(tool_name: str, arguments: Dict[str, Any]):
    ...
    result = asyncio.run(async_func_27())
    logger.success(format_json(result))
        ...
        if json_match:
            ...
            if "tool" in tool_request and "arguments" in tool_request:
                ...
                async def run_async_code_f9760798():
                    async def run_async_code_cc54571c():
                        tool_result = await execute_tool(tool_name, arguments)
                        return tool_result
                    tool_result = asyncio.run(run_async_code_cc54571c())
...
async def run_async_code_93c374ec():
    async def run_async_code_ce9f0bd6():
        response, messages = await query_claude(query, tools)
        return response, messages
    response, messages = asyncio.run(run_async_code_ce9f0bd6())
...
try:
    if tools:
        ...
        async def run_async_code_0a0d84f4():
            async def run_async_code_3dcc6690():
                result = await execute_tool(tool_name, arguments)
                return result
            result = asyncio.run(run_async_code_3dcc6690())
            logger.success(format_json(result))
            return result
    ...
    try:
        if 'tools' not in globals() or not tools:
            ...
            async def run_async_code_83797809():
                async def run_async_code_bb73a22b():
                    tools_local = await discover_tools()
                    return tools_local
                tools_local = asyncio.run(run_async_code_bb73a22b())
        ...
        while True:
            ...
            async def run_async_code_bffa5c19():
                async def run_async_code_bec93bbd():
                    response, messages = await query_claude(user_input, tools_local, messages)
                    return response, messages
                response, messages = asyncio.run(run_async_code_bec93bbd())
                logger.success(format_json(response, messages))
                return response, messages
