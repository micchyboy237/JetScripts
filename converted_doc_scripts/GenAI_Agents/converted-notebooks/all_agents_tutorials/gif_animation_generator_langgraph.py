    ...
    for attempt in range(retries):
        try:
            async def async_func_54():
                ...
                return response
            response = asyncio.run(async_func_54())
...
async def create_images(state: GraphState) -> GraphState:
    ...
    async def run_async_code_89f30b4d():
        async def run_async_code_b0d2b05b():
            image_urls = await asyncio.gather(*tasks)
            return image_urls
        image_urls = asyncio.run(run_async_code_b0d2b05b())
        logger.success(format_json(image_urls))
        return image_urls
    image_urls = asyncio.run(run_async_code_89f30b4d())
    logger.success(format_json(image_urls))
...
async def create_gif(state: GraphState) -> GraphState:
    ...
    images = []
    async def async_func_82():
        async with aiohttp.ClientSession() as session:
            tasks = [get_image_data(session, url) for url in image_urls if url]
            async def run_async_code_4108a51e():
                image_data_list = await asyncio.gather(*tasks)
                return image_data_list
            image_data_list = asyncio.run(run_async_code_4108a51e())
            logger.success(format_json(image_data_list))
            
        return result

    result = asyncio.run(async_func_82())
    logger.success(format_json(result))
...
async def run_async_code_869264d5():
    async def run_async_code_7e29ec88():
        result = await run_workflow(query)
        return result
    result = asyncio.run(run_async_code_7e29ec88())
    logger.success(format_json(result))
    return result
result = asyncio.run(run_async_code_869264d5())
logger.success(format_json(result))
...
if result and result["gif_data"]:
    ...
    if save_gif == 'yes':
        ...
        logger.debug(f"GIF saved as {filename}")
    else:
