...
tools = [write_email, check_calendar_availability,
...
def triage_email_with_procedural_memory(state: State, config: dict, store: InMemoryStore) -> dict:
    ...
    current_prompt_template = store.get(
    ...
    prompt = PromptTemplate.from_template(current_prompt_template).format(
        examples=formatted_examples, **email)
    messages = [HumanMessage(content=prompt)]
    result = llm_router.invoke(messages)
    return {"triage_result": result.classification}
...
def create_email_agent(store):
    workflow = StateGraph(State)
    workflow.add_node("triage", lambda state,
                      config: triage_email_with_procedural_memory(state, config, store))
...
# Create a fresh agent with updated prompts
