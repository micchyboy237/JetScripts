...
async def discover_tools():
    ...
    async with stdio_client(server_params) as (read, write):
        ...
        async with ClientSession(read, write) as session:
            ...
            logger.debug(
                f"{GREEN}✅ Successfully discovered {len(tool_info)} tools{RESET}")
            logger.debug(f"{SEP}")
            return tool_info
...
"""
    ...
    filtered_messages = [
        msg for msg in previous_messages if msg["role"] != "system"]
    ...
    try:
        ...
        if json_match:
            ...
            if "tool" in tool_request and "arguments" in tool_request:
                ...
                logger.debug(
                    f"{YELLOW}🔧 Claude wants to use tool: {tool_name}{RESET}")
                ...
                logger.debug(
                    f"{PURPLE}🔄 Getting Claude's interpretation of the tool result...{RESET}")
        ...
        logger.debug(
            f"{YELLOW}⚠️ No tool usage detected in response: {str(e)}{RESET}")
...
async def chat_session():
    ...
    try:
        if 'tools' not in globals() or not tools:
            logger.debug(
                f"{BLUE}🔍 No tools found, discovering available tools...{RESET}")
        else:
            tools_local = tools

        logger.debug(
            f"{GREEN}✅ Agent ready with {len(tools_local)} tools:{RESET}")
