...
* Interactive DataMapPlot and deprecate non-chat Ollama models by [@MaartenGr](https://github.com/MaartenGr) in [#2287](https://github.com/MaartenGr/BERTopic/pull/2287)
...
* Fix handling Ollama returning None objects by [@jeaninejuliettes](https://github.com/jeaninejuliettes) in [#2280](https://github.com/MaartenGr/BERTopic/pull/2280)
...
* Migrated OpenAIBackend to ollama>=1 by [@peguerosdc](https://github.com/peguerosdc) in [#1724](https://github.com/MaartenGr/BERTopic/pull/1724)
...
* Fixed AuthenticationError while using Ollama() [#1678](https://github.com/MaartenGr/BERTopic/issues/1678)
...
* Replacing the deprecated `text-ada-001` model with the latest `nomic-embed-text` from Ollama by [@atmb4u](https://github.com/atmb4u) in [#1800](https://github.com/MaartenGr/BERTopic/pull/1800)
* Prevent invalid empty input error when retrieving embeddings with ollama backend by [@liaoelton](https://github.com/liaoelton) in [#1827](https://github.com/MaartenGr/BERTopic/pull/1827)
...
* Prevent `model` argument being passed twice when using `generator_kwargs` in Ollama by [@ninavandiermen](https://github.com/ninavandiermen) in [#1733](https://github.com/MaartenGr/BERTopic/pull/1733)
...
* Fixed n-gram Keywords need delimiting in Ollama() [#1546](https://github.com/MaartenGr/BERTopic/issues/1546)
* Fixed Ollama v1.0 issues [#1629](https://github.com/MaartenGr/BERTopic/issues/1629)
...
* Fixed engine support for Azure Ollama embeddings [#1577](https://github.com/MaartenGr/BERTopic/issues/1487)
* Fixed Ollama Representation: KeyError: 'content' [#1570](https://github.com/MaartenGr/BERTopic/issues/1570)
...
*  Ollama: Allow retrying on Service Unavailable errors by [@agamble](https://github.com/agamble) in [#1407](https://github.com/MaartenGr/BERTopic/pull/1407)
...
"""
logger.info("The new `.merge_models` feature allows for any number of fitted BERTopic models to be merged. Doing so allows for a number of use cases:")
...
"""
logger.info("Then, we can combine all three models into one with `.merge_models`:")
...
"""
logger.info("Zeroshot Topic Modeling is a technique that allows you to find pre-defined topics in large amounts of documents. This method allows you to not only find those specific topics but also create new topics for documents that would not fit with your predefined topics.")
...
"""
logger.info("When we run `topic_model.get_topic_info()` you will see something like this:")
...
Let's illustrate this with an example. In the code below, we will use [`tiktoken`](https://github.com/ollama/tiktoken) to count the number of tokens in each document and limit them to 100 tokens. All documents that have more than 100 tokens will be truncated.

We use `bertopic.representation.Ollama` to represent our topics with nicely written labels. We specify that documents that we put in the prompt cannot exceed 100 tokens each. Since we will put 4 documents in the prompt, they will total roughly 400 tokens:
"""
logger.info("When using LLMs with BERTopic, we can truncate the input documents in `[DOCUMENTS]` in order to reduce the number of tokens that we have in our input prompt. To do so, all text generation modules have two parameters that we can tweak:")


tokenizer= tiktoken.encoding_for_model("gpt-3.5-turbo")

client = ollama.Ollama()
representation_model = Ollama(
    client,
    model="llama3.2",
    delay_in_seconds=2,
    chat=True,
...
* Added new embedding models
    * Ollama: `bertopic.backend.OpenAIBackend`
    * Cohere: `bertopic.backend.CohereBackend`
* Added example of [summarizing topics](https://maartengr.github.io/BERTopic/getting_started/representation/representation.html#summarization) with Ollama's GPT-models
* Added `nr_docs` and `diversity` parameters to Ollama and Cohere representation models
...
* Added `exponential_backoff` parameter to `Ollama` model
...
"""
logger.info("In this new release, we introduce `multi-aspect topic modeling`! During the `.fit` or `.fit_transform` stages, you can now get multiple representations of a single topic. In practice, it works by generating and storing all kinds of different topic representations (see image below).")
...
"""
logger.info("Saving the topic modeling with `.safetensors` or `pytorch` has a number of advantages:")
...
* Added `delay_in_seconds` parameter to Ollama and Cohere representation models for throttling the API
...
Within Ollama's API, the ChatGPT models use a different API structure compared to the GPT-3 models.
In order to use ChatGPT with BERTopic, we need to define the model and make sure to set `chat=True`:
"""
logger.info("## **Version 0.14.1**")


ollama.api_key = MY_API_KEY
representation_model = Ollama(model="llama3.2", delay_in_seconds=10, chat=True)
...
"""
logger.info("Prompting with ChatGPT is very satisfying and can be customized in BERTopic by using certain tags.")
...
"""
then that will be rendered as follows and passed to Ollama's API:
"""
logger.info("then that will be rendered as follows and passed to Ollama's API:")
...
* Fine-tune [topic representations](https://maartengr.github.io/BERTopic/getting_started/representation/representation.html) with `bertopic.representation`
    * Diverse range of models, including KeyBERT, MMR, POS, Transformers, Ollama, and more!'
...
"""
logger.info("Our candidate topics, as extracted with c-TF-IDF, do not take into account a keyword's part of speech as extracting noun-phrases from all documents can be computationally quite expensive. Instead, we can leverage c-TF-IDF to perform part of speech on a subset of keywords and documents that best represent a topic.")
...
"""
logger.info("When we calculate the weights of keywords, we typically do not consider whether we already have similar keywords in our topic. Words like "car" and "cars"")
...
"""
logger.info("To perform zero-shot classification, we feed the model with the keywords as generated through c-TF-IDF and a set of candidate labels. If, for a certain topic, we find a similar enough label, then it is assigned. If not, then we keep the original c-TF-IDF keywords.")
...
"""
logger.info("Nearly every week, there are new and improved models released on the 🤗 [Model Hub](https://huggingface.co/models) that, with some creativity, allow for")
...
"""
logger.info("Instead of using a language model from 🤗 transformers, we can use external APIs instead that")
...
"""
logger.info("Then, get yourself an API key and use Cohere's API as follows:")
...
<h3><b><a href="https://maartengr.github.io/BERTopic/getting_started/representation/representation.html#ollama">Text Generation: Ollama</a></b></h3>

Instead of using a language model from 🤗 transformers, we can use external APIs instead that
do the work for you. Here, we can use [Ollama](https://ollama.com/api/) to extract our topic labels from the candidate documents and keywords.
To use this, you will need to install ollama first:

# pip install ollama

Then, get yourself an API key and use Ollama's API as follows:
"""
logger.info("# pip install ollama")
...
"""
![ollama](https://user-images.githubusercontent.com/25746895/218417357-cf8c0fab-4450-43d3-b4fd-219ed276d870.svg)
...
like ollama:
"""
logger.info("In BERTopic, we can leverage this package in order to more efficiently combine external knowledge. Here, this")
...
"""
logger.info("Then, you can create your chain as follows:")

chain = load_qa_chain(Ollama(temperature=0, openai_api_key=MY_API_KEY), chain_type="stuff")

"""
Finally, you can pass the chain to BERTopic as follows:
"""
logger.info("Finally, you can pass the chain to BERTopic as follows:")
...
"""
logger.info("To calculate and visualize the topic distributions in a document on a token-level, we can run the following:")
...
"""
logger.info("BERTopic now supports fully-supervised classification! Instead of using a clustering algorithm, like HDBSCAN, we can replace it with a classifier, like Logistic Regression:")
...
"""
logger.info("When you already have a bunch of labels and simply want to extract topic representations from them, you might not need to actually learn how those can predicted. We can bypass the `embeddings -> dimensionality reduction -> clustering` steps and go straight to the c-TF-IDF representation of our labels:")
...
"""
logger.info("Outlier reduction is an frequently-discussed topic in BERTopic as its default cluster model, HDBSCAN, has a tendency to generate many outliers. This often helps in the topic representation steps, as we do not consider documents that are less relevant, but you might want to still assign those outliers to actual topics. In the modular philosophy of BERTopic, keeping training times in mind, it is now possible to perform outlier reduction **after** having trained your topic model. This allows for ease of iteration and prevents having to train BERTopic many times to find the parameters you are searching for. There are 4 different strategies that you can use, so make sure to check out the [documentation](https://maartengr.github.io/BERTopic/getting_started/outlier_reduction/outlier_reduction.html)!")
...
"""
logger.info("The default embedding model in BERTopic is one of the amazing sentence-transformers models, namely `"all-MiniLM-L6-v2"`. Although this model performs well out of the box, it typically needs a GPU to transform the documents into embeddings in a reasonable time. Moreover, the installation requires `pytorch` which often results in a rather large environment, memory-wise.")
...
"""
logger.info("Then, we can use BERTopic without `sentence-transformers` as follows using a CPU-based embedding technique:")
...
"""
logger.info("As a result, the entire package and resulting model can be run quickly on the CPU and no GPU is necessary!")
...
"""
logger.info("Only the topics for the most recent batch of documents are tracked. If you want to be using online topic modeling, not for a streaming setting but merely for low-memory use cases, then it is advised to also update the `.topics_` attribute as variations such as hierarchical topic modeling will not work afterward:")
...
"""
logger.info("Explicitly define, use, and adjust the `ClassTfidfTransformer` with new parameters, `bm25_weighting` and `reduce_frequent_words`, to potentially improve the topic representation:")
