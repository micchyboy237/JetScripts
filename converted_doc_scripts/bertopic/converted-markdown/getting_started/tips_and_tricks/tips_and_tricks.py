...
"""
logger.info("We can also use the `ClassTfidfTransformer` to reduce the impact of frequent words. The end result is very similar to explicitly removing stopwords but this process does this automatically:")
...
"""
logger.info("Lastly, we can use a KeyBERT-Inspired model to reduce the appearance of stop words. This also often improves the topic representation:")
...
"""
logger.info("## **Diversify topic representation**")
...
"""
logger.info("Since MMR is using word embeddings to diversify the topic representations, it is necessary to pass the embedding model to BERTopic if you are using pre-computed embeddings:")
...
"""
logger.info("Depending on the embeddings you are using, you might want to normalize them first in order to")
...
"""
logger.info("## **Lightweight installation**")
...
"""
logger.info("This installs a bare-bones version of BERTopic. If you want to use UMAP and Model2Vec for instance, you'll need to first install them:")
...
"""
logger.info("## **Finding similar topics between models**")
...
"""
logger.info("In the code above, there is one important thing to note and that is the `sentence_model`. This model needs to be exactly the same in all BERTopic models, otherwise, it is not possible to compare topic models.")
...
"""
logger.info("Now that we know which topics are similar to each other, we can extract the most similar topics. Let's say that we have topic 10 in the `en_model` which represents a topic related to trains:")
...
"""
logger.info("To find the matching topic, we extract the most similar topic in the `sim_matrix`:")
...
"""
logger.info("Now that we have our images and captions, we need to generate our image embeddings:")
...
"""
logger.info("Finally, we can fit BERTopic the way we are used to, with documents and embeddings:")
...
"""
logger.info("After fitting our model, let's inspect a topic about skateboarders:")
...
"""
logger.info("Based on the above output, we can take an image to see if the representation makes sense:")
