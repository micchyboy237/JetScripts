...
from bertopic.representation import MaximalMarginalRelevance, Ollama
...
"""
logger.info("These patterns follow Spacy's [Rule-Based Matching](https://spacy.io/usage/rule-based-matching). Then, the resulting keywords are sorted by")
...
"""
logger.info("You can define custom POS patterns to be extracted:")
...
"""
logger.info("## **MaximalMarginalRelevance**")
...
All of the above models can make use of the candidate topics, as generated by c-TF-IDF, to further fine-tune the topic representations. For example, `MaximalMarginalRelevance` takes the keywords in the candidate topics and re-ranks them. Similarly, the keywords in the candidate topic can be used as the input for GPT-prompts in `Ollama`.

Although the default candidate topics are generated by c-TF-IDF, what if we were to chain these models? For example, we can use `MaximalMarginalRelevance` to improve upon the keywords in each topic before passing them to `Ollama`.

This is supported in BERTopic by simply passing a list of representation models when instantiation the topic model:
"""
logger.info("## **Chain Models**")


client = ollama.Ollama()
openai_generator = Ollama(client)
...
"""
logger.info("Then, we can use that model as follows:")
...
"""
logger.info("There are a few things to take into account when creating your custom model:")
...
!!! Tip
    ...
    model.
"""
logger.info("You can change the `__init__` however you want, it does not influence the underlying structure. This")
