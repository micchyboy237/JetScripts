...
"""
logger.info("Or select a SentenceTransformer model with your parameters:")
...
"""
logger.info("You can also choose a custom vectorizer for creating the vocabulary and define custom arguments for the distillatio process:")
...
"""
logger.info("These transformers also work quite well using `sentence-transformers` which has great optimizations tricks that make using it a bit faster.")
...
"""
logger.info("## **Scikit-Learn Embeddings**")
...
## **Ollama**
To use Ollama's external API, we need to define our key and explicitly call `bertopic.backend.OpenAIBackend`
...
"""
logger.info("Here, you will probably notice that creating the embeddings is quite fast whereas `fit_transform` is quite slow.")
