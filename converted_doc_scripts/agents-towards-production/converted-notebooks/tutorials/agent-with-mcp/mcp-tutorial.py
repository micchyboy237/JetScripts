...
"""
![](https://europe-west1-atp-views-tracker.cloudfunctions.net/working-analytics?notebook=tutorials--agent-with-mcp--mcp-tutorial)
...
📖 **For more background on the Model Context Protocol (MCP) and its applications, check out our detailed blog post:** [Model Context Protocol (MCP) explained](https://open.substack.com/pub/diamantai/p/model-context-protocol-mcp-explained?r=336pe4&utm_campaign=post&utm_medium=web&showWelcomeOnShare=false)
...
![Claude Desktop connected with MCP](assets/Claude_Desktop_with_MCP.png)

#### Step 5: Try ask the price of Bitcoin

Type in "What is the current price of Bitcoin ?", and you will get response like:

![Track Bitcoin price with MCP](assets/track_bitcoin_price_with_mcp.png)
...
![Track Crypto Market Data with MCP](assets/track_crypto_market_data_with_mcp.png)
...
![Track Crypto Market Data with MCP](assets/customized_mcp_host.png)
    ...
    async def async_func_17():
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                async def run_async_code_a6019461():
                    tools = await session.list_tools()
                    return tools
                tools = asyncio.run(run_async_code_a6019461())
...
async def run_async_code_cc1bb2b1():
    tools = await discover_tools()
    return tools
tools = asyncio.run(run_async_code_cc1bb2b1())
logger.success(format_json(tools))
    ...
    async def async_func_27():
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                async def run_async_code_3d69f36c():
                    result = await session.call_tool(tool_name, arguments)
                    return result
                result = asyncio.run(run_async_code_3d69f36c())
    ...
    try:
        ...
        if json_match:
            ...
            if "tool" in tool_request and "arguments" in tool_request:
                ...
                async def run_async_code_f9760798():
                    tool_result = await execute_tool(tool_name, arguments)
                    return tool_result
                tool_result = asyncio.run(run_async_code_f9760798())
...
async def run_async_code_93c374ec():
    response, messages = await query_claude(query, tools)
    return response, messages
response, messages = asyncio.run(run_async_code_93c374ec())
...
try:
    if tools:
        ...
        async def run_async_code_0a0d84f4():
            result = await execute_tool(tool_name, arguments)
            return result
        result = asyncio.run(run_async_code_0a0d84f4())
        logger.success(format_json(result))
...
async def chat_session():
    ...
    try:
        if 'tools' not in globals() or not tools:
            ...
            async def run_async_code_83797809():
                tools_local = await discover_tools()
                return tools_local
            tools_local = asyncio.run(run_async_code_83797809())
        ...
        while True:
            ...
            async def run_async_code_bffa5c19():
                response, messages = await query_claude(user_input, tools_local, messages)
                return response, messages
            response, messages = asyncio.run(run_async_code_bffa5c19())
            logger.success(format_json(response, messages))
...
async def run_async_code_f3a42e18():
    await chat_session()
asyncio.run(run_async_code_f3a42e18())
