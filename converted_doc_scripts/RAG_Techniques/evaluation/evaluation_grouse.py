...
"""
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/NirDiamant/RAG_Techniques/blob/main/evaluation/evaluation_grouse.ipynb)
...
### Import libraries
"""
logger.info("# RAG Evaluation and Meta-Evaluation with GroUSE")
...
"""
logger.info("### Avoid nested asyncio loops inside notebooks (this line is not needed if you run the code in a Python script)")
...
For this tutorial, you will need access to the Ollama API and get an Ollama API key. You can get one [here](https://platform.openai.com/api-keys).
"""
logger.info("### Setup your API key")

# os.environ["OPENAI_API_KEY"] = input("Add your Ollama API key:")
...
"""
logger.info("### Initialize the evaluator")
...
"""
logger.info("### Evaluate a good answer")
...
logger.debug("Answer Relevancy (1 to 5):", result.answer_relevancy.answer_relevancy)
logger.debug("Answer Relevancy (1 to 5):", result.answer_relevancy.answer_relevancy_justification)
logger.debug("Completeness (1 to 5):", result.completeness.completeness)
logger.debug("Completeness (1 to 5):", result.completeness.completeness_justification)
logger.debug("Faithfulness (0 or 1):", result.faithfulness.faithfulness)
logger.debug("Faithfulness (0 or 1):", result.faithfulness.faithfulness_justification)

"""
### How does it behave with an irrelevant answer?
"""
logger.info("### How does it behave with an irrelevant answer?")
...
logger.debug("Answer Relevancy (1 to 5):", result.answer_relevancy.answer_relevancy)
logger.debug("Justification:", result.answer_relevancy.answer_relevancy_justification)

"""
### Evaluation of an incomplete sample
"""
logger.info("### Evaluation of an incomplete sample")
...
logger.debug("Completeness (1 to 5):", result.completeness.completeness)
logger.debug("Justification:", result.completeness.completeness_justification)

"""
### Evaluation of an unfaithful sample
"""
logger.info("### Evaluation of an unfaithful sample")
...
logger.debug("Faithfulness (0 or 1):", result.faithfulness.faithfulness)
logger.debug("Justification:", result.faithfulness.faithfulness_justification)

"""
### Evaluation of information utility in case there is no answer to the question in the references
"""
logger.info("### Evaluation of information utility in case there is no answer to the question in the references")
...
logger.debug("Usefulness (0 or 1):", result.usefulness.usefulness)
logger.debug("Justification:", result.usefulness.usefulness_justification)
logger.debug("Positive Acceptance:", result.positive_acceptance)
logger.debug("Negative Rejection:", result.negative_rejection)

"""
### Note that all the results are cached and we can compute the global statistics on all the samples
"""
logger.info("### Note that all the results are cached and we can compute the global statistics on all the samples")
...
logger.debug("Average answer relevancy: ", evaluation_report.answer_relevancy)
logger.debug("Average completeness: ", evaluation_report.completeness)
logger.debug("Average faithfulness: ", evaluation_report.faithfulness)
logger.debug("Average usefulness: ", evaluation_report.usefulness)

"""
## Create your own Judge LLM

Since GPT-4 is expensive, let's create a new evaluator using llama3.1. For that, we need to adapt the evaluation prompt to the model by using the train set of the [GroUSE unit tests](https://huggingface.co/datasets/illuin/grouse).
Make sure that the output follows the same format as described in the prompts below.

### Answer Relevancy prompt
"""
logger.info("## Create your own Judge LLM")
...
### Completeness prompt
"""
logger.info("### Completeness prompt")
...
### Faithfulness prompt
"""
logger.info("### Faithfulness prompt")
...
"""
logger.info("### Save prompts to use them later")
...
"""
logger.info("### Check results on train set")

meta_evaluations = meta_evaluate_pipeline("llama3.1", prompts_path, train_set=True)
logger.debug("Aggregated metrics")
logger.debug(meta_evaluations.report)
...
"""
logger.info("### Once you are happy with your prompts, you can evaluate the Judge model on the test set")

meta_evaluations = meta_evaluate_pipeline("llama3.1", prompts_path, train_set=False)
