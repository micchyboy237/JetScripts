...
"""
logger.info("# RAPTOR: Recursive Abstractive Processing and Thematic Organization for Retrieval")
...
def print_query_details(result: Dict[str, Any]):
    ...
    for doc in result['retrieved_documents']:
        ...
        logger.debug(f"    Content: {doc['content'][:100]}...")  # Show first 100 characters
        logger.debug(f"    Similarity Score: {doc['similarity_score']}")
        logger.debug(f"    Tree Level: {doc['metadata'].get('level', 'Unknown')}")
        logger.debug(f"    Origin: {doc['metadata'].get('origin', 'Unknown')}")
        if 'child_docs' in doc['metadata']:
            logger.debug(f"    Number of Child Documents: {len(doc['metadata']['child_docs'])}")
        logger.debug()
