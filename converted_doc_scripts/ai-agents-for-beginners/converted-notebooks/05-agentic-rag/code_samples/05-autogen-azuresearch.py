...
async def main():
    ...
    for query_data in user_inputs:
        ...
        async def run_async_code_861c24b7():
            async def run_async_code_226028c8():
                result = await ask_unified_rag(query, evaluator, location)
                return result
            result = asyncio.run(run_async_code_226028c8())
            logger.success(format_json(result))
            return result
        result = asyncio.run(run_async_code_861c24b7())
        logger.success(format_json(result))
        if result:
