...
async def run_async_code_2defc511():
    """Initial test run with a simple query."""
...
#4.
You can change the system message to see how the LLM responds. We will cover `tools` in Lesson
"""
...
async def assistant_run():
    """Run the assistant agent with a vacation planning query."""
    ...
    assistant_content = getattr(
        getattr(response, "chat_message", None), "content", str(response))
    html_output += f"<div style='margin-left:20px; white-space:pre-wrap'>{assistant_content}</div>"
    html_output += "</div>"
...
# -------------------------
async def main():
    # run both tasks inside the same event loop
    await run_async_code_2defc511()
    await assistant_run()
    await client.close()
    logger.success("Assistant run complete.")
    logger.info("\n\n[DONE]", bright=True)
