...
logger.success(format_json(result))
    ...
    agent_definition = asyncio.run(async_func_28())
    ...
    try:
        ...
        async def run_async_code_1dd81ebf():
            await thread.delete() if thread else None
            return 
         = asyncio.run(run_async_code_1dd81ebf())
        logger.success(format_json())
        async def run_async_code_102e83d5():
            await client.agents.delete_agent(agent.id)
            return 
         = asyncio.run(run_async_code_102e83d5())
        logger.success(format_json())
