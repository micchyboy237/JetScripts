...
Below is a screenshot showing usage for `llama3.1` calls. This is useful to see costly steps and optimize your agent.
...
2. You set a model that is used as judge-model; in this case `llama3.1` queried via Azure.
...
async def run_async_code_4cec0c1e():
    await my_agent("What is the capital of France?")
    return 
 = asyncio.run(run_async_code_4cec0c1e())
...
for item in dataset.items:
    ...
    with item.run(
        ...
        async def run_async_code_67c4d228():
            async def run_async_code_4a461f23():
                generated_answer = await my_agent(user_query = item.input["text"])
                return generated_answer
            generated_answer = asyncio.run(run_async_code_4a461f23())
            logger.success(format_json(generated_answer))
            return generated_answer
        generated_answer = asyncio.run(run_async_code_67c4d228())
        logger.success(format_json(generated_answer))
