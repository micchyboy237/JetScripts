...
"""
logger.info("# Building Agent-to-Agent Communication Systems with MCP")
...
"""
logger.info("### 4. Multi-Turn Interactions")
...
"""
logger.info("## Implementing Long-Running Agents on MCP - Code Overview")
...
for i, step in enumerate(steps):
    ...
    async def run_async_code_d6fbf5e0():
        await anyio.sleep(2)  # Simulate work
        return 
     = asyncio.run(run_async_code_d6fbf5e0())
...
async def message_handler(message) -> None:
    if isinstance(message, types.ServerNotification):
        if isinstance(message.root, types.LoggingMessageNotification):
            console.logger.debug(f"📡 [dim]{message.root.params.data}[/dim]")
        elif isinstance(message.root, types.ProgressNotification):
            progress = message.root.params
            console.logger.debug(f"🔄 [yellow]{progress.message} ({progress.progress}/{progress.total})[/yellow]")
...
"""
logger.info("#### Elicitation - Requesting User Input")
...
elicit_result = asyncio.run(async_func_0())
logger.success(format_json(elicit_result))
...
async def elicitation_callback(context, params):
    console.logger.debug(f"💬 Server is asking for confirmation:")
    console.logger.debug(f"   {params.message}")
...
"""
logger.info("#### Sampling - Requesting AI Assistance")
...
sampling_result = asyncio.run(async_func_0())
logger.success(format_json(sampling_result))
...
async def sampling_callback(context, params):
    ...
    console.logger.debug(f"🧠 Server requested sampling: {message_text}")
...
"""
logger.info("#### Resumability - Session Continuity Across Disconnections")
...
class SimpleEventStore(EventStore):
    ...
    async def replay_events_after(self, last_event_id: EventId, send_callback: EventCallback) -> StreamId | None:
        ...
        for _, event_id, message in self._events[start_index:]:
            async def run_async_code_782d8605():
                await send_callback(EventMessage(message, event_id))
                return 
             = asyncio.run(run_async_code_782d8605())
            logger.success(format_json())
...
### 1. Run the Demo
"""
logger.info("## Extending to Multi-Agent Communication on MCP")
...
"""
logger.info("### 2. Test Resumption Capabilities")
