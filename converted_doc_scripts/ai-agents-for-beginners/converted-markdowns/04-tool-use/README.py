...
1. **Initialize an LLM that supports function calling:**
    ...
    Not all models support function calling, so it's important to check that the LLM you are using does.     <a href="https://learn.microsoft.com/azure/ai-services/openai/how-to/function-calling" target="_blank">Azure Ollama</a> supports function calling. We can start by initiating the Azure Ollama client. 

    ```python
    # Initialize the Azure Ollama client
    client = AzureOllama(
...
1. **The function code required to carry out the task:**
    ...
    ```python
      def get_current_time(location):
        """
logger.info("# Tool Use Design Pattern")Get the current time for a given location"""
        logger.debug(f"get_current_time called with location: {location}")  
        location_lower = location.lower()
        
        for key, timezone in TIMEZONE_DATA.items():
            if key in location_lower:
                logger.debug(f"Timezone found for {key}")  
        ...
        logger.debug(f"No timezone data found for {location_lower}")  
...
"""
logger.info("# Handle function calls")
...
2. Action Tools:
    ...
    - <a href="https://learn.microsoft.com/azure/ai-services/agents/how-to/tools/openapi-spec?tabs=python&pivots=overview" target="_blank">Ollama defined tools</a>
...
"""
logger.info("### Azure AI Agent Service")
...
agent = project_client.agents.create_agent(
    model="llama3.1", name="my-agent", instructions="You are helpful agent",
    toolset=toolset
)
...
"""
logger.info("## What are the special considerations for using the Tool Use Design Pattern to build trustworthy AI agents?")
