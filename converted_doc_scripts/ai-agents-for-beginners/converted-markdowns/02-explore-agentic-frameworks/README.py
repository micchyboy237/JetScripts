...
async def main():
    async def async_func_36():
        ...
        return response
    response = asyncio.run(async_func_36())
...
using Microsoft.SemanticKernel.Connectors.AzureOllama;

ChatHistory chatHistory = [];
chatHistory.AddUserMessage("I'd like to go to New York on January 1, 2025");

var kernelBuilder = Kernel.CreateBuilder();
kernelBuilder.AddAzureOllamaChatCompletion(
...
async def run_async_code_63e0836e():
    async def run_async_code_643ca808():
        ...
        return var response
    var response = asyncio.run(run_async_code_643ca808())
    logger.success(format_json(var response))
    return var response
var response = asyncio.run(run_async_code_63e0836e())
logger.success(format_json(var response))
...
"""
logger.info("### Leverage Collaborative Tools")
...
- **AI Connectors**: This is an interface with external AI services and data sources for use in both Python and C
  ...
  ```
    ...
    Here you have a simple example of how you can create a kernel and add a chat completion service. Semantic Kernel creates a connection to an external AI service, in this case, Azure Ollama Chat Completion.
...
- **Plugins**: These encapsulate functions that an application can use. There are both ready-made plugins and custom ones you can create. A related concept is "prompt functions." Instead of providing natural language cues for function invocation, you broadcast certain functions to the model. Based on the current chat context, the model may choose to call one of these functions to complete a request or query. Here's an example:
  ...
  async def main():
      ...
      kernel_function = KernelFunctionFromPrompt(
          ...
          prompt="""
logger.info("### Learn in Real-Time")
...
async def main() -> None:
    ...
    logger.success(format_json(result))
        ...
        agent_definition = asyncio.run(async_func_39())
        ...
        try:
            for user_input in user_inputs:
                ...
                async def async_func_63():
                    response = await agent.get_response(
                        messages=user_input,
                        thread_id=thread,
                    )
                    return response
                response = asyncio.run(async_func_63())
                logger.success(format_json(response))
...
- **Agent**. Azure AI Agent Service integrates with Azure AI Foundry. Within AI Foundry, an AI Agent acts as a "smart" microservice that can be used to answer questions (RAG), perform actions, or completely automate workflows. It achieves this by combining the power of generative AI models with tools that allow it to access and interact with real-world data sources. Here's an example of an agent:
    ...
    In this example, an agent is created with the model `llama3.1`, a name `my-agent`, and instructions `You are helpful agent`. The agent is equipped with tools and resources to perform code interpretation tasks.
