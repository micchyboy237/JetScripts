...
"""
logger.info("# Metacognition in AI Agents")
...
logger.debug("Suggested Itinerary:", itinerary)
...
### Example Interaction
"""
logger.info("#### Why Metacognition Matters")
...
5. **Corrective RAG Process**
   ...
   - **Tool**: Travel Agent uses algorithms to rank and filter new search results, emphasizing the relevance based on user feedback.
     - Example:
       ...
       logger.debug("Updated Itinerary:", new_itinerary)
       ```
...
"""
logger.info("## 3. Corrective RAG System")
...
"""
logger.info("### Pre-emptive Context Load")
...
logger.debug(travel_agent.get_destination_info("Paris"))
logger.debug(travel_agent.get_destination_info("Tokyo"))
...
logger.debug("Initial Plan:", initial_plan)

refined_plan = travel_agent.iterate_plan(initial_plan, preferences, budget)
logger.debug("Refined Plan:", refined_plan)
...
Here’s how you can update the previous example to use Azure Ollama Services:

#### Requirements

1. You need to have an Azure subscription.
2. Create an Azure Ollama resource and get your API key.
...
logger.debug("Recommended Destinations:")
...
2. **Getting Recommendations (`get_recommendations` method)**: This method generates a prompt for the Azure Ollama service based on the user's preferences and makes an HTTP POST request to the Azure Ollama API to get re-ranked and scored destinations.

3. **Generating Prompt (`generate_prompt` method)**: This method constructs a prompt for the Azure Ollama, including the user's preferences and the list of destinations. The prompt guides the model to re-rank and score the destinations based on the provided preferences.

4. **API Call**: The `requests` library is used to make an HTTP POST request to the Azure Ollama API endpoint. The response contains the re-ranked and scored destinations.

5. **Example Usage**: The travel agent collects user preferences (e.g., interest in sightseeing and diverse culture) and uses the Azure Ollama service to get re-ranked and scored recommendations for travel destinations.

Make sure to replace `your_azure_openai_api_key` with your actual Azure Ollama API key and `https://your-endpoint.com/...` with the actual endpoint URL of your Azure Ollama deployment.
...
"""
logger.info("#### Code Explanation - Preference Booker")
...
logger.debug("Top Museums in Paris:", museums)
...
"""
logger.info("### Evaluating Relevancy")
...
logger.debug("Updated Itinerary with Feedback:", updated_items)
...
2. **Generating Code to Fetch Data**
   ...
   def generate_code_to_fetch_data(preferences):
       ...
       code = f"""
logger.info("### Search with Intent")
       def search_flights():
   ...
   def generate_code_to_fetch_hotels(preferences):
       ...
       code = f"""
logger.info("# Example: Generate code to search for hotels")
       def search_hotels():
...
3. **Executing Generated Code**
   ...
   logger.debug("Flight Options:", flights)
   logger.debug("Hotel Options:", hotels)
   ```
...
5. **Adjusting Based on Feedback**
   ...
   logger.debug("Updated Itinerary:", updated_itinerary)
   ```
...
"""
logger.info("# Execute the generated code using exec")
...
Here's an example:
"""
logger.info("#### Explanation - Booking Based on Feedback")
...
logger.debug(f"Recommended hotel (cheapest): {recommended_hotel['name']}")

reflection_result = agent.reflect_on_choice()
logger.debug(reflection_result)

adjusted_recommendation = agent.recommend_hotel(hotels, 'highest_quality')
logger.debug(f"Adjusted hotel recommendation (highest_quality): {adjusted_recommendation['name']}")
...
"""
logger.info("#### Agents Metacognition Abilities")
