...
To create your `.env` file run the following command in your terminal.
"""
logger.info("## Set Up for Samples using Azure AI Foundry and Azure AI Agent Service")
...
As a security best practice, we'll use [keyless authentication](https://learn.microsoft.com/azure/developer/ai/keyless-connections?tabs=csharp%2Cazure-cli?WT.mc_id=academic-105485-koreyst) to authenticate to Azure Ollama with Microsoft Entra ID. 
...
## Additional Environment Variables - Azure Search and Azure Ollama 

For the Agentic RAG Lesson - Lesson 5 - there are samples that use Azure Search and Azure Ollama.
...
- `AZURE_OPENAI_SERVICE` - Find this in the **Included capabilities** tab for **Azure Ollama Service** on the **Overview** page.
...
- `AZURE_OPENAI_CHAT_DEPLOYMENT_NAME` - Select your chat model (e.g., `llama3.1`) and note the **Deployment name** from the model details.

### Azure Portal

- `AZURE_OPENAI_ENDPOINT` - Look for **Azure AI services**, click on it, then go to **Resource Management**, **Keys and Endpoint**, scroll down to the "Azure Ollama endpoints", and copy the one that says "Language APIs".
...
Rather than hardcode your credentials, we'll use a keyless connection with Azure Ollama. To do so, we'll import `DefaultAzureCredential` and later call the `DefaultAzureCredential` function to get the credential.
"""
logger.info("### Step 3: Sign in to Azure")
