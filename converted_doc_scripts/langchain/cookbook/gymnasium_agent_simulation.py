            ...
            for attempt in tenacity.Retrying(
                ...
                before_sleep=lambda retry_state: logger.debug(
                    f"ValueError occurred: {
                        retry_state.outcome.exception()}, retrying..."
                ),
            ):
                with attempt:
                    action = self._act()
...
agent = GymnasiumAgent(model=ChatOllama(model="llama3.1"), env=env)
