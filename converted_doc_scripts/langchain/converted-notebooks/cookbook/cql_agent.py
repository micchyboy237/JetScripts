...
```bash
pip install ipykernel python-dotenv cassio pandas jet.adapters.langchain.chat_ollama langchain langchain-community langchainhub langchain_experimental ollama-multi-tool-use-parallel-patch
```
...
"""
logger.info("### Connect to Cassandra")
...
"""
logger.info("### Load Data into Cassandra")
...
"""
logger.info("Create and load tables:")
...
logger.debug("Sensors inserted successfully.")
...
def insert_data_batch(name, group):
    ...
    day, device = name
    logger.debug(f"Inserting batch for day: {day}, device: {device}")
...
"""
logger.info("## Load the Tools")


"""
The `CassandraDatabase` object is loaded from `cassio`, though it does accept a `Session`-type parameter as an alternative.
"""
logger.info("The `CassandraDatabase` object is loaded from `cassio`, though it does accept a `Session`-type parameter as an alternative.")
...
"""
logger.info("The tools can be invoked directly:")

logger.debug("Executing a CQL query:")
...
for tool in tools:
    logger.debug("\t" + tool.name + " - " + tool.description + " - " + str(tool))
