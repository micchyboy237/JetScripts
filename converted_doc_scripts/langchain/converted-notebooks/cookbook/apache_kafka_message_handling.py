...
"""
logger.info("#  Using Apache Kafka to route messages")
...
"""
logger.info("### 2. Build and install the llama-cpp-python library (with CUDA enabled so that we can advantage of Google Colab GPU")
...
"""
logger.info("### 3. Download and setup Kafka and Zookeeper instances")
...
"""
logger.info("### 4. Check that the Kafka Daemons are running")
...
"""
logger.info("### 5. Import the required dependencies and initialize required variables")
...
"""
logger.info("### 6. Download the "llama-2-7b-chat.Q4_K_M.gguf" model")

model_name = "llama-2-7b-chat.Q4_K_M.gguf"
model_path = f"./state/{model_name}"

if not Path(model_path).exists():
    logger.debug("The model path does not exist in state. Downloading model...")
    hf_hub_download("TheBloke/Llama-2-7b-Chat-GGUF", model_name, local_dir="state")
else:
    logger.debug("Loading model from state...")
...
"""
logger.info("### 7. Load the model and initialize conversational memory")
...
logger.debug("--------------------------------------------")
logger.debug(f"Prompt={chain.prompt}")
logger.debug("--------------------------------------------")
...
"""
logger.info("### 8. Initialize the chat conversation with the chat bot")
...
def chat_init():
    ...
    with Producer(
        ...
        logger.debug(f"Producing value {value}")
        producer.produce(
            topic="chat",
...
"""
logger.info("### 9. Initialize the reply function")
...
def reply(row: dict, state: State):
    ...
    logger.debug(row)
    logger.debug("-------------------------------")
    logger.debug(f"Thinking about the reply to: {row['text']}...")

    msg = chain.run(row["text"])
    logger.debug(f"{role.upper()} replying with: {msg}\n")
...
"""
logger.info("### 10. Check the Kafka topic for new human messages and have the model generate a reply")
...
"""
logger.info("### 11. Enter a human message")
...
logger.debug("Replied to chatbot with message: ")
logger.debug("--------------------------------------------")
logger.debug(value)
logger.debug("--------------------------------------------")
...
"""
logger.info("### Why route chat messages through Kafka?")
