...
"""
logger.info("Let's first use the model directly. `ChatModel`s are instances of LangChain "Runnables", which means they expose a standard interface for interacting with them. To just simply call the model, we can pass in a list of messages to the `.invoke` method.")
...
"""
logger.info("The model on its own does not have any concept of state. For example, if you ask a followup question:")
...
"""
logger.info("We now need to create a `config` that we pass into the runnable every time. This config contains information that is not part of the input directly, but is still useful. In this case, we want to include a `thread_id`. This should look like:")
...
"""
logger.info("This enables us to support multiple conversation threads with a single application, a common requirement when your application has multiple users.")
...
"""
logger.info("Great! Our chatbot now remembers things about us. If we change the config to reference a different `thread_id`, we can see that it starts the conversation fresh.")
...
"""
logger.info("However, we can always go back to the original conversation (since we are persisting it in a database)")
...
"""
logger.info("# Async function for node:")
...
"""
logger.info("We can now update our application to incorporate this template:")
...
"""
logger.info("We invoke the application in the same way:")
...
"""
logger.info("Awesome! Let's now make our prompt a little bit more complicated. Let's assume that the prompt template now looks something like this:")
...
"""
logger.info("Note that the entire state is persisted, so we can omit parameters like `language` if no changes are desired:")
...
"""
logger.info("To  use it in our chain, we just need to run the trimmer before we pass the `messages` input to our prompt.")

workflow = StateGraph(state_schema=State)


def call_model(state: State):
    logger.debug(f"Messages before trimming: {len(state['messages'])}")
    trimmed_messages = trimmer.invoke(state["messages"])
    logger.debug(f"Messages after trimming: {len(trimmed_messages)}")
    logger.debug("Remaining messages:")
...
"""
logger.info("Now if we try asking the model our name, it won't know it since we trimmed that part of the chat history. (By defining our trim stragegy as `'last'`, we are only keeping the most recent messages that fit within the `max_tokens`.)")
