...
"""
logger.info("## Hallucination detection and Factual Consistency Score")

resp = rag.invoke(query_str)
logger.debug(resp["answer"])
logger.debug(f"Vectara FCS = {resp['fcs']}")
...
"""
logger.info("For backwards compatibility, you can also enable summarization with a retriever, in which case the summary is added as an additional Document object:")
...
Since MQR uses an LLM we have to set that up - here we choose `ChatOllama`:
"""
logger.info("## Advanced LangChain query pre-processing with Vectara")
