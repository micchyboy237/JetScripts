...
"""
logger.info("# DatabricksVectorSearch")
...
"""
logger.info("### Create a Vector Search Endpoint and Index (if you haven't already)")
...
"""
logger.info("Next, we will create a new VectorSearch endpoint.")
...
"""
logger.info("Lastly, we will create an index that can be queried on the endpoint. There are two types of indexes in Databricks Vector Search and the `DatabricksVectorSearch` class support both use cases.")
...
"""
logger.info("Note: By default, similarity search only returns the primary key and text column. If you want to retrieve the custom metadata associated with the document, pass the additional columns in the `columns` parameter when initializing the vector store.")
