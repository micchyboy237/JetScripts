...
2. An api key from [Ollama](https://platform.ollama.com/)
...
Our chatbot integrates with ChatGpt via the langchain library, so you'll need an API key from Ollama first:

To get an api key for Ollama:
1. Register at https://platform.ollama.com/
...
"""
logger.info("## Part 2: Connect to Yellowbrick and create the embedding tables")
...
try:
    ...
    logger.debug(f"Table '{embedding_table}' created successfully!")
except psycopg2.Error as e:
...
logger.debug(yellowbrick_doc_connection_string)
...
logger.debug(f"Extracted {len(yellowbrick_documents)} documents successfully!")
...
"""
logger.info("## Part 4: Load the Yellowbrick Vector Store with Documents")
...
logger.debug(f"Created vector store with {len(documents)} documents")
...
"""
logger.info("## Part 6: Introducing an Index to Increase Performance")
