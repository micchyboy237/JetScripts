...
"""
logger.info("# SingleStoreVectorStore")
...
"""
### Vector index
Enhance your search efficiency with SingleStore DB version 8.5 or above by leveraging [ANN vector indexes](https://docs.singlestore.com/cloud/reference/sql-reference/vector-functions/vector-indexing/). By setting `use_vector_index=True` during vector store object creation, you can activate this feature. Additionally, if your vectors differ in dimensionality from the default Ollama embedding size of 1536, ensure to specify the `vector_size` parameter accordingly.
...
logger.debug(vectorResults[0].page_content)
...
logger.debug(textResults[0].page_content)
...
logger.debug(filteredByTextResults[0].page_content)
...
logger.debug(filteredByVectorResults[0].page_content)
...
logger.debug(weightedSumResults[0].page_content)
...
In the realm of multi-modal data analysis, the integration of diverse information types like images and text has become increasingly crucial. One powerful tool facilitating such integration is [CLIP](https://ollama.com/research/clip), a cutting-edge model capable of embedding both images and text into a shared semantic space. By doing so, CLIP enables the retrieval of relevant content across different modalities through similarity search.
...
"""
logger.info("## Multi-modal Example: Leveraging CLIP and OpenClip Embeddings")

# %pip install -U langchain ollama lanchain-singlestore langchain-experimental
