...
- Enabled the Ollama integration in your Astra DB organization,
...
"""
logger.info("### Server-side embedding initialization ("vectorize", method 2)")
...
vector_store_integrated_embeddings = AstraDBVectorStore(
    ...
    token=ASTRA_DB_APPLICATION_TOKEN,
    namespace=ASTRA_DB_KEYSPACE,
    collection_vector_service_options=ollama_vectorize_options,
)
...
"""
logger.info("### Auto-detect initialization (method 3)")
...
for res in results:
    logger.debug(f'* "{res.page_content}", metadata={res.metadata}')
...
"""
logger.info("#### Specify a different keyword query (requires hybrid search)")
