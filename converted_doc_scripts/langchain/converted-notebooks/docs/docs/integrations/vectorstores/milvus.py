...
"""
logger.info("Here we create a Milvus database:")
...
try:
    ...
    if db_name in existing_databases:
        ...
        for collection_name in collections:
            ...
            collection.drop()
            logger.debug(f"Collection '{collection_name}' has been dropped.")

        db.drop_database(db_name)
        logger.debug(f"Database '{db_name}' has been deleted.")
    else:
        logger.debug(f"Database '{db_name}' does not exist.")
...
"""
logger.info("Note the change in the URI below. Once the instance is initialized, navigate to http://127.0.0.1:9091/webui to view the local web UI.")
...
"""
logger.info("### Compartmentalize the data with Milvus Collections")
...
Here is a simple example of hybrid search in Milvus with Ollama dense embedding for semantic search and BM25 for full-text search:
"""
...
In the code above, we define an instance of `BM25BuiltInFunction` and pass it to the `Milvus` object. `BM25BuiltInFunction` is a lightweight wrapper class for [`Function`](https://milvus.io/docs/manage-collections.md#Function) in Milvus. We can use it with `OllamaEmbeddings`  to initialize a dense + sparse hybrid search Milvus vector store instance.
...
"""
logger.info("### Rerank the candidates")
...
"""
logger.info("To conduct a search using the partition key, you should include either of the following in the boolean expression of the search request:")
