...
"""
logger.info("maximum sequence length..."` can be ignored in this case — more details")

for d in docs[:3]:
    logger.debug(f"- {d.page_content=}")
...
## End-to-end Example
"""
logger.info("## End-to-end Example")
...
"""
logger.info("Defining the pipeline parameters:")
...
"""
logger.info("Now we can instantiate our loader and load documents:")
...
"""
logger.info("Determining the splits:")
...
"""
logger.info("Inspecting some sample splits:")
...
for i, doc in enumerate(resp_dict["context"]):
    ...
    logger.debug(f"  text: {json.dumps(clip_text(doc.page_content, threshold=350))}")
    for key in doc.metadata:
        if key != "pk":
            val = doc.metadata.get(key)
            clipped_val = clip_text(val) if isinstance(val, str) else val
            logger.debug(f"  {key}: {clipped_val}")
