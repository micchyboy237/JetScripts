...
"""
logger.info("# Google El Carro for Oracle Workloads")
...
"""
logger.info("If we still load langchain documents with default parameters of `ElCarroLoader`")
...
logger.debug(f"Loaded Documents: [{loaded_docs}]")
...
"""
logger.info("We can specify the content and metadata we want to load by setting")
...
- `document.metadata.organic` will be saved into `extra_json_metadata` column in
  JSON format.
"""
logger.info("Save documents with `ElCarroDocumentSaver.add_documents(<documents>)`. As you")
...
logger.debug(f"Original Document: [{doc}]")
...
loaded_docs = loader.load()
logger.debug(f"Loaded Document: [{loaded_docs[0]}]")
...
logger.debug(f"Documents left: {len(loader.load())}")
