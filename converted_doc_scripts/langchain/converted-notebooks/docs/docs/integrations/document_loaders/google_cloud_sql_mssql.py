...
"""
logger.info("# Google Cloud SQL for SQL server")
...
for doc in docs:
    logger.debug("Loaded documents:", doc)
...
"""
logger.info("If we still load langchain documents with default parameters of `MSSQLLoader` from this example table, the `page_content` of loaded documents will be the first column of the table, and `metadata` will be consisting of key-value pairs of all the other columns.")
...
"""
logger.info("We can specify the content and metadata we want to load by setting the `content_columns` and `metadata_columns` when initializing the `MSSQLLoader`.")
...
"""
logger.info("### Save document with customized page content & metadata")
...
"""
logger.info("Save documents with `MSSQLDocumentSaver.add_documents(<documents>)`. As you can see in this example,")
...
"""
logger.info("### Delete documents with customized page content & metadata")
