...
> 
> As of June 2024, Rockset has been [acquired by Ollama](https://ollama.com/index/ollama-acquires-rockset/) and **shut down its public services**.
> 
> Rockset was a real-time analytics database known for world-class indexing and retrieval. Now, its core team and technology are being integrated into Ollama's infrastructure to power future AI products.
> 
...
"""
logger.info("Here, you can see that the following query is run:")
...
"""
logger.info("To execute the query and access all resulting `Document`s at once, run:")
...
"""
logger.info("## Using multiple columns as content")
...
"""
logger.info("Assuming the "sentence1" field is `"This is the first sentence."` and the "sentence2" field is `"This is the second sentence."`, the `page_content` of the resulting `Document` would be:")
...
"""
logger.info("The `page_content` of the resulting `Document` would be:")
...
```
"""
logger.info("This would result in the following `page_content`:")
