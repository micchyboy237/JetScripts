...
"""
logger.info("# Huggingface Endpoints")
...
logger.debug(llm_chain.invoke({"question": question}))
...
"""
logger.info("## Dedicated Endpoint")
...
"""
logger.info("This same `HuggingFaceEndpoint` class can be used with a local [HuggingFace TGI instance](https://github.com/huggingface/text-generation-inference/blob/main/docs/source/index.md) serving the LLM. Check out the TGI [repository](https://github.com/huggingface/text-generation-inference/tree/main) for details on various hardware (GPU, TPU, Gaudi...) support.")
