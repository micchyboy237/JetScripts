...
```
pip install langchain langchain-ollama langchain-memgraph --user
```
...
"""
logger.info("To familiarize yourself with the data and verify the updated graph schema, you can print it using the following statement:")
...
"""
logger.info("Now you can start asking questions!")
...
"""
logger.info("#### Return query intermediate steps")
...
logger.debug(f"Final response: {response['result']}")
...
"""
logger.info("#### Limit the number of query results")
...
"""
logger.info("Below is an example text about Charles Darwin ([source](https://en.wikipedia.org/wiki/Charles_Darwin)) from which knowledge graph will be constructed.")
...
"""
logger.info("The next step is to initialize `LLMGraphTransformer` from the desired LLM and convert the document to the graph structure.")
...
"""
logger.info("Under the hood, LLM extracts important entities from the text and returns them as a list of nodes and relationships. Here's how it looks like:")

logger.debug(graph_documents)
...
"""
logger.info("### Storing into Memgraph")
...
"""
logger.info("Here's how the graph would like in such case:")
...
"""
logger.info("Here's how the graph would look like:")
...
"""
logger.info("The constructed graph would look like this:")
...
logger.debug(chain.invoke("Who Charles Robert Darwin collaborated with?")["result"])
