...
"""
logger.info("By default, all files with these MIME types can be converted to `Document`.")
...
"""
logger.info("You can customize the criteria to select the files. A set of predefined filter are proposed:")
...
"""
logger.info("Else, you can customize the prompt with a specialized `PromptTemplate`")
...
for doc in retriever.invoke("machine learning"):
    logger.debug(f"{doc.metadata['name']}:")
    logger.debug("---")
    logger.debug(doc.page_content.strip()[:60] + "...")
...
"""
logger.info("## Use Google Drive 'description' metadata")
