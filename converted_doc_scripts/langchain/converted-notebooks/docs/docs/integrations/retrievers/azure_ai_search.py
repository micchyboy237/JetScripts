...
With an API Key
"""
logger.info("# AzureAISearchRetriever")
...
"""
logger.info("With an Azure AD Token")
...
# azure_ollama_api_key: str = "<YOUR_AZURE_OPENAI_API_KEY>"
azure_ollama_api_version: str = "2023-05-15"
azure_deployment: str = "text-embedding-ada-002"

"""
We'll use an embedding model from Azure Ollama to turn our documents into embeddings stored in the Azure AI Search vector store. We'll also set the index name to `langchain-vector-demo`. This will create a new vector store associated with that index name.
"""
logger.info("We'll use an embedding model from Azure Ollama to turn our documents into embeddings stored in the Azure AI Search vector store. We'll also set the index name to `langchain-vector-demo`. This will create a new vector store associated with that index name.")

embeddings = AzureOllamaEmbeddings(
    model=azure_deployment,
    azure_endpoint=azure_endpoint,
    ollama_api_key=azure_ollama_api_key,
)
...
"""
logger.info("Next, we'll load data into our newly created vector store. For this example, we load the `state_of_the_union.txt` file. We'll split the text in 400 token chunks with no overlap. Finally, the documents are added to our vector store as emeddings.")
...
"""
logger.info("Next, we'll create a retriever. The current `index_name` variable is `langchain-vector-demo` from the last step. If you skipped vector store creation, provide your index name in the parameter. In this query, the top result is returned.")
...
"""
logger.info("Now we can retrieve the data that is relevant to our query from the documents we uploaded.")
