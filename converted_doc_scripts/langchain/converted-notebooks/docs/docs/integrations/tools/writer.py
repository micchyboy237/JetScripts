...
"""
logger.info("All tools are stored in the `tools` attribute of the `ChatWriter` instance:")
...
"""
logger.info("The tool choice mode is stored at the `tool_choice` attribute, which is `auto` by default:")
...
"""
logger.info("In the case of function tools, you will receive an assistant message with the tool call request.")
...
"""
logger.info("Then you can manually handle tool call request, send to model and receive final response:")
...
"""
logger.info("With a `GraphTool`, the model will call it remotely and return usage info in the `additional_kwargs` under the `graph_data` key:")

logger.debug(response.additional_kwargs["graph_data"])

"""
The `content` attribute contains the final response:
"""
logger.info("The `content` attribute contains the final response:")
