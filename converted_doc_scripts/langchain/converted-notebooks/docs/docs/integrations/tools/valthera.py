...
pip install ollama langchain jet.adapters.langchain.chat_ollama valthera langchain_valthera langgraph
...
def get_ollama_api_key() -> str:
    """Get Ollama API key with error handling."""
...
try:
    ...
    reasoning_engine = ReasoningEngine(
        llm=ChatOllama(
            model_name="gpt-4-turbo", temperature=0.0, ollama_api_key=api_key
        ),
        decision_rules=decision_rules,
    )
except ValueError as e:
    logger.debug(f"Error initializing reasoning engine: {e}")


try:
    api_key = get_ollama_api_key()  # Reuse the function for consistency
    trigger_generator = TriggerGenerator(
        llm=ChatOllama(
            model_name="gpt-4-turbo", temperature=0.7, ollama_api_key=api_key
        )
    )
except ValueError as e:
    logger.debug(f"Error initializing trigger generator: {e}")
...
try:
    ...
    logger.debug("Running Valthera agent workflow...")
    for response in graph.stream(inputs, stream_mode="values"):
        logger.debug(response)

except Exception as e:
    logger.debug(f"Error running Valthera workflow: {e}")
