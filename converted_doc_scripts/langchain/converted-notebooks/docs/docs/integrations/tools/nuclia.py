...
"""
logger.info("# Nuclia Understanding")
...
"""
logger.info("You can push files to the Nuclia Understanding API using the `push` action. As the processing is done asynchronously, the results might be returned in a different order than the files were pushed. That is why you need to provide an `id` to match the results with the corresponding file.")
...
"""
logger.info("You can now call the `pull` action in a loop until you get the JSON-formatted result.")
...
"""
logger.info("You can also do it in one step in `async` mode, you only need to do a push, and it will wait until the results are pulled:")
