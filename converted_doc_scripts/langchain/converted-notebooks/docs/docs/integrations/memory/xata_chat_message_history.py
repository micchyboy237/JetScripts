...
# %pip install --upgrade --quiet  xata langchain-ollama langchain langchain-community
...
"""
logger.info("## Create a simple memory store")
...
"""
logger.info("The above code creates a session with the ID `session-1` and stores two messages in it. After running the above, if you visit the Xata UI, you should see a table named `memory` and the two messages added to it.")
...
Let's now see a more complex example in which we combine Ollama, the Xata Vector Store integration, and the Xata memory store integration to create a Q&A chat bot on your data, with follow-up questions and history.

We're going to need to access the Ollama API, so let's configure the API key:
"""
logger.info("## Conversational Q&A chain on your data with memory")
...
"""
logger.info("To test, let's tell the agent our name:")
...
"""
logger.info("Now, let's now ask the agent some questions about Xata:")
...
"""
logger.info("Notice that it answers based on the data stored in the document store. And now, let's ask a follow up question:")
...
"""
logger.info("And now let's test its memory:")
