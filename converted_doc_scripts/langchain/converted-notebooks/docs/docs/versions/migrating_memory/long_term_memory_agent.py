...
"""
logger.info("# A Long-Term Memory Agent")

# %pip install -U --quiet langgraph langchain-ollama langchain-tavily tiktoken
...
def pretty_print_stream_chunk(chunk):
    for node, updates in chunk.items():
        ...
        if "messages" in updates:
            updates["messages"][-1].pretty_logger.debug()
        else:
...
"""
logger.info("You can see that the agent saved the memory about user's name. Let's add some more information about the user!")
...
"""
logger.info("We can then compile the graph exactly as before:")
