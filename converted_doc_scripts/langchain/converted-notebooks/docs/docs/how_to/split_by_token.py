...
:::note
[tiktoken](https://github.com/ollama/tiktoken) is a fast `BPE` tokenizer created by `Ollama`.
:::


We can use `tiktoken` to estimate tokens used. It will probably be more accurate for the Ollama models.
...
"""
logger.info("# How to split text by tokens")
...
"""
logger.info("To split with a [CharacterTextSplitter](https://python.langchain.com/api_reference/text_splitters/character/langchain_text_splitters.character.CharacterTextSplitter.html) and then merge chunks with `tiktoken`, use its `.from_tiktoken_encoder()` method. Note that splits from this method can be larger than the chunk size measured by the `tiktoken` tokenizer.")
...
"""
logger.info("To implement a hard constraint on the chunk size, we can use `RecursiveCharacterTextSplitter.from_tiktoken_encoder`, where each split will be recursively split if it has a larger size:")
...
"""
logger.info("We can also load a `TokenTextSplitter` splitter, which works with `tiktoken` directly and will ensure each split is smaller than chunk size.")
...
"""
logger.info("## SentenceTransformers")
...
logger.debug(text_token_count)

token_multiplier = splitter.maximum_tokens_per_chunk // text_token_count + 1

text_to_split = text * token_multiplier

logger.debug(f"tokens in text to split: {splitter.count_tokens(text=text_to_split)}")

text_chunks = splitter.split_text(text=text_to_split)

logger.debug(text_chunks[1])
...
"""
logger.info("## Hugging Face tokenizer")
