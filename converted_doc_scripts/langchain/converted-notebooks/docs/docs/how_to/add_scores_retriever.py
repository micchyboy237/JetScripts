...
"""
logger.info("# How to add scores to retriever results")
...
"""
logger.info("## SelfQueryRetriever")
...
"""
logger.info("Invoking this retriever will now include similarity scores in the document metadata. Note that the underlying structured-query capabilities of `SelfQueryRetriever` are retained.")
...
"""
logger.info("## MultiVectorRetriever")
...
"""
logger.info("Next we will add some fake "sub-documents" to our vector store. We can link these sub-documents to the parent documents by populating the `"doc_id"` key in its metadata.")
