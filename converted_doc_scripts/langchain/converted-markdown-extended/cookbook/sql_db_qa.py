...
"""
logger.info("# SQL Database Chain")
...
"""
logger.info("## Return Intermediate Steps")
...
"""
logger.info("Set up the SQLDatabase and LLM")
...
"""
logger.info("Now we need to add a place for memory in the prompt template")
...
"""
logger.info("Now let's create and run out chain")
...
"""
logger.info("## Choosing how to limit the number of rows returned")
...
"""
logger.info("## Adding example rows from each table")
...
"""
logger.info("The sample rows are added to the prompt after each corresponding table's column information:")

logger.debug(db.table_info)
...
"""
logger.info("CREATE TABLE "Track" (")
...
"""
logger.info("CREATE TABLE "Playlist" (")
...
"""
logger.info("Then limit the tables visible from SQLDatabase to the created view.")
...
"""
logger.info("## SQLDatabaseSequentialChain")
...
Sometimes you may not have the luxury of using Ollama or other service-hosted large language model. You can, ofcourse, try to use the `SQLDatabaseChain` with a local model, but will quickly realize that most models you can run locally even with a large GPU struggle to generate the right output.
"""
...
"""
logger.info("Loading checkpoint shards: 100%|██████████| 8/8 [00:32<00:00,  4.11s/it]")
...
"""
logger.info("This model should work for very simple SQL queries, as long as you use the query checker as specified above, e.g.:")
...
"""
logger.info("How many customers are there?")
...
"""
logger.info("huggingface/tokenizers: The current process just got forked, after parallelism has already been used. Disabling parallelism to avoid deadlocks...")
...
"""
logger.info("List all the customer first names that start with 'a'")
...
"""
logger.info("Now that you have some examples (with manually corrected output SQL), you can do few-shot prompt seeding the usual way:")
...
"""
logger.info("Using embedded DuckDB without persistence: data will be transient")
...
"""
logger.info("How many customers are from Brazil?")
...
"""
logger.info("How many customers are not from Brazil?")
...
"""
logger.info("How many customers are there in total?")
