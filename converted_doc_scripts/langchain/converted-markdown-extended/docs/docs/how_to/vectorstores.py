...
"""
logger.info("# How to create and query vector stores")
...
"""
logger.info("import Tabs from '@theme/Tabs';")
...
db = Chroma.from_documents(documents, OllamaEmbeddings(model="mxbai-embed-large"))
...
"""
logger.info("This walkthrough uses the `FAISS` vector database, which makes use of the Facebook AI Similarity Search (FAISS) library.")
...
"""
logger.info("This notebook shows how to use functionality related to the LanceDB vector database based on the Lance data format.")
...
db = LanceDB.from_documents(documents, OllamaEmbeddings(model="mxbai-embed-large"))
...
embedding_vector = OllamaEmbeddings(model="mxbai-embed-large").embed_query(query)
