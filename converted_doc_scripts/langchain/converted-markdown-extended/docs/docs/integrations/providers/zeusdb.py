...
async def main():
    ...
    logger.debug(f"Found the following {len(results)} results:")
    ...
    **Example 1: - Create from texts (creates index and adds texts in one step)**
    ...
    logger.debug("texts store count:", vector_store_texts.get_vector_count())         # -> 2
    logger.debug("texts store peek:", vector_store_texts.zeusdb_index.list(2))        # [('id1', {...}), ('id2', {...})]
    
    results = vector_store_texts.similarity_search("Hello", k=1)
    logger.debug(f"Found in texts store: {results[0].page_content}")                  # -> "Hello world"
    ...
    **Example 2: - Create from documents (creates index and adds documents in one step)**
    """
    logger.info("texts store count: 2")
    ...
    logger.debug("docs store count:", vector_store_docs.get_vector_count())           # -> 2
    logger.debug("docs store peek:", vector_store_docs.zeusdb_index.list(2))          # [('id3', {...}), ('id4', {...})]
    
    results = vector_store_docs.similarity_search("Python", k=1)
    logger.debug(f"Found in docs store: {results[0].page_content}")                   # -> "Python is great"
    ...
    **How to Load your vector store**
    ...
    logger.debug("vector count:", loaded_store.get_vector_count())
    logger.debug("index info:", loaded_store.info())
    logger.debug("store peek:", loaded_store.zeusdb_index.list(2))
    ...
    **Notes**
    ...
    """
    logger.info("### Advanced Search Options")
    ...
    logger.debug(results_with_scores)
    ...
    **Expected results:**
    ...
    """
    logger.info("#### MMR search for diversity")
    ...
    **Script version (`python my_script.py`):**
    """
    logger.info("All operations support async/await:")
    ...
    async def main():
        ...
        logger.success(format_json(ids))
        logger.debug("Added IDs:", ids)
        ...
        logger.success(format_json(results_fast, results_powerful))
        logger.debug("Fast results:", [d.page_content for d in results_fast])
        logger.debug("Powerful results:", [d.page_content for d in results_powerful])
    
        deleted = await vector_store.adelete(ids=ids[:1])
        logger.success(format_json(deleted))
        logger.debug("Deleted first doc:", deleted)
    ...
    **Expected results:**
    ...
    logger.debug(f"Index size: {stats.get('total_vectors', '0')} vectors")
    logger.debug(f"Dimension: {stats.get('dimension')} | Space: {stats.get('space')} | Index type: {stats.get('index_type')}")
    
    performance = vector_store.benchmark_search_performance(
        query_count=100,
        max_threads=4
    )
    logger.debug(f"Search QPS: {performance.get('parallel_qps', 0):.0f}")
    
    if vector_store.is_quantized():
        progress = vector_store.get_training_progress()
        logger.debug(f"Quantization training: {progress:.1f}% complete")
    else:
        logger.debug("Index is not quantized")
    ...
    **Expected results:**
    ...
    - Ollama(`nomic-embed-text`, `text-embedding-3-large`)
