...
"""
logger.info("## How to use tags with Log10")
...
llm = Ollama(model_name="gpt-3.5-turbo-instruct", callbacks=[log10_callback], temperature=0.5)
completion = llm.predict("You are a ping pong machine.\nPing?\n")
logger.debug(completion)

"""
You can also intermix direct Ollama calls and Langchain LLM calls:
"""
logger.info("You can also intermix direct Ollama calls and Langchain LLM calls:")
...
with log10_session(tags=["foo", "bar"]):
    ...
    llm = Ollama(model_name="text-ada-001", temperature=0.5)
...
"""
logger.info("## How to debug Langchain calls")
