...
- **Unified API Access**: Connect to 250+ LLMs (Ollama, Claude, Gemini, Groq, Mistral) through one API
...
You can connect to TrueFoundry's unified LLM gateway through the `ChatOllama` interface.
...
Connect to TrueFoundry by updating the `ChatOllama` model in LangChain:
"""
logger.info("### Basic Setup")


llm = ChatOllama(
    api_key=TRUEFOUNDRY_API_KEY,
    base_url=TRUEFOUNDRY_GATEWAY_BASE_URL,
    model="ollama-main/gpt-4o"  # Similarly you can call any model from any model provider
)
...
"""
logger.info("### LangGraph Integration")
...
"""
logger.info("## Observability and Governance")
