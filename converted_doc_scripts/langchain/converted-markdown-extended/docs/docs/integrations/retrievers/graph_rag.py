...
for doc in results:
    logger.debug(f"{doc.id}: {doc.page_content}")
...
"""
logger.info("### Comparison to Standard Retrieval")
...
"""
logger.info("This creates a retriever that starts with the nearest 5 animals (`start_k=5`),")
...
"""
logger.info("For either case, invoking the retriever returns:")
