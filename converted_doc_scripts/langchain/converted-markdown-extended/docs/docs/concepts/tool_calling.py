...
This need motivates the concept of *tool calling*. You can use [tool calling](https://platform.ollama.com/docs/guides/function-calling/example-use-cases) to request model responses that match a particular schema.
...
[Many](https://platform.ollama.com/docs/guides/function-calling) [model providers](https://platform.ollama.com/docs/guides/function-calling) support tool calling.
...
"""
logger.info("As a specific example, let's take a function `multiply` and bind it as a tool to a model that supports tool calling.")
...
"""
logger.info("The result would be an `AIMessage` containing the model's response in natural language (e.g., "Hello!").")
