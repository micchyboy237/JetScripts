...
| **function** (legacy) | This is a legacy role, corresponding to Ollama's legacy function-calling API. **tool** role should be used instead.                                                                                             |
...
- **Legacy** [FunctionMessage](#legacy-functionmessage): corresponds to the **function** role in Ollama's **legacy** function-calling API.
...
"""
logger.info("When invoking a chat model with a string as input, LangChain will automatically convert the string into a `HumanMessage` object. This is mostly useful for quick testing.")
...
"""
logger.info("#### Multi-modal Content")
...
- A **list of dictionaries** -- Each dictionary represents a content block and is associated with a `type`.
    * Used by Ollama for surfacing agent thought process when doing [tool calling](/docs/concepts/tool_calling).
    * Used by Ollama for audio outputs. Please see [multi-modal content](/docs/concepts/multimodality) for more information.
...
This is a legacy message type, corresponding to Ollama's legacy function-calling API. `ToolMessage` should be used instead to correspond to the updated tool-calling API.

## Ollama Format

### Inputs

Chat models also accept Ollama's format as **inputs** to chat models:
"""
logger.info("### ToolMessage")
...
At the moment, the output of the model will be in terms of LangChain messages, so you will need to convert the output to the Ollama format if you
need Ollama format for the output as well.

The [convert_to_ollama_messages](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.utils.convert_to_ollama_messages.html) utility function can be used to convert from LangChain messages to Ollama format.
