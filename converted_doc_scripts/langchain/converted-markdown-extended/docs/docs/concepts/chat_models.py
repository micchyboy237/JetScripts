...
* Integrations with many chat model providers (e.g., Ollama, Ollama, Ollama, Microsoft Azure, Google Vertex, Amazon Bedrock, Hugging Face, Cohere, Groq). Please see [chat model integrations](/docs/integrations/chat/) for an up-to-date list of supported models.
* Use either LangChain's [messages](/docs/concepts/messages) format or Ollama format.
...
LangChain chat models are named with a convention that prefixes "Chat" to their class names (e.g., `ChatOllama`, `ChatOllama`, `ChatOllama`, etc.).
...
However, LangChain also has implementations of older LLMs that do not follow the chat model interface and instead use an interface that takes a string as input and returns a string as output. These models are typically named without the "Chat" prefix (e.g., `Ollama`, `Ollama`, `Ollama`, etc.).
These models implement the [BaseLLM](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.llms.BaseLLM.html#langchain_core.language_models.llms.BaseLLM) interface and may be named with the "LLM" suffix (e.g., `OllamaLLM`, `OllamaLLM`, `OllamaLLM`, etc.). Generally, users should not use these models.
:::
...
2. **Ollama's Message Format**: Ollama's message format.
...
- Standard parameters are currently only enforced on integrations that have their own integration packages (e.g. `langchain-ollama`, `langchain-anthropic`, etc.), they're not enforced on models in `langchain-community`.
