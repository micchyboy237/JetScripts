...
"""
logger.info("# LangChain Expression Language (LCEL)")
...
"""
logger.info("Invoking the `chain` with some input:")
...
"""
logger.info("corresponds to the following:")
...
"""
logger.info("Will yield a `final_output` dictionary with the same keys as the input dictionary, but with the values replaced by the output of the corresponding runnable.")
...
"""
logger.info("## Composition Syntax")
...
"""
logger.info("It gets automatically converted to the following:")
...
"""
logger.info("You have to be careful because the `mapping` dictionary is not a `RunnableParallel` object, it is just a dictionary. This means that the following code will raise an `AttributeError`:")
...
It gets automatically converted to the following:
"""
logger.info("#### Function to RunnableLambda")
...
"""
logger.info("You have to be careful because the lambda function is not a `RunnableLambda` object, it is just a function. This means that the following code will raise an `AttributeError`:")
