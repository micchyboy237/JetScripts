...
`langchain`, `langchain-core`, `langchain-text-splitters`, and integration packages (`langchain-ollama`, `langchain-anthropic`, etc.) follow [semantic versioning](https://semver.org/) in the format of 0.**Y**.**Z**. The packages are under rapid development, and so are currently versioning the packages with a major version of 0.
...
"""
logger.info("# LangChain release policy")
