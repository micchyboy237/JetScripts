...
[![](https://img.shields.io/badge/Open%20on%20Github-grey?logo=github)](https://github.com/microsoft/autogen/blob/main/dotnet/samples/AutoGen.MLX.Sample/Tool_Call_With_Ollama_And_LiteLLM.cs)
...
[!code-csharp[Define WeatherReport function](../../samples/AutoGen.MLX.Sample/Tool_Call_With_Ollama_And_LiteLLM.cs?name=Function)]
...
[!code-csharp[Define WeatherReport function](../../samples/AutoGen.MLX.Sample/Tool_Call_With_Ollama_And_LiteLLM.cs?name=Create_tools)]

## Create @AutoGen.MLX.MLXChatAgent with `GetWeatherReport` tool and chat with it

Because LiteLLM proxy server is openai-api compatible, we can use @AutoGen.MLX.MLXChatAgent to connect to it as a third-party openai-api provider. The agent is also registered with a @AutoGen.Core.FunctionCallMiddleware which contains the `WeatherReport` tool. Therefore, the agent can call the `WeatherReport` tool when generating a response.

[!code-csharp[Create an agent with tools](../../samples/AutoGen.MLX.Sample/Tool_Call_With_Ollama_And_LiteLLM.cs?name=Create_Agent)]
