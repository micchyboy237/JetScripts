...
"""
logger.info("# AutoGen Python Development Guide")
...
"""
logger.info("To serve the documentation locally, run:")
...
"""
logger.info("When you make changes to the doc strings or add new modules, you may need to")
            ...
            async def main() -> None:
                ...
                async def async_func_36():
                    async with McpWorkbench(server_params=params) as workbench:
                        async def run_async_code_08ec5e83():
                            tools = await workbench.list_tools()
                            return tools
                        tools = asyncio.run(run_async_code_08ec5e83())
                        logger.success(format_json(tools))
                        logger.debug(tools)
                        async def run_async_code_7169854a():
                            result = await workbench.call_tool(tools[0]["name"], {"url": "https://github.com/"})
                            return result
                        result = asyncio.run(run_async_code_7169854a())
                ...
                result = asyncio.run(async_func_36())
                logger.success(format_json(result))
            asyncio.run(main())
