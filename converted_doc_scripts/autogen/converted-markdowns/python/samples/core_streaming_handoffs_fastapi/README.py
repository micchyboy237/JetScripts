...
"""
logger.info("# AutoGen-Core Streaming Chat with Multi-Agent Handoffs via FastAPI")
                        ...
                        if line:
                            ...
                            if 'content' in data and isinstance(data['content'], dict) and 'message' in data['content']:
                                ...
                                if message_type == 'function':
                                    logger.debug(f"[{message_type.upper()}] {message_content}", end='\n', flush=True) # Print function calls on new lines for clarity
                                    logger.debug("<<< Assistant: ", end="", flush=True) # Reprint prefix for next string part
                                else:
                                    logger.debug(message_content, end='', flush=True)
