...
async def main():
    ...
    Refer to the documentation from [Ollama](https://platform.ollama.com/docs/guides/function-calling) 
    and [Ollama](https://docs.anthropic.com/en/docs/build-with-claude/tool-use) for more information about tool calling in LLMs.
    ...
    which can be used as a tool by the agent and automatically generates the tool schema from the function signature and docstring.
    ...
    For {py:class}`~jet.adapters.autogen.ollama_client.OllamaChatCompletionClient` and {py:class}`~jet.adapters.autogen.ollama_client.AzureOpenAIChatCompletionClient`,
    ...
    - {py:class}`~autogen_ext.agents.ollama.OpenAIAssistantAgent`: An agent that is backed by an Ollama Assistant, with ability to use custom tools.
