...
import time
from jet.transformers.object import make_serializable, prettify_value
from jet.logger import logger

file_dir = os.path.dirname(os.path.realpath(__file__))
log_dir = os.path.join(file_dir, "jet-logs")
os.makedirs(log_dir, exist_ok=True)
logger.log("Log dir:", log_dir, colors=["GRAY", "INFO"])

# Initial unique log filename
timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
log_file_path = os.path.join(log_dir, f"setup_{timestamp}_{
                             int(time.time())}.log")
...
def response(flow: http.HTTPFlow):
    ...
    if flow.request.path.startswith("/api/chat") or flow.request.path.startswith("/api/generate"):
        ...
        logger.log("Saved File Path:", log_file_path,
